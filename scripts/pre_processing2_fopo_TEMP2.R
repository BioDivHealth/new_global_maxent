...
ws2<-rasterize(wrld_simpl,template)
#removeTmpFiles(1)
...
##land cover
#futlg<-list.files("X:\\DRtemp\\LUH1\\",recursive = TRUE ,pattern=".tif",full.names=TRUE)
...
#lc2<-disaggregate(lc2,12,method='bilinear')#/144
#lc2[lc2<0]<-0
#lc2[lc2>1]<-1
#lc2<-mask(lc2,ws2)
#lc2<-brick("X:\\DRtemp\\temp\\land_cover_masked.tif")
names(lc2)<-names(lc)

#writeRaster(lc2,format="GTiff",file="X:\\DRtemp\\temp\\land_cover_masked.tif")
#system.time(
#plot(disaggregate(subset(lc,1),12,method=''))#/144
#)
