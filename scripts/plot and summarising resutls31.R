 ...
 #                      size=2,pch=21)
  ...
  ##make dummy
    ...
    if(jj==4){res6x$sort<-paste0(res6x$range,res6x$group);xxx<-"Both"}
      ...
      cordata1<-data.frame(disease=unique(resN$disease),correlation=slp,bootstrap=x,NDIS=NDIS[qq],ndis=length(unique(resN$disease)),panel=types[tt],panel2=rr,pval=sim_slp,sd=sim_sd,vars=rr)
