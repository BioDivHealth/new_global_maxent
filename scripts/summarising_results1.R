...
breaks2<-seq(from=0,to=max(log(popgdp7$gdp_ssp1+1),na.rm=TRUE),by=(max(log(popgdp7$gdp_ssp1+1),na.rm=TRUE)/10))
...
##agregate
res12<-	res11[res11$auc>0.5 & cases_per_year>10 ,lapply(.SD,function (x) sum(x,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=names(res11)[c(22:36,ncol(res11))]]
