 ...
 #                      size=2,pch=21)
  ...
  res7z<-res7z[,-1]
   ...
   res7<-rbind(res7,res7z)
    ...
    res7b<-res7[!is.na(res7$correlation) & res7$panel2!="fungi" & res7$panel!="Emerging" & res7$panel!="KingTrans",] #& res7$panel=="RCP"
   ...
   # res7b$Trend<-gsub("0.5","0.25",res7b$Trend,fixed=TRUE)
    ...
    #write.csv(res11c,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure16092022.csv")
    
    res11c<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure16092022.csv")

    not_res=c("")
    
    res11c$type<-factor(res11c$type,levels=c("Range","Cases","Poor Cases"))
    res11c2$type<-factor(res11c2$type,levels=c("Range","Cases","Poor Cases"))
    
    gg<-
      ggplot(data=res11c[res11c$SSP=="SSP  0",],aes(x=type,y=Count,col=(type))) +
    ...
    stat_summary(data=res11c2[res11c$SSP=="SSP  0",],aes(x=type,y=Count),col="grey",alpha=0.75,fun =  mean, geom = "point",size=2) +#,position = position_dodge(3)) + 
    stat_summary(data=res11c2[res11c$SSP=="SSP  0",],aes(x=type,y=Count),col="grey",alpha=0.75,fun.data=stat_HPDI, geom = "errorbar",width=0.2) +#,position = position_dodge(3)) + 
    ...
    xlab("")+
    ylab("Ratio of Expanding\nto Non-Expanding Ranges")+
