...
diseases<-read.csv(file="C:\\Users\\xxxx\\Dropbox\\legion2\\diseases1.csv")
...
#po#ints1<-list.files("X:\\DRtemp\\temp\\points\\", pattern="_points",full.names=FALSE)
...
link2b<-list.files("V:/resultsY2", pattern=".tif",full.names=TRUE)
link2<-list.files("V:/resultsY2", pattern=".tif",full.names=FALSE)
...
### problems "babesia venatorum" "schistosoma mekongi" "tensaw"
#6,41
...
for (x in 1:length(dis1)){
  ...
  #print(x)
  if(x==21){dis1[x]<-sort(unique(diseases$disease))[21]}
  if(dis1[x]=="tensaw"){dis1[x]<-"tensaw "}
  #if(x==198){dis1[x]<-sort(unique(diseases$disease))[198]}
  

  #### NUMBER 42 not in database - why???
  ## also: number c(42,63,56,54,50,51)
  suppressWarnings(rm(dis2))
  ##choose one disease
	dis2<-diseases[diseases$disease==dis1[x],]
	if(length(dis2$tag[!is.na(dis2$tag)])==0){print("PROBLEM1");next}
	
	dis2<-dis2[!(dis2$name1 %in% c("ducks","chickens","cattle","ducks","goats","human","pigs","sheep")),]			
	...
	#if(paste(" ",dis1[x]," ALL4.r",sep="") %in% list.files(".\\outputs\\", pattern="ALL4.r",full.names=FALSE)){next}
	if(dis1[x]=="tensaw "){dis1[x]<-"tensaw"}
	

		##endemic region
	if(x==21){dis_trans<-d1[d1$name=="babesia venatorum",]}else{	dis_trans<-d1[d1$name==dis1[x],]}
	ccc<-strsplit(dis_trans$countries,",")[[1]]
	#if(length(ccc)>=25){print("not widespread");next}
	...
	if(length(bothmod)==0){print("ERROR NO SCENARIO OVERLAP");print("PROBLEM2");print(dis2)}

	##randomly choose hosts if too many
	#if(nrow(dis2[dis2$type=="hosts"])>25)	{
	...
	#if(nrow(dis2[dis2$type=="vectors"])>25)	{
	...
	link4<-link3[link3$species %in% dis2$name1,]
	...
	if (x==1){fin1<-link4} else {fin1<-rbind(fin1,link4)}
	
	print(x)
	
  }

  fin1<-fin1[!duplicated(fin1$filen),]
  
  save(fin1,file="D:\\fin1.r")
  ...
  for(i in 1:nrow(fin1)){
    ...
    if(filen %in% list.files("D:\\ResultsY2",full.names=TRUE)){next}
    
    rrr<-raster(fin1[i,"filen"])
    
    writeRaster(rrr,file=filen,overwrite=TRUE)
    print(i)
		...
		for(z in 1:nrow(dis2)){
		  ...
		  #z=z+1
			...
			link4<-link4[link4$year!=2010,]
					...
					t1<-list.files("V:\\new_present\\",full.names=TRUE)[list.files("V:\\new_present\\")==paste0(link5$species,"_present_XXX.tif") ]
    ...
    #save present day
    save(res6,file=paste("V:/per_disease2/",dis3$disease,"ALL4.r"))
    rm(res6);gc()
