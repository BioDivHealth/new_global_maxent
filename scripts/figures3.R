...
res12<-res11[!is.na(year),names(res11)[c(99,3:6,23:34,81:97)],with=F]
...
for (i in 1:length(uuu)){
  ...
  expo<-ggplot(data=res15,aes(x=year,y=Exposure))+
    geom_smooth(method="loess",se=TRUE,span=1)+
    geom_point()+
    #facet_grid(group3~burden,scales="free_y")+
    #geom_hline(yintercept = 1,lty=2)+
  ...
  vuln<-ggplot(data=res15,aes(x=year,y=Vulnerability))+
    geom_smooth(method="loess",se=TRUE,span=1)+
    geom_point()+
    #facet_grid(group3~burden,scales="free_y")+
    #geom_hline(yintercept = 1,lty=2)+
...
png(paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\test_plots\\plot2_",uuu[i],".png",sep=""))

grid.arrange(haz,ris1,area1,burd1,expo,vuln,nrow=3,top=paste(uuu[i],"Vul.=",round(res15$vw[1]/res15$area,2),"Exp.=",round(res15$ew[1]/res15$area,2),sep=" ") ,bottom="Year")
