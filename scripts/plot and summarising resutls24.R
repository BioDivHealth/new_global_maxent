 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  for(qq in 1:length(NDIS)){
    ...
    for (x in 1:100){
      ...
      cordata1<-data.frame(disease=unique(resN$disease),correlation=slp,bootstrap=x,NDIS=length(unique(resN$disease)))
      ...
      if(x==1) {resN2<-resN} else {resN2<-rbind(resN2,resN)}
      
      print(x)
    }
  
    if(qq==1) {res7<-resN2} else {res7<-rbind(res7,resN2)}
    ...
    res7$uni<-paste(res7$bootstrap,res7$NDIS)
    
    
    res7b<-res7[!duplicated(paste0(res7$correlation,res7$bootstrap,res7$NDIS)),]
    ...
    res7c<-aggregate(res7b$uni2,by=list(res7b$bootstrap,res7b$NDIS,res7b$Trend),sum)
    
    names(res7c)<-c("bootstrap","NDIS","Trend","Count")
    
    ggplot(res7c,aes(x=Trend,y=Count,group=bootstrap,fill=NDIS)) +
    #res7$uni<-paste(res7$SSP,res7$rep)]
