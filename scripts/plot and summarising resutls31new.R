...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  p<-ggplot(r6x[ !r6x$ff %in% c("2.6_3","2.6_5","2.6_2") & r6x$threshold=="ALL",],aes(y=lm2_fitted2,x=jitter(Year),col=SSP)) +
  ...
  typesX<-c("RCP/SSP")#c("Baseline","Vectored","Emerging","Kingdom","Tropical/Temperate")
  ...
  ##make dummy
    ...
    if(jj==4){res6x$sort<-paste0(res6x$range,res6x$group);xxx<-"Both"}
      ...
      for(i in 1:length(unique(resN$disease))){
        ...
        sim_slp2[i]<-mean(cor2,na.rm=TRUE)
        sim_sd2[i]<-sd(cor2,na.rm=TRUE)
        sim_slp3[i]<-mean(cor3,na.rm=TRUE)
        sim_sd3[i]<-sd(cor3,na.rm=TRUE)
      ...
      cordata1<-data.frame(disease=unique(resN$disease),correlation=slp,correlation2=slp2,correlation3=slp3,bootstrap=x,NDIS=NDIS[qq],ndis=length(unique(resN$disease)),panel=types[tt],panel2=rr,pval=sim_slp,sd=sim_sd,pval2=sim_slp2,sd=sim_sd2,pval3=sim_slp3,sd=sim_sd3,vars=rr)
