...
conts <- stack(list.files(here("data","conts"), pattern = ".tif", full.names = T))
...
load(file = here("scripts", "functions", "regions.r"))
load(file = here("scripts", "functions", "subregions.r"))
source(file = "scripts/functions/functions7.R")

# Note: this is a sample file. Please unzip to use on your computer.
xxx1 <- list.files(here("data", "worldclim"), full.names = TRUE, pattern = ".tif")
...
# Note: this is a sample file. Please unzip to use on your computer.
lcf <- stack(list.files(here("data", "MODIS"), full.names = TRUE, pattern = ".tif"))
...
writeRaster(lc, format = "GTiff", file = here("data", "MODIS", "lc.tif"))
# lc<-stack("data/MODIS/lc.tif")
...
# Note: this is a sample file. Please unzip to use on your computer.
invar <- stack(list.files(here("data", "worldclim", "invariant_data", "wc2.1_30s_elev.zip.download"), pattern = ".tif", full.names = TRUE))
...
# writeRaster(invar2,format="GTiff",file="data/worldclim/invariant_data/invar2.tif")
# invar<-stack(file="data/worldclim/invariant_data/invar.tif")
# for (i in 1:nlayers(invar)){
# 	invar2<-(subset(invar,i))
# 	writeRaster(invar2,format="GTiff",paste(file="data/worldclim/invariant_data/invar_",names(invar)[i],".tif",sep=""))
# 	}

# invar<-stack(list.files("data/worldclim/",pattern=".tif",full.names=TRUE))
# alt<-raster("data/w001001.adf")
...
#
## load invariant data
# invar<-stack("data/worldclim/invariant_data/invar2.tif")
# names(invar)<-c("DEMSRE1a","INMSRE1a","INSSRE1a","OPISRE1a","SBDHWS1a","SCLHWS1a","SGRHWS1a","SLPSRT1a","SOCHWS1a","SSLHWS1a","SSNHWS1a","TBDHWS1a","TCLHWS1a","TGRHWS1a","TOCHWS1a","TPHHWS1a","TSLHWS1a","TSNHWS1a","TWISRE1a")
...
# writeRaster(spdata,format="GTiff",file="data/predictorsX.tif",overwrite=TRUE)
# names1<-names(spdata)
# write.csv(names1,file="data/names1.csv")
# spdata<-stack("data/predictorsX.tif")
names(spdata) <- read.csv(here("data", "names1.csv"), stringsAsFactors = FALSE)$x

############## PROCESS SPECIES #################

ID <- function() {
    return(paste(sample(c(0:9, letters, LETTERS), size = 12, replace = TRUE), collapse = ""))
}
...
for (i in i:(nrow(spec1b))) {
    ...
    if (!nrow(dom) == 0) {
        for (y in 1:nrow(dom)) {
          print(paste("Processing", dis, "with", dom$type[y], ":", dom$species[y]))
            write.table(
               data.frame(disease = dis, type = dom$type[y], name1 = tolower(dom$species[y])),
               file = here("data", "disease_analyses_artur", paste0(ID(), "_XXX.csv")),
               sep = ",")}}
    ...
    if (!nrow(hv2) == 0) {
        ...
        while (z <= nrow(hv2)) {
            ...
            cat("data1$key[1]:", data1$key[1], "\n")
            cat("data1$key[length(data1$key)]:", data1$key[length(data1$key)], "\n")
            cat("data1$sens:", data1$sens, "\n")
            cat("sort(geog1$sr1):", sort(geog1$sr1), "\n")
            ...
            write.csv(
              data.frame(disease = dis, type = hv2$type[z], name1 = name1),
              file = here("data", "disease_analyses_artur", paste0(name1, "-", ID(), ".csv")))
            
            if (here("data", "disease_analyses_artur", 
                     paste0(name1, "_points.r")) %in% 
                list.files(here("data", "disease_analyses_artur"), full.names = TRUE)) {
                print(z)
                z <- z + 1
                next
            }
            save(data1, file = here("data", "disease_analyses_artur", paste0(name1, "_points.r")))
            # save(xm, file=paste("data/disease_analyses_artur/",name1,"_maxent.r",sep=""))}
            print(z)
            z <- z + 1
            gc()
        }
