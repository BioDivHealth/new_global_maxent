...
# ============================|
#   Set Raster Template/Mask  ----
# ============================|

## Set up base raster template for global analysis
template <-
  raster(
    nrow = 3600,
    ncol = 8640,
...
# Utility: Combine columns into data.table/data.frame
...
# Years and RCP scenarios for projections
...
data(wrld_simpl)
# Assign correct subregion for Russia
...
# ============================|
#   Load Disease Lookup Tables ----
# ============================|
load(file = "scripts/functions/dis1.r") # Lookup for disease metadata

diseases <- read.csv(file = "data/diseases1.csv", stringsAsFactors = FALSE, encoding = "latin1")

# ============================|
#   Gather Species Occurrence Files ----
# ============================|
# Find all occurrence data files for each species
points1a <- list.files("data/points", pattern = "all_points", full.names = TRUE)
points1 <- list.files("data/points", pattern = "_points", full.names = TRUE)
points2b <- points1[!points1 %in% points1a]
points1a <- list.files("data/points", pattern = "all_points", full.names = FALSE)
points1 <- list.files("data/points", pattern = "_points", full.names = FALSE)
...
# ============================|
#   Load Livestock Density Data ----
# ============================|
# Gridded livestock data for future projections
lt2 <- fread("≈livestock_future_2030_2050_2070_2080b.csv")
setkey(lt2, cell_by_year)


# ============================| 
#   Load Future Climate Niche Data ----
# ============================|
# Lists of raster files for future climate scenarios
link2b <- list.files(
...
# Clean and parse filenames for scenario extraction
link2 <- gsub("_6.tif", "_6.0.tif", link2, fixed = TRUE)
link2 <- gsub("_present_", ";2010;present", link2, fixed = TRUE) # Insert year and present marker
...
# ============================| 
#   Parse Scenario Table ----
# ============================|
# Create table with scenario info for each raster
...
###
# ============================| 
#   Load Future Land Use Data ----
# ============================|
futlg <- read.csv(file = "data/futlg.csv", stringsAsFactors = FALSE)
head(futlg)
# Clean up file paths for future land use rasters
futlg$filen <- gsub("C:/", "OS/Users/Public/Documents/", futlg$filen, fixed = TRUE)
futlg$filen <- gsub("/updated_states", "", futlg$filen, fixed = TRUE)
futlg$filen <- gsub("LUH1//", "LUH1/", futlg$filen, fixed = TRUE)

# ============================| 
#   Load Habitat, Density, and Dispersal Data ----
# ============================|
# Forest/non-forest land raster
fnf <- raster("data/fnf_map.txt")
plot(fnf)

# Species habitat preferences
spec_hab <- read.csv("data/habitat_both_fin.csv", stringsAsFactors = FALSE)
...
# Host density and dispersal distance data
spec_dist <- read.csv("data/host_density_d_distance.csv", stringsAsFactors = FALSE)
spec_desp <- read.csv("data/dispersal_distance_all.csv", stringsAsFactors = FALSE)
...
### NOTE: ZIKA transmission model is currently set to HUMAN->VECTOR->HUMAN, may need generalization
...
for (x in (x + 1):length(dis1)) {
  ...
  # Select records for current disease
  dis2 <- diseases[diseases$disease == dis1[x], ]
  # Check for missing tag info
  if (length(dis2$tag[!is.na(dis2$tag)]) == 0) {
    print("PROBLEM1")
  }

  # Remove duplicate names for current disease
  dis2 <- dis2[!duplicated(dis2$name1), ]

  # Skip if output for this disease already exists
  ...
  # ============================ | 
  #   Define Endemic and Pseudoabsence Regions ----
  # ============================|
  dis_trans <- d1[d1$name == dis1[x], ]
  if (dis_trans$no.countries > 150) {
    next # Skip widespread diseases
  }
  ...
  # Define subregion and region for pseudoabsence sampling
  regn <- wrld_simpl[wrld_simpl$SUBREGION %in% countr$SUBREGION, ]
  regn2 <- wrld_simpl[wrld_simpl$REGION %in% countr$REGION, ]
  regn2 <- suppressWarnings(crop(regn2, remove_small_islands(regn2, min_val = 10)))
  template2 <- crop(template, regn2)

  # ============================ | 
  #   Find Overlap of Host/Vector Scenarios ----
  # ============================|
  ...
  # Limit number of host/vector SDMs per disease for computational efficiency
  ...
  # Randomize order of records for unbiased processing
  dis2 <- dis2[sample(1:nrow(dis2), nrow(dis2), replace = FALSE), ]
  st2 <- Sys.time() # Timing for performance tracking
  # ============================ | 
  #   Loop Over Disease Records (Hosts/Vectors) ----
  # ============================|
  for (z in 1:nrow(dis2)) {
    dis3 <- dis2[z, ] # Current host/vector record
    # Skip non-wild hosts (GLW data)
    if (dis3$name1 %in% c("ducks", "chickens", "cattle", "goats", "human", "pigs", "sheep")) {
      next
    }
    # Load known locations for this record
    load(file = as.vector(points3[points3$species == dis3$name1, "filen"])) # loads data1$data1
    ...
    # Only keep valid species names with <2 spaces
    no_spaces <- function(x, what = " ") lengths(regmatches(x, gregexpr(what, x)))
    species1 <- unique(dt1$names2[!is.na(dt1$names2)])
    species1 <- species1[no_spaces(species1) < 2]
    num_spec <- length(species1)
    # Remove numeric-only names
    species1 <- species1[sapply(species1, FUN = function(x) is.na(as.numeric(x) == ((as.numeric(x)) / 1)))]
    # Extract genus if available
    if (length(species1) > 0) {
      species2 <- unique(read.table(text = species1, sep = " ", stringsAsFactors = FALSE)$V1)
    } else {
      species2 <- c()
    }
    # Parse habitat info from name
    ...
    if (length(strsplit(species1a, " ")[[1]]) > 1) {
      species2a <- strsplit(species1a, " ")[[1]][1]
    } else {
      species2a <- species1a
    }
    ...
    if (nrow(spec_hab2) == 0) {
      ...
      lcsuit <-
        (f1 * mean(spec_hab2$f, na.rm = TRUE)) + (nf1 * mean(spec_hab2$nf,
          na.rm =
            TRUE
        )) + (water * mean(spec_hab2$water, na.rm = TRUE)) + (mean(spec_hab2$Grasslands,
          na.rm =
            TRUE
        ) * subset(lc2, (1:nlayers(lc2))[futlh$variable == "gpast"])) + (mean(spec_hab2$Croplands,
          na.rm =
            TRUE
        ) * subset(lc2, (1:nlayers(lc2))[futlh$variable == "gcrop"])) + (mean(spec_hab2$Urban.and.built.up,
          na.rm =
            TRUE
        ) * subset(lc2, (1:nlayers(lc2))[futlh$variable == "gurbn"]))
      meds4 <- NA
    }
    ...
    res3 <- foreach(
      ...
      res_foreach[, cell_by_year_by_RCP := paste(cell.id, year, RCP,
        sep =
          "_"
      )]
  ...
  lt3 <-
    lt2[, c("cell_by_year", names(lt2)[tolower(names(lt2)) %in% dis2$name1]),
      with =
        FALSE
    ]
