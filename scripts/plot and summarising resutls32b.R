...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  ##make subset
  res7<-res6[ res6$AUC>0.675 & res6$RCP %in% c(6.0) & res6$Year %in% c(2070),]
  ...
  gg <- gg + scale_shape_manual(values=c(21,24))
 ...
 #                      size=2,pch=21)
  gg <- gg + geom_point( aes(x=(Longitude), y=change_max_y),fill="00000000",col="00000000",size=2,pch=21)
  gg <- gg + geom_point( aes(x=(Longitude), y=change_min_y),fill="orange",size=2,pch=21)
  #gg <- gg + geom_line( aes(x=(change_x), y=change_y, group=disease),lty=2)
  gg <- gg+theme_cowplot()
  gg <- gg + geom_hline(yintercept = 0)
  ...
  ##for figure 1
  typesX<-c("Baseline","Vectored","Emerging","Kingdom","Tropical/Temperate")
  ...
  for(tt in 1:length(types)){
    ...
    for(ff in 1:100){
      ...
      samp2<-c("Africa","Oceania","North America","South America","Asia","Europe")#,6)
  ...
  #x=1
  ###foreach
  NDIS<-120#c(40,80,120,160) ###check sim p value
  ...
  ##make dummy
    ...
    if(jj==4){res6x$sort<-paste0(res6x$range,res6x$group);xxx<-"Both"}
      ...
      res6y[,cor1:=cor(richness,Year,method="spearman",use="complete.obs")[1]
            ,by=disease]
      
      res6y[,cor2:=cor(cases,Year,method="spearman",use="complete.obs")[1]
            ,by=disease]
      
      res6y[,cor3:=cor(poor_cases,Year,method="spearman",use="complete.obs")[1]
            ,by=disease]
      ...
      ##so as to give RCP SSP different values
      res6y[,disease2:=disease]
      if(types[tt]=="RCP/SSP"){res6y[,disease2:=paste(disease,RCP,SSP,sep="_")]}
      if(types[tt]=="RCP"){res6y[,disease2:=paste(disease,RCP,sep="_")]}
      if(types[tt]=="SSP"){res6y[,disease2:=paste(disease,SSP,sep="_")]}
      
      nam1<-names(res6y)[!names(res6y) %in% c("disease2","disease","RCP","SSP","cor1","cor2","cor3","lm1","lm2","lm3")]
      
      res6y[,(nam1):=NULL]
      res6y<-res6y[!duplicated(disease2),]
      ...
      for(i in 1:length(unique(resN$disease))){
        ...
        dis1<-unique(resN2$disease)[i]
      
        r3<-resN2[resN2$disease==dis1 ,]
        ...
        for (hh in 1:reps){
          ...
          #lm1<-lm(data=r3,formula="richness~Year")$coefficients
          lm2<-cor(r4$richness,r4$Year,method="spearman",use="complete.obs")[c(1,1)]
          cor1[hh]<-lm2[2]
        
           }## and of hh simulate
          
        sim_slp[i]<-mean(cor1,na.rm=TRUE)
        
        sim_sd[i]<-sd(cor1,na.rm=TRUE)
        #if(is.null(cor2)){cor2<-cor1} else {cor2<-c(cor2,cor1)}
        #print(i)
        }## for i in n diseases 
      
      dfx<-data.frame(disease=unique(resN$disease),pval=sim_slp,sd=sim_sd,stringsAsFactors = FALSE)
    
      #hist(slp,main=paste(i,"_",round(length(slp[slp>0])/length(slp),2)))
      #mean(slp)
    
      cordata1<-data.frame(res6y,bootstrap=x,NDIS=NDIS[qq],ndis=length(unique(resN$disease)),panel=types[tt],panel2=rr,vars=rr)
      ...
      cordata1<-merge(cordata1,dfx,by="disease")
      ...
      if(is.null(res6z)) {res6z<-cordata1} else {res6z<-rbind(res6z,cordata1)}
  ...
  res6a<-res6z#[!duplicated(paste(res6z$results_type,res6z$vars,res6z$disease,sep="_")),]
  ...
  res6a$Group<-paste(res6a$results_type,res6a$vars,sep=" ")
  ...
  ggplot(res6a[!res6a$results_type %in% c("SSP","Regional","RCP/SSP")  & res6a$Group!="Kingdom fungi",],aes(y=lm1*abs(cor1),x=Group,group=Group,col=results_type)) +
  ...
  #facet_grid(Vectored~SSP,scales="free")
    ...
    res7$correlation2<-rnorm(n=rep(1,nrow(res7)),mean=res7$pval,sd=res7$sd)
    ...
    #res7[1,]
    res7$panel2[res7$panel2==" "]<-""
    res7$Panel<-paste(res7$panel,res7$panel2,sep=" ")
    ...
    res7b<-res7[res7$panel2!="fungi",] #[!duplicated(paste0(res7$correlation,res7$bootstrap,res7$NDIS,res7$panel)),]
    ...
    res7c$Panel<-gsub("Vectored 1","Vectored",res7c$Panel,fixed=TRUE)
    ...
    res7c$Panel<-gsub("Emerging ","Emerging",res7c$Panel,fixed=TRUE)
    
    
    res7c$Panel<-factor(res7c$Panel, levels=c("All","Vectored","Non-vectored","Bacteria","Virus","Parasite","Temperate","Tropical","Emerging"))
    ...
    res7c2$Panel<-gsub("Vectored 1","Vectored",res7c2$Panel,fixed=TRUE)
    ...
    res7c2$Panel<-gsub("Emerging ","Emerging",res7c2$Panel,fixed=TRUE)
    
    res7c2$Panel<-factor(res7c2$Panel, levels=c("All","Vectored","Non-vectored","Bacteria","Virus","Parasite","Temperate","Tropical","Emerging"))
    
        
    #table(res7c$Panel,res7c$Trend)
      
    gg<-ggplot(data=res7c[res7c$Panel!="fungi",],aes(x=Trend,y=Count,col=Panel)) +
    #res7$uni<-paste(res7$SSP,res7$rep)]
    #geom_density(alpha=0.1,col=NA)+
    #ggplot(res7,aes(x=jitter(Year),y=richness,col=as.factor(SSP),group=uni)) +
    #stat_summary(fun =  mean, geom = "point",size=2) + 
    stat_summary(data=res7c2[res7c2$Panel!="fungi",],aes(x=Trend,y=Count,group=as.factor(Panel)),col="light grey",alpha=0.75,fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    stat_summary(data=res7c2[res7c2$Panel!="fungi",],aes(x=Trend,y=Count,group=as.factor(Panel)),col="light grey",alpha=0.75,fun.data=stat_HPDI, geom = "errorbar",width=0.2,position = position_dodge(3)) + 
    #stat_summary(fun =  function(x) quantile (x,0.45), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    #stat_summary(fun =  function(x) quantile (x,0.55), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    stat_summary(fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    #stat_summary(fun.data=median_hilow, geom = "errorbar",width=0.2,position = position_dodge(3))+
