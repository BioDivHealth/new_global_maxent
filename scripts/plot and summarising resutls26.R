...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  ##make plot
  gg <- ggplot()
  gg <- gg + geom_point(data=res3z[res3z$AUC>0.5 & res3z$RCP %in% c(6.0,0.0) & res3z$Year %in% c(2070)  ,], 
  ...
  ##make plot
  gg <- ggplot()
  gg <- gg + geom_point(data=datPF[datPF$Disease=="Disease",], 
                        aes(x=(change_x), y=(change_y), group=disease,fill=Longitude), #
                        size=3,pch=21)
  gg <- gg+theme_cowplot()
  gg <- gg + geom_hline(yintercept = 0)
  gg <- gg + geom_vline(xintercept = 0)
