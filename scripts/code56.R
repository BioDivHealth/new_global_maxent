          ...
          #world_simpl@data[wrld_simpl]
				...
				res3 = foreach(ii=1:nrow(link4), .combine=rbl,.packages=c("raster","data.table")) %dopar%  {
				 ...
				 #for(ii in 11:nrow(link4)){
				  ...
				  res_foreach<-data.table(species=link5$species,RCP=link5$RCP, year=link5$year,type=dis3$type,cell.id=values(template2), clim_future=values(clims2),lc_suit_future=values(lcsuit),i=ii)
