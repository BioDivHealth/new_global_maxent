...
res1<-list.files("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\newdata\\more_results4\\",pattern="ALL.csv",full.names=TRUE)
res2<-list.files("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\newdata\\more_results4\\",pattern="ALL.csv",full.names=FALSE)
...
res5<-res3c#[res3c$richness>quantile(res3c$richness,0.01) & res3c$richness<quantile(res3c$richness,0.99),]#dcast(res3,MinMax+RCP+Year+disease+pres_size~Type,value.var=c("Gain","change"))#rbind(res4,res3)
...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6x<-res6[ res6$AUCmax>0.5  ,]# res6$Vectored==0 & & res6$disease.y!="None reported"
  
  unique(res6x$disease)
  #x=1
  ###foreach
    ...
    cur1<-resN[resN$disease==kk[j],][1,]
      ...
      #    png(file=paste("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\present_maps\\present_",kk[j],".png",sep=""),width=1000,height=500)
       ...
       #   plot(mask1,col="#00000050",main=paste(kk[j],round(cur1$AUCmax,2),sep=" "),legend=FALSE)
        ...
        #  plot(wrld_simpl[wrld_simpl$NAME!="Antartica" & wrld_simpl$ISO2 %in% strsplit(cur1$countries,",")[[1]],],lwd=1,border="olivedrab",add=TRUE)
  ...
  gain2<-aggregate(gain,75,sum,na.rm=TRUE)
  loss2<-aggregate(loss,75,sum,na.rm=TRUE)
  gain2[gain2==0]<-NA
  loss2[loss2==0]<-NA
