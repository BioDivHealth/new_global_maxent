...
for (i in i:length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	###create realised present mean - abudance from habitat????
	res6[,realised_present_mean2:= clim_future_mean2 * lc_suit_future_mean * density_mean]
	
	
	## final hazard
	res6[,res6$Hazard:=realised_future_mean2 * CFR * spillover_rate] ## or without CFR and spill over and divide up into cost groups
	
	## final exposure
	## could include cattle??
	## could cut by at risk groups
	res6[,res6$Exposure:=sum_human_population_future]

	## final Vulnerability
	res6[,res6$Vulnerability:=pov_frac_future]
	
	## final risk (units are deaths with CFR - cases without)
	res6[,res6$Risk:=Hazard * Vulnerability * Exposure]

	###do again for present day
	res6[,res6$RiskP:=realised_present_mean2 * CFR * spillover_rate * sum_human_population * pov_frac]
	
	## delta risk
	res6[,delta_risk:= RiskP-Risk]
	...
	res7<-res6[!is.na(sub_region),lapply(.SD,sum,na.rm=TRUE),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Vulnerability","Exposure","Risk","pop-ssp2","pop-ssp3","gdp2010","humans2010")]

		res7b<-res6[!is.na(diss_me),lapply(.SD,mean,na.rm=TRUE),by=diss_me,.SDcols=c("value")]
	
	setkey(res7,diss_me)
	setkey(res7b,diss_me)
	res8<-res7[res7b]
	
	###match to
	res8<-res8[match(ad1$diss_me,res8$diss_me),]
