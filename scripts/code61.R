...
for (x in length(dis1):1){
  ...
  ##choose one disease
	...
	if(paste(" ",dis1[x]," ALL3.r",sep="") %in% list.files("V:\\per_disease2\\", pattern="ALL3.r",full.names=FALSE)){next}
    ...
    res5c<-res5[ ,.(mean(clim_future,na.rm=TRUE), sd(clim_future,na.rm=TRUE),mean(lc_suit_future,na.rm=TRUE), sd(lc_suit_future,na.rm=TRUE)), by = .(type,cell_by_year_by_RCP,merge1)]
    ...
    #save present day
    save(res6,file=paste("V:/per_disease2/",dis3$disease,"ALL3.r"))
    rm(res6);gc()
