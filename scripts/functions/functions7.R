...
remove_small_islands<-function(polys){
temp3<-NULL
if(length(polys)==1){return(extent(polys))}
for(i in 1:length(polys)){
temp1x<-polys@polygons[[i]]
#print(length(temp1x@Polygons))
for(x in 1:length(temp1x@Polygons)){
temp2x<-temp1x@Polygons[[x]]
#print(slot(temp2x,"area"))}
if(slot(temp2x,"area")>5){
...
ws4a=st_as_sf(ws4a)
sr1<-unique(ws3a$SUBREGION)#fasterize(ws3a,regions)
r1<-fasterize(ws4a,regions)
...
if(is.null(locations3)){xxx=TRUE}else{if(nrow(locations3)<25){xxx=TRUE}else{xxx=FALSE}}
...
#locations1<-tryCatch(occ2df(occ(query=key, limit=500000)),error=function(e) e)
#if(class(locations1)[1]=="simpleError"){locations1<-data.frame(a=NULL,b=NULL)}
#if(nrow(locations1)>0){
#locations2<-locations1[!is.na(locations1$longitude),]
#sortx<-paste(locations2$longitude,locations2$latitude,sep="")
...
#(ids <- get_ids(names="Lutxomyia", db = 'gbif'))
