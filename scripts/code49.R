...
for (x in x:length(dis1)){
  ...
  ##choose one disease
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			if(nrow(spec_desp2)==0|nrow(spec_dist2)==0){
			  ...
			  ff1<-ff[ff$data_source_title=="NCBI",]
			  if(nrow(ff1)==0){ff1<-ff[ff$data_source_title=="EOL",]}
			  if(nrow(ff1)==0){ff1<-ff[ff$data_source_title=="ITIS",]}
			  ...
			  ffx1<-ffx[ffx$data_source_title=="NCBI",]
			  if(nrow(ffx1)==0){ffx1<-ffx[ffx$data_source_title=="EOL",]}
			  if(nrow(ffx1)==0){ffx1<-ffx[ffx$data_source_title=="ITIS",]}
    ...
    #library(dismo)
    #AUC1<-evaluate(p=res6$present_realised[res6$ea==1],a=res6$present_realised[res6$ea==0])
