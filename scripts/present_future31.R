...
for (i in (i+1):length(pres1)){
  ...
  rm(res6);gc()
	...
	#res8[,ExposureSSP1:=log(pop_ssp1+1)-Exposure]
	#res8[,ExposureSSP2:=log(pop_ssp2+1)-Exposure]
	#res8[,ExposureSSP3:=log(pop_ssp3+1)-Exposure]
	...
	adc_data<-res8[res8$year==2030,]
	adc_data<-adc_data[match(ad1$diss_me,adc_data$diss_me),]
	...
	for (dd in 1:25){
	  ...
	  ## plot result
	  ad1b<-ad1
	  ad1b@data<-cbind(ad1@data,as.data.frame(adc_data))
...
save(df1,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\df1.r")
save(res100,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\res100.r")
