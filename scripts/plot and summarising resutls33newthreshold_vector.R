...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  typesX<-c("Threshold","Vectored")#c("Baseline","Accuracy","Vectored","RCP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","RCP/SSP","Threshold")
  ...
  #x=1
  ###foreach
  NDIS<-80#c(40,80,120,160) ###check sim p value
  ...
  res7<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings29032022.csv",stringsAsFactors = F)
    ...
    unique(res7$panel)    
    res7b<-res7[!is.na(res7$correlation) & res7$panel=="Vectored" ,]
