...
for (i in (nums)){
  ...
  thr1<-do_auc(data1=res6a,test_column="Hazard",group_column="countr2")
  thr2<-do_auc(data1=res6a,test_column="clim_present_mean2",group_column="countr2")
  ...
  for (zz in 1:length(cols1)){
    ...
    for(yy in 1:length(unis)){
      ...
      res9<-res6[res6$year_RCP==unis[yy] & !is.na(res6$countr2),]
      
      if(zz==2|zz==4){res9<-res6[res6$year_RCP==unis[yy] & !is.na(res6$surrounding),]}
      ...
      pl1<-crop(pl1,extent( min(res9$x)-5, max(res9$x)+5,min(res9$y)-5, max(res9$y)+5))
      pl1[pl1==0]<-NA
      
      system.time(
        pl2<-buffer(pl1,width=11.6*1000,doEdge=TRUE)
      )
