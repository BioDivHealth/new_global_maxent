...
for (i in (nums)){
 # gc()
  ...
  #cols1<-c("clim_present_mean2","Hazard","HazardT","HazardTD","Hazard_future","Hazard_futureTD","host_range","host_range2","RiskL")
    ...
    ##make range
    poly1min<-rast_to_range(x=pl1,present=countr,crumb_size=1e10)
    ...
    ##make range
    poly1max<-rast_to_range(x=pl1,present=countr,crumb_size=1e10)
    
    
    pdf(file=paste("C:\\Users\\xxxx\\Documents\\risk_maps2\\",nam_dis,"_",sample(1:1000,1),"all_plots.pdf",sep=""),width=8,height=8)
      plot(pl1,main=paste(nam_dis,round(dis_trans$AUC,2),sep=" "),col="grey",legend=FALSE)
    ...
    results1max<-as.data.frame(xtabs(data=resF2min,dummy~HazardT+Hazard_futureT+year+RCP))
    ...
    ##
    pdf(file=paste("C:\\Users\\xxxx\\Documents\\results_plots1\\",nam_dis,"_",sample(1:1000,1),"results_plot.pdf",sep=""),width=5,height=5)
    
    ggplot(results4,aes(x=Year,y=Gain,col=RCP))+
      geom_point()+
      theme_classic()+
