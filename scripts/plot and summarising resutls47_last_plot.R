...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  ##number of hosts
  library(stringr)
  res6$hostn<-str_count(res6$all_host,"\\,")
  res6$vectn<-str_count(res6$all_vector,"\\,")
  ...
  rrr<-res6[!duplicated(res6$disease) ,]
  
  xtabs(data=rrr,dummy~TempTrop+Pole)
  
  xtabs(data=rrr,dummy~TempTrop)
  
  xtabs(data=rrr,dummy~Kingdom)
  
  xtabs(data=rrr,dummy~Type2)
  
  xtabs(data=rrr,dummy~host_type)
  
  xtabs(data=rrr,dummy~secondary_type)
...
#  mods4<-mods2[mods2$RCP==6.0,]
  ...
  #for (f in 1:250){
    ...
    r<-raster()
    res(r)<-1#sample(c(10,12.5,15,17.5,20),1)
 ...
 #                      size=2,pch=21)
  ...
  ###taxonomic breadth
  res6$num_hv=res6$hostn+res6$vectn
  res6$num_hv[res6$num_hv>1]=1
  
  
  ###what sensitivity groups 
  types<-c("Baseline","Accuracy","Regional","Vectored","RCP","SSP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","RCP/SSP","Threshold","KingTrans","Breadth")
  
  typesX<-c("Baseline","Accuracy","Regional","Vectored","RCP","SSP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","RCP/SSP","Threshold","KingTrans","Breadth")
  ...
  res7$panel<-paste0(res7$panel,"/SSP")
  
  res7$panel2<-paste0(res7$panel2,"_0")
  
  
  ##write to disck
  #write.csv(res7,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings18112022.csv")
  res7z<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings18112022.csv",stringsAsFactors = F)
  res7z<-res7z[,-1]
 
  res7z$panel<-gsub("/SSP/SSP","_SSP",res7z$panel)
  res7z$panel<-gsub("/SSP","",res7z$panel)
  res7z$panel<-gsub("RCP_SSP","RCP/SSP",res7z$panel)
  
  
  
   #res7<-rbind(res7,res7z)
    ...
    #res7b<-res7z[!is.na(res7$correlation) & res7$panel2!="fungi" & res7$panel!="Emerging" & res7$panel!="KingTrans",] #& res7$panel=="RCP"
  
    res7b<-res7z[!is.na(res7z$correlation) & res7z$panel2!="fungi_0" & res7z$panel!="Emerging" & res7z$panel=="RCP/SSP",] #& res7$panel=="RCP"

    #res7b<-res7z[!is.na(res7z$correlation) & res7z$panel2!="fungi_0" & res7z$panel!="Emerging" & res7z$panel=="RCP",] #& res7$panel=="RCP"
    ...
    ##choose what to show
    res7b$correlationX<-res7b$correlation3
    res7b$correlationSX<-res7b$correlationSc
   ...
   # res7b$Trend<-gsub("0.5","0.25",res7b$Trend,fixed=TRUE)
    ...
    res7c$Panel2<-gsub("_0","",res7c$Panel)
    rrr<-read.table(text=res7c$Panel2,sep="_",stringsAsFactors = F)
    names(rrr)<-c("RCP","SSP")
    res7c<-cbind(res7c,rrr)
    res7c$Count=res7c$Count/sum(res7c$Count,na.rm=TRUE)
  
    res7c$Trend[res7c$Trend=="Contraction"|res7c$Trend=="Static" ]<-"Contraction"
    res7c$Trend2[res7c$Trend2=="Contraction"|res7c$Trend2=="Static" ]<-"Contraction"
    
    
    res8c<-aggregate(res7c$Count,by=list(res7c$bootstrap,res7c$NDIS,res7c$Panel,res7c$Trend,res7c$Trend2,res7c$Panel2,res7c$RCP,res7c$SSP),sum)
    
    ##find what average non expanding
    res8c3<-res8c[ res8c$Group.4=="Contraction", c("Group.6","x")]
    names(res8c3)<-c("Panel2","Count")
    #res8c3$rep<-1:nrow(res8c3)
    res8c3b<-aggregate(res8c3$Count,by=list(res8c3$Panel2),sum)
    res8c3b$x=res8c3b$x/max(res7c$bootstrap) ##after summing divide by bootstrap contractions per sample
    names(res8c3b)<-c("Panel2","Mean_Count")
    
    
    ##count expanding
    res8c4<-res8c[ res8c$Group.4!="Contraction", c("Group.6","Group.7","Group.8","x")]
    names(res8c4)<-c("Panel2","RCP","SSP","Count_expansion")
    res8c4$rep<-1:nrow(res8c4)
    
    ##merge
    res9c<-merge(res8c4,res8c3b,by="Panel2")
    res9c$Count=res9c$Count_expansion/res9c$Mean_Count
    rm(res8c4,res8c3b,res8c3)
    ...
    res7c2$Trend[res7c2$Trend=="Contraction"|res7c2$Trend=="Static" ]<-"Contraction"
    res7c2$Trend2[res7c2$Trend2=="Contraction"|res7c2$Trend2=="Static" ]<-"Contraction"
    
    
    res8c2<-aggregate(res7c2$Count,by=list(res7c2$bootstrap,res7c2$NDIS,res7c2$Panel,res7c2$Trend,res7c2$Trend2,res7c2$Panel2,res7c2$RCP,res7c2$SSP),sum)
    
    
    ##find what average non expanding
    res8c3<-res8c2[ res8c2$Group.4=="Contraction", c("Group.6","x")]
    names(res8c3)<-c("Panel2","Count")
    #res8c3$rep<-1:nrow(res8c3)
    res8c3b<-aggregate(res8c3$Count,by=list(res8c3$Panel2),sum)
    res8c3b$x=res8c3b$x/max(res7c$bootstrap) # 
    names(res8c3b)<-c("Panel2","Mean_Count")
    
    
    ##count expanding
    res8c4<-res8c2[ res8c2$Group.4!="Contraction", c("Group.6","Group.7","Group.8","x")]
    names(res8c4)<-c("Panel2","RCP","SSP","Count_expansion")
    res8c4$rep<-1:nrow(res8c4)
    
    ##merge
    res9c2<-merge(res8c4,res8c3b,by="Panel2")
    res9c2$Count=res9c2$Count_expansion/res9c2$Mean_Count
    
    ##
    #res9c$type="Poor Cases"
    #res9c2$type="Poor Cases"
    ...
    ##combine
    res11c<-rbind(res9c,res10c,res10cb)
    res11c2<-rbind(res9c2,res10c2,res10c2b)
    
    res11c$RCP<-paste("RCP ",res11c$RCP)
    res11c2$RCP<-paste("RCP ",res11c2$RCP)

    res11c$SSP<-paste("SSP ",res11c$SSP)
    res11c2$SSP<-paste("SSP ",res11c2$SSP)
    
    #res11c$type[res11c$type=="Cases in Poverty"]<-"Poorer Cases"
    #res11c2$type[res11c2$type=="Cases in Poverty"]<-"Poorer Cases"
    ...
    #write.csv(res11c,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure02122022.csv")
    #write.csv(res11c2,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure02122022b.csv")
    
        
    res11c<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure02122022.csv")

    res11c2<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\all_processed_for_figure02122022b.csv")
    
    res11c$RCP<-gsub("RCP/SSP","RCP",res11c$RCP)
    res11c2$RCP<-gsub("RCP  ","RCP ",res11c2$RCP)
    res11c$RCP<-gsub("RCP  ","",res11c$RCP)

    not_res=c("")
    
    res11c$type<-gsub("Range","H",res11c$type)
    res11c$type<-gsub("Poor Cases","H*E*V",res11c$type)
    res11c$type<-gsub("Cases","H*E",res11c$type)
    
    
    res11c2$type<-gsub("Range","H",res11c2$type)
    res11c2$type<-gsub("Poor Cases","H*E*V",res11c2$type)
    res11c2$type<-gsub("Cases","H*E",res11c2$type)
    
      
    res11c$type<-factor(res11c$type,levels=c("H","H*E*V","H*E"))
    res11c2$type<-factor(res11c2$type,levels=c("H","H*E*V","H*E"))
    
    #plaus<-c("RCP/SSP 2.6_1","RCP/SSP 4.5_2","RCP/SSP 6_3","RCP/SSP 4.5_4","RCP/SSP 4.5_4","RCP/SSP 4.5_2",)
    
    zz=mean(res11c2$Count[res11c2$RCP=="RCP 4.5" & res11c2$type!="H" & res11c$SSP!="SSP  0"])

    zz2=HPDI(res11c2$Count[res11c2$RCP=="RCP 4.5" & res11c2$type!="H" & res11c$SSP!="SSP  0"])
    
    zzH=zz2[1]
    zzL=zz2[2]    
        
    gg<-
      ggplot(data=res11c[res11c$SSP!="SSP  0" & res11c$RCP=="RCP 4.5" & res11c$type!="H",],aes(x=type,y=Count,col=(type))) +
    ...
    #stat_summary(data=res11c2[res11c2$SSP!="SSP  0" & res11c2$type!="Range",],aes(x=type,y=Count),col="grey",alpha=0.75,fun =  mean, geom = "point",size=2) +#,position = position_dodge(3)) + 
    #stat_summary(data=res11c2[res11c2$SSP!="SSP  0" & res11c2$type!="Range",],aes(x=type,y=Count),col="grey",alpha=0.75,fun.data=stat_HPDI, geom = "errorbar",width=0.2) +#,position = position_dodge(3)) + 
    ...
    stat_summary(fun =  mean, geom = "point",size=3) +#,position = position_dodge(3)) + 
    ...
    #geom_hline(data=res7c,aes(yintercept=mean(Count,na.rm=TRUE),group=Panel),col="black",lty=2)+
      #xlim(2,9)+
      geom_hline(yintercept = 0.75/1.5,lty=2)+ 
      #geom_hline(yintercept = zzH,lty=1,col="light grey",alpha=0.5)+ 
      #geom_hline(yintercept = zzL,lty=1,col="light grey",alpha=0.5)+ 
  ...
  #    Year<-sample(c(2030,2050,2070,2080),4,replace=F)
      
  #    richness<-sample(seq(from=0,to=2,by=0.01),4)
      
  #    lm1s<-cor(richness,Year,method="spearman",use="pairwise.complete.obs")[c(1,1)]
      
  #    return(lm1s[2])
      
   # }
    
  #tt<-  replicate(1000,sim_cor())
