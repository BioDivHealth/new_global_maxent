...
for (x in 1:length(dis1)){
	...
	dis2<-res1[res1$disease==dis1[x],]
		...
		for(z in 1:nrow(dis2)){
			...
			if(link4$density>3000000]<-3000000
				...
				for(y in 1:nrow(link4)){
					...
					if(y>1 & link5$RCP==link4$RCP[y-1] & link5$year==link4$year [y-1]{}else{
					...
					k<-lcsuit*mean(link5$density,na.rm=TRUE) ## make these number relavent to habitat type
					...
					IP[cellFromXY(IP,dt1)]<-mean(link5$density,na.rm=TRUE)
				...
				## run gas model??
				## issues of genera level will be lower than lots of species?? density * num spec in gbif sample

				
				##create data frame use data.table or feather
				## read up
				## add the year coloumns onto side
				if(y==1){res1<-data.table(disease=dis3$disease,type=dis3$type,species=link5$species,year=link5$year,RCP=link5$RCP,model=link5$model, maxent=values(clims2),forest=values(f1),nonforest=values(nf1),water=values(water),lc_suit=values(lcsuit),abundance=values(k),realised=values(IP),values(futpop2),values(gdppop2))}
