...
p3_treemap <- treemap(
  ...
  type = "index",
  palette = "Set3",
  title = "Host Taxonomic Diversity: Class > Order",
  fontsize.title = 14,
...
# 4. SUNBURST-STYLE DONUT CHART
p4_donut <- host_clean %>%
  count(Class, Order) %>%
  filter(n >= 2) %>%  # Only orders with 2+ species
  group_by(Class) %>%
  mutate(
    class_total = sum(n),
    order_prop = n / class_total
  ) %>%
  ungroup() %>%
  arrange(Class, desc(n)) %>%
  mutate(
    Order_label = ifelse(n >= 3, Order, "Other"),
    Order_label = paste0(Order_label, " (", n, ")")
  ) %>%
  ggplot(aes(x = 2, y = n, fill = Order_label)) +
  geom_col() +
  coord_polar("y") +
  ...
  labs(title = "Host Orders within Each Class")

# 5. TAXONOMIC DIVERSITY SUMMARY TABLE
...
# 6. DETAILED TAXONOMIC BREAKDOWN
top_families <- host_clean %>%
  count(Class, Order, Family, sort = TRUE) %>%
  slice_head(n = 20)

p6_family_detail <- top_families %>%
  mutate(
  ...
  labs(
    title = "Top 20 Host Families by Species Count",
...
ggsave(here(output_dir, "host_orders_by_class.png"), p4_donut, 
       width = 14, height = 10, dpi = 300, bg = "white")
ggsave(here(output_dir, "host_families_top20.png"), p6_family_detail, 
       width = 12, height = 10, dpi = 300, bg = "white")

# Save treemap as PNG
png(here(output_dir, "host_taxonomy_treemap.png"), width = 12, height = 8, 
