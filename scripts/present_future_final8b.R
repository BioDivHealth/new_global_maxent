...
for (i in (7)){
  ...
  ##threshold again
    ...
    if(nrow(res6a[is.na(res6a$countr),])<100){
      print("AUC PROBLEM")
    ...
    thr1<-do_auc(data1=res6a,test_column="Hazard",group_column="countr")
    thr2<-do_auc(data1=res6a,test_column="clim_present_mean2",group_column="countr")
  ...
  plot(countr)
  points(res6$x[res6$HazardT==1 &res6$year_RCP==unique(res6$year_RCP)[1] & !is.na(res6$regionPA)],res6$y[res6$HazardT==1 res6$year_RCP==unique(res6$year_RCP)[1] & !is.na(res6$surrounding)])
  
  #res6$dummy=1
  res7<-  [!is.na(res6$HazardT) & !is.na(res6$Hazard_futureT) & !is.na(res6$surrounding),]
  
  unis<-unique(res6$year_RCP)
  
  res9<-res6[res6$year_RCP==unis[1] & !is.na(res6$surrounding),]
  ...
  for (zz in 1:length(cols1)){
    ...
    pl2<-rasterToPolygons(pl1,dissolve=TRUE,na.rm=TRUE)
    
    pl3<-drop_crumbs(pl2, 1000000000, drop_empty = TRUE)
    
    pl4<-disaggregate(pl3)
    
    drops<-over(pl4,countr)
    
    pl5<-pl4[!is.na(drops[,1]),]
    
    pl6<-fill_holes(pl5,threshold=1e16)
    ...
    rm(pl1,pl2,pl3,pl4,pl5)#plot(pl1,main=titls[zz],font=32,cex.main=3)
    #points(xyFromCell(template,unique(res6$cell.id)),col="red",pch=20)
