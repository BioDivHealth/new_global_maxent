...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  typesX<-c("Baseline","Vectored","Emerging","Kingdom","Tropical/Temperate","RCP/SSP")
  ...
  ##make dummy
    ...
    if(jj==4){res6x$sort<-paste0(res6x$range,res6x$group);xxx<-"Both"}
      ...
      cordata1<-data.frame(disease=unique(resN$disease),correlation=slp,bootstrap=x,NDIS=NDIS[qq],ndis=length(unique(resN$disease)),panel=types[tt],pval=sim_slp,sd=sim_sd,vars=rr)
      ...
      if(is.null(resN2)) {resN2<-cordata1} else {resN2<-rbind(resN2,cordata1)}
