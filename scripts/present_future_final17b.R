...
# load the data 
coastlines <- readOGR("C:\\Users\\xxxx\\Document\\ne-coastlines-10m/ne_10m_coastline.shp")
...
##read livestock data
#CHECK
lvst<-stack(list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\data/",pattern="_Da.tif",full.names=TRUE))
names(lvst)<-c("buffalo","chickens","cattle","ducks","goats","horses","pigs","sheep")

##read in admins to move away from GRID
ad1<-readOGR("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\admin1/ne_10m_admin_1_states_provinces.shp","ne_10m_admin_1_states_provinces")


#load(file="X:/DRtemp/babesia_problem.r")
#source("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\R_scripts/TSS.r")
...
##defaunation surface
#predicts3<-raster("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\global_IUCN/data/final-abund-bii-isl-main.tif")
...
#writeRaster(predicts2b,format="GTiff",file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\global_IUCN/data/predicts_resampled_template.tif",overwrite=TRUE);gc()
predicts<-raster("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\data/predicts_resampled_template.tif")
...
###find all present day
pres1<-list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\per_disease3/",pattern=" ALL4",full.names=TRUE)
pres2<-list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\per_disease3/",pattern=" ALL4",full.names=FALSE)
pres2<-gsub("  ALL4.r","",pres2)
pres2<-gsub(" ALL4.r","",pres2)


###find all present day
iucn1<-list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\disease_host_ranges/",pattern="3.tif",full.names=TRUE)
iucn2<-list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\disease_host_ranges/",pattern="3.tif",full.names=FALSE)
...
#writeOGR(wrld_simpl3,driver="ESRI Shapefile",dsn="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\wrld_simpl3.shp","wrld_simpl3")
...
for (i in sample(nums)){
  ...
  ##first writing
  write.csv(nam_dis,file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_1.csv",sep=""))
  
  if( paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_1.csv",sep="") %in% list.files("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done/",full.names=TRUE) ){next}
  ...
  if(nrow(d2c)==1){load(pres1[pres2==paste0("",d2c$disease)[1]]);res6[,name:=d2c$disease[1]]} else {
    ...
    for (rr in 1:nrow(d2c)){
      ...
      if(d2c$disease[rr]=="babesia venatorum"){d2c$disease[rr]<-pres2[20];load(pres1[pres2==d2c$disease[rr]])}
  ...
  ##second waymarker
  write.csv(nam_dis,file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_2.csv",sep=""))
  ### Hazard options
  ...
  for(ww in 1:10){
    ...
    if(ww==1){rt2A<-rbind(rt1,rt1b)} else {rt2A<-rbind(rt2A,rt1,rt1b)}
    #print(ww)
  }
  ...
  rt2<-rt2[rt2$AUC_best.stat>quantile(rt2$AUC_best.stat,0.05),]
  rt2C<-rt2C[rt2$AUC_best.stat>quantile(rt2C$AUC_best.stat,0.05),]
  
  ##put in main data frame
  dis_trans$AUC_cutoff<-mean(rt2$AUC_cutoff)
  dis_trans$AUC<-mean(rt2$AUC_best.stat)
  dis_trans$AUC_cutoffclim<-mean(rt2C$AUC_cutoff)
  dis_trans$AUC_clim<-mean(rt2C$AUC_best.stat)
  
  ##put in main data frame
  dis_trans$AUC_cutoffmax<-max(rt2$AUC_cutoff)
  dis_trans$AUCmax<-max(rt2$AUC_best.stat)
  dis_trans$AUC_cutoffclimmax<-max(rt2C$AUC_cutoff)
  dis_trans$AUC_climmax<-max(rt2C$AUC_best.stat)
  
  ##put in main data frame
  dis_trans$AUC_cutoffmin<-min(rt2$AUC_cutoff)
  dis_trans$AUCmin<-min(rt2$AUC_best.stat)
  dis_trans$AUC_cutoffclimmin<-min(rt2C$AUC_cutoff)
  dis_trans$AUC_climmin<-min(rt2C$AUC_best.stat)
  ...
  if(nrow(pd)>0){
    for (uu in 1:25){
      ...
      real1<-extract(pl1,res6[sample(1:nrow(res6),1000),c("x","y")])
      not_real<-extract(pl1,randomPoints(pl1,10000))
      
      e1<-dismo::evaluate(p=real1,a=not_real)
  ...
  if(dis_trans$AUC_cutoffclimmax>quantile(res6$clim_present_mean2,0.975,na.rm=TRUE)){dis_trans$AUC_cutoffclimmax<-quantile(res6$clim_present_mean2,0.95,na.rm=TRUE)}
  if(dis_trans$AUC_cutoffclimmin<quantile(res6$clim_present_mean2,0.025,na.rm=TRUE)){dis_trans$AUC_cutoffclimmin<-quantile(res6$clim_present_mean2,0.05,na.rm=TRUE)}
  
  ### percell thresholds CLIMATE ONLY
  res6[,HazardT:=ifelse(clim_present_mean2>=dis_trans$AUC_cutoffclim,1,0)]
  res6[,Hazard_futureT:=ifelse(clim_future_mean2>=dis_trans$AUC_cutoffclim,1,0)]
  res6[,HazardTmin:=ifelse(clim_present_mean2>=dis_trans$AUC_cutoffclimmin,1,0)]
  res6[,Hazard_futureTmin:=ifelse(clim_future_mean2>=dis_trans$AUC_cutoffclimmin,1,0)]
  res6[,HazardTmax:=ifelse(clim_present_mean2>=dis_trans$AUC_cutoffclimmax,1,0)]
  res6[,Hazard_futureTmax:=ifelse(clim_future_mean2>=dis_trans$AUC_cutoffclimmax,1,0)]
  
  ##third waymarker
  write.csv(nam_dis,file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_3.csv",sep=""))
  ...
  ##plot to see where the data are LAND-USE
  #plot(wrld_simpl)
  pl1<-ws2
  pl1[pl1==1]<-0
  ...
  ##fourth waymarker
  write.csv(nam_dis,file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_4.csv",sep=""))
  
  pdf(file=paste("C:\\Users\\xxxx\\Documents\\risk_maps2\\",nam_dis,"_",sample(1:1000,1),"all_plots.pdf",sep=""),width=8,height=8)
  plot(pl2*pl1,main=paste(nam_dis,round(dis_trans$AUC_clim,2),sep=" "),legend=FALSE)
  ...
  ##fifth waymarker
  write.csv(nam_dis,file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\being_done//",nam_dis,"_5.csv",sep=""))
  ...
  for (ee in c(2030,2050,2070,2080)){
    ...
    res6t[,presentfuture:=over(resx[res6$year==ee,],get(paste0("poly",ee,"i")))]
    res6t[,presentfuturemin:=over(resx[res6$year==ee,],get(paste0("poly",ee,"min","i")))]
    res6t[,presentfuturemax:=over(resx[res6$year==ee,],get(paste0("poly",ee,"max","i")))]
    ...
    ##write gain loss
    fwrite(dis_trans,file=paste("C:\\Users\\xxxx\\Documents\\gainlossresults\\",nam_dis,"_",sample(1:1000,1),"_1.csv",sep=""))
