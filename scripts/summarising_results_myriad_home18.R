...
#for (j in j:2){
  ...
  for (k in sss){
    ...
    centroid1<-colMeans(xyFromCell(pres4, which(pres4[]>0)))
    ...
    #assign(combs$uni[qq],pres_spill)
    if(class(tryCatch(get(paste(typex,"human_deaths",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(typex,"human_deaths",sep="_"),pres_spill25)} else {assign(paste(typex,"human_deaths",sep="_"),get(paste(typex,"human_deaths",sep="_"))+pres_spill25)}
    ...
    df1<-data.frame(disease=resF$name[1],model="present",richness=sum(values(pres4),na.rm=TRUE),disease_richness=sum(values(pres4a),na.rm=TRUE),humans=sum(values(pres4b),na.rm=TRUE),cases=sum(values(pres_spill),na.rm=TRUE),deaths=sum(values(pres_spill),na.rm=TRUE),x=centroid1[1],y=centroid1[2])
    ...
    #    for (qq in 1){
      ...
      ##get centroid for each to see how it moves
      centroid2<-colMeans(xyFromCell(fut4, which(fut4[]>0)))
      ...
      df2<-data.frame(disease=resF$name[1],model=paste(combs[qq,2],"all",sep="_"),richness=sum(values(fut4),na.rm=TRUE),disease_richness=sum(values(fut4a),na.rm=TRUE),humans=sum(values(fut4b),na.rm=TRUE),cases=sum(values(fut_spill),na.rm=TRUE),deaths=sum(values(fut_spill),na.rm=TRUE),x=centroid2[1],y=centroid2[2])
    ...
    ##indivdual results
    df4<-rbind(df1,df3)
    write.csv(df4,file=paste("D:\\Users\\xxxx\\Documents\\more_results\\per_disease_results","_",sample(1e7,1),typex,".csv",sep=""))
...
for (i in 1:length(names2)){
  ...
  dfy<-data.frame(subset=typex,type=names2[i],value=sum(values(fut4),na.rm=TRUE))
