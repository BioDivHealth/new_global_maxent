...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  typesX<-c("Baseline","Accuracy","Regional","Vectored","RCP","SSP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","Threshold","KingTrans")
  ...
  res7<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings23062022.csv",stringsAsFactors = F)
  ...
  for(pp in 1:3){
    ...
    ##choose what to show
    if(pp==1){res7b$correlationX<-res7b$correlation;res7b$correlationSX<-res7b$correlationS;ty1="Area"}
    if(pp==2){res7b$correlationX<-res7b$correlation2;res7b$correlationSX<-res7b$correlationSb;ty1="Cases"}
    if(pp==3){res7b$correlationX<-res7b$correlation3;res7b$correlationSX<-res7b$correlationSc;ty1="Poor Cases"}
   ...
   # res7b$Trend<-gsub("0.5","0.25",res7b$Trend,fixed=TRUE)
    ...
    #store
    if(pp==1){res10c<-res9c;res10c2<-res9c2;next}

    ##combine
    res10c<-rbind(res9c,res10c)
    res10c2<-rbind(res9c2,res10c2)
  ...
  res11c2$SSP<-paste("SSP ",res11c2$SSP)
    ...
    gg<-
      ggplot(data=res11c[,],aes(x=type,y=Count,col=as.factor(SSP))) +
    #res7$uni<-paste(res7$SSP,res7$rep)]
    #geom_density(alpha=0.1,col=NA)+
    #ggplot(res7,aes(x=jitter(Year),y=richness,col=as.factor(SSP),group=uni)) +
    #stat_summary(fun =  mean, geom = "point",size=2) + 
    stat_summary(data=res11c2[,],aes(x=type,y=Count),col="light grey",alpha=0.75,fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    stat_summary(data=res11c2[,],aes(x=type,y=Count),col="light grey",alpha=0.75,fun.data=stat_HPDI, geom = "errorbar",width=0.2,position = position_dodge(3)) + 
    #stat_summary(fun =  function(x) quantile (x,0.45), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    #stat_summary(fun =  function(x) quantile (x,0.55), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    stat_summary(fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    #stat_summary(fun.data=median_hilow, geom = "errorbar",width=0.2,position = position_dodge(3))+
