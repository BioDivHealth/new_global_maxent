...
#          occurrence input per Chikungunya vector species.
#
...
default_batch_config <- list(
  ...
  local_source_manifest_path = file.path(repo_root(), "sdms", "runs", "chikungunya", "calibration", "local_vector_occurrence_sources_manifest.csv"),
  occurrence_root = file.path(repo_root(), "sdms", "runs", "chikungunya", "calibration", "occurrences"),
  roles = "vector",
...
read_csv_if_exists <- function(path) {
  ...
  data.table::fread(path, data.table = FALSE, showProgress = FALSE, fill = TRUE)
}
...
date_to_year <- function(x) {
  value <- suppressWarnings(as.Date(substr(as.character(x), 1, 10)))
  suppressWarnings(as.integer(format(value, "%Y")))
}

derive_year <- function(data, source_method) {
  if ("year" %in% names(data)) {
    return(suppressWarnings(as.integer(data$year)))
  }

  if (source_method == "vectormap") {
    year_col <- first_existing_column(data, c("EarliestYearCollected", "LatestYearCollected"))
    if (!is.na(year_col)) {
      return(suppressWarnings(as.integer(data[[year_col]])))
    }
  }
...
local_source_path <- function(local_manifest, species, source_method) {
  ...
  if (nrow(hits) == 0 || !file.exists(hits$output_path[[1]])) {
    return(NA_character_)
  }

  normalizePath(hits$output_path[[1]], winslash = "/", mustWork = TRUE)
}
...
run_dir <- file.path(
  ...
  "chikungunya",
  "calibration",
  "combined_vector_occurrence_runs",
  timestamp
)
