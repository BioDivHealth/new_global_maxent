...
species_occurrence_dir <- ensure_dir(file.path(repo_root(), "sdms", "runs", "chikungunya", "calibration", "occurrences", species_safe))
...
if (raw_rows_before_year_filter != nrow(raw)) {
  write.csv(raw, raw_path, row.names = FALSE, na = "")
}
gbif_key_col <- "key"
