...
GP2$diss_me<-extract(pres_ad1,GP2$cell.id)

##get gini
gini<-stack("V:\\GINI_STACK.tif")
names(gini)<-read.csv(file="V:\\GINI_STACK_NAMES.csv",stringsAsFactors = FALSE)$x
gini2<-extract(gini,xyFromCell(template,GP2$cell.id))
gini3<-melt(gini2)
gini3$year<-read.table(text=as.character(gini3$Var2),sep="_",stringsAsFactors = FALSE)[,1]
gini3$year<-as.numeric(gsub("X","",gini3$year))
setDT(as.data.frame(gini3))
setkey(gini3,year)
setkey(GP2,year)
GP2b<-GP2[gini3]
