...
for (i in (nums)){
 # gc()
  ...
  ##threshold again
    ...
    if(nrow(res6a[is.na(res6a$countr2),])<100){
      print("AUC PROBLEM")
      ##put in main data frame
      dis_trans$AUC_cutoff<-0.1
      dis_trans$AUC<-0.1
      dis_trans$AUC_cutoffclim<-0.1
      dis_trans$AUC_clim<-0.1
    }
    ...
    for(ww in 1:25){
        ...
        if(zero_one==1){
        thr1<-do_auc(data1=res6a,test_column="Hazard",group_column="countr2",weights="dist_cent",timesP=timesP,timesA=timesA)
        thr2<-do_auc(data1=res6a,test_column="clim_present_mean2",group_column="countr2",weights="dist_cent",timesP=timesP,timesA=timesA)
               }else{
       thr1<-do_auc(data1=res6a,test_column="Hazard",group_column="countr2",weights="dummy",timesP=timesP,timesA=timesA)
       thr2<-do_auc(data1=res6a,test_column="clim_present_mean2",group_column="countr2",weights="dummy",timesP=timesP,timesA=timesA)
           }
    
        rt1<-data.frame(thr1,timesA=timesA,timesP=timesP)
        rt1b<-data.frame(thr2,timesA=timesA,timesP=timesP)
    
       if(ww==1){rt2<-rt1;rt2b<-rt1b} else {rt2<-rbind(rt2,rt1);rt2b<-rbind(rt2b,rt1b)}
       #print(ww)
      }
    
    ##cut out cut offs
    rt3<-rt2[seq(from=2,to=nrow(rt2),by=4),]
    rt3b<-rt2b[seq(from=2,to=nrow(rt2),by=4),]
    
    ##cut out cut offs
    rt3A<-rt2[seq(from=1,to=nrow(rt2),by=4),]
    rt3bA<-rt2b[seq(from=1,to=nrow(rt2),by=4),]
    
    
    ##put in main data frame
    dis_trans$AUC_cutoff<-mean(rt3[,1])
    dis_trans$AUC<-mean(rt3A[,1])
    dis_trans$AUC_cutoffclim<-mean(rt3b[,1])
    dis_trans$AUC_clim<-mean(rt3bA[,1])

    ##put in main data frame
    dis_trans$AUC_cutoffmax<-max(rt3[,1])
    dis_trans$AUCmax<-max(rt3A[,1])
    dis_trans$AUC_cutoffclimmax<-max(rt3b[,1])
    dis_trans$AUC_climmax<-max(rt3bA[,1])
    
    ##put in main data frame
    dis_trans$AUC_cutoffmin<-min(rt3[,1])
    dis_trans$AUCmin<-min(rt3A[,1])
    dis_trans$AUC_cutoffclimmin<-min(rt3b[,1])
    dis_trans$AUC_climmin<-min(rt3bA[,1])
    
    if(d2c$disease[1]=="angiostrongylus costaricensis"){dis_trans$AUC_cutoff<-0.1}
  ...
  #cols1<-c("clim_present_mean2","Hazard","HazardT","HazardTD","Hazard_future","Hazard_futureTD","host_range","host_range2","RiskL")
    ...
    pl1[res9b$cell.id]<-res9b[ ,names(res9b)[names(res9b)==cols1[zz]],drop=TRUE]
    ...
    ##make range
    poly1<-rast_to_range(x=pl1,present=countr,crumb_size=1e10)
    
    jpeg(file=paste("C:\\Users\\xxxx\\Documents\\risk_maps2\\",nam_dis,"_",sample(1:1000,1),"all_plots.jpg",sep=""),width=1500,height=1500)
    plot(pl1,main=paste(nam_dis,round(dis_trans$AUC,2),sep=" "))
    plot(regn3,add=TRUE)
    plot(poly1,add=TRUE,border="blue")
    plot(ad2,add=TRUE,size=2,border="red")
    plot(coastlines,add=TRUE,size=2,border="green")
    ...
    results2<-results1[results1$HazardT!=results1$Hazard_futureT,]
    ...
    results3<-aggregate(results2$Freq2,by=list(results2$RCP,results2$year),sum)
    names(results3)<-c("RCP","Year","Gain")
    ...
    ##addd to old data
    dis_trans2<-cbind(dis_trans,results4)
     ## final hazard
