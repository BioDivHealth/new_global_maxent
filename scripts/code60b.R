...
link2b2<-list.files("X:/DRtemp/resultsY", pattern="_present_",full.names=TRUE)
link22<-list.files("X:/DRtemp/resultsY", pattern="_present_",full.names=FALSE)
...
for (x in (x+1):length(dis1)){
  ...
  ##choose one disease
	...
	if(paste(" ",dis1[x]," ALL2.r",sep="") %in% list.files("V:\\per_disease2\\", pattern="ALL2.r",full.names=FALSE)){next}
	...
	if(is.na(ccc[1])){print("no recognised countries");print(dis_trans);next}
	countr<-wrld_simpl[wrld_simpl$ISO2 %in% ccc ,]
	#if(nrow(countr)!=length(ccc)){print("issue with countries");print(ccc);next}
	if(nrow(countr)==0){print("no recognised countries");print(dis_trans);next}
    ...
    ### stop if not
    if(nrow(res6v)==0 | nrow(res6h)==0){print("problem with vectors/hosts");next}
    ...
    #save present day
    save(res6,file=paste("V:/per_disease2/",dis3$disease,"ALL2.r"))
    rm(res6);gc()
