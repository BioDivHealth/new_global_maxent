...
for (i in (nums)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res6[,HazardDF := clim_present_mean2 * lc_suit_present_mean2 * defaun ]
	...
	if(noabs==1){AUC_cutoff<-0.1;AUC_cutoff2<-0.1;AUC_cutoff_real<-0.1} else {AUC_cutoff<-mean(thr2[,2],na.rm=TRUE);AUC_cutoff2<-mean(thr2x[,2],na.rm=TRUE)}
  ...
  # risk calculate	
	...
	titls<-paste(c("HazardT","HazardL","Host Range","Hazard Change"),round(mean(thr2[,1]),2),sep=" ")
	...
	if(zz==1){pl1[res9$cell.id]<-res9$HazardT}
	if(zz==2){pl1[res9$cell.id]<-log(res9$Hazard+1)}
	if(zz==3){pl1[res9$cell.id]<-res9$host_range}
	#if(zz==3){pl1[res9$cell.id]<-log(res9$Hazard_future+1)}
	if(zz==4){xxx<-(log(res9$Hazard+1))-(log(res9$Hazard_future+1));xxx[xxx==0]=NA;pl1[res9$cell.id]<-xxx}
