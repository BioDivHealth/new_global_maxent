...
for (i in (i+1):length(pres1)){
  ...
  if(nrow(countr)/nrow(regn)>0.5){regn<-regn2}
  if(nrow(countr)/nrow(regn)>0.5){regn<-wrld_simpl2}
  ...
  rm(res6);gc()
	...
	res8[,ExposureSSP1:=cut(log(pop_ssp1+1),breaks=breaks1,labels=FALSE)/10]
	res8[,ExposureSSP2:=cut(log(pop_ssp2+1),breaks=breaks1,labels=FALSE)/10]
	res8[,ExposureSSP3:=cut(log(pop_ssp3+1),breaks=breaks1,labels=FALSE)/10]
	res8[,Exposure:=cut(log(humans2010+1),breaks=breaks1,labels=FALSE)/10]
	
	res8[,VulnerabilitySSP1:=(cut(log(1/(gdp_ssp1/pop_ssp1)),breaks=breaks2,labels=FALSE)/10)*(gini_SSP1/100)]
	res8[,VulnerabilitySSP2:=(cut(log(1/(gdp_ssp2/pop_ssp2)),breaks=breaks2,labels=FALSE)/10)*(gini_SSP2/100)]
	res8[,VulnerabilitySSP3:=(cut(log(1/(gdp_ssp3/pop_ssp3)),breaks=breaks2,labels=FALSE)/10)*(gini_SSP3/100)]

	res8[,Vulnerability:=(cut(log(1/(gdp2010/humans2010)),breaks=breaks2,labels=FALSE)/10)*(X2010_GINI_PRESENT_C/100)]
	
	
	#high gdp -10 low vulnerability
	#1-gdp - poor is high
	
	###gini 1 is unequal si high is bad
	...
	res8[,RiskP:= Hazard * Exposure * Vulnerability]# pov_frac_present]
...
#}## end of i loop
  ...
  ##make raster layer
  ras1<-fasterize(ad1c,template,field="RiskP",fun="sum")
  ...
  ad1c$RiskP2<-ad1c$RiskP
  ad1c$RiskP2[ad1c$RiskP2>thr$kappa]<-1
  ad1c$RiskP2[ad1c$RiskP2!=1]<-0
  ...
  jpeg(file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\risk_maps\\",pres2[i],"_",sample(1:1000,1),"Risk.jpg",sep=""),width=2500,height=1000)
    print(
	  ggplot(data = ad1c) +
    geom_sf(aes(fill = RiskP),colour=NA) +
    scale_fill_viridis_c(option = "plasma")+
	 geom_sf(data=countr2,aes(),colour="white", alpha=0,size=1)+
	   geom_sf(data=pd2,aes(),cex=0.1)+
	   labs(title=paste(pres2[i]," - AUC ",round(e@auc,2),"  AUC points ",round(epd_auc,2),sep=""))+
	   theme(plot.title=element_text(size=32),axis.text = element_text(size=28))
	   	  #+	facet_wrap(.~year_RCP)
    )
	dev.off()

	jpeg(file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\risk_maps\\",pres2[i],"_",sample(1:1000,1),"Threshold_Risk.jpg",sep=""),width=2500,height=1000)
	print(
	  ggplot(data = ad1c) +
	    geom_sf(aes(fill = RiskP2),colour=NA) +
	    scale_fill_viridis_c(option = "plasma")+
	    geom_sf(data=countr2,aes(),colour="white", alpha=0,size=1)+
	    geom_sf(data=pd2,aes(),cex=0.1)+
	    labs(title=paste(pres2[i]," - AUC ",round(e@auc,2),"  AUC points ",round(epd_auc,2),sep=""))+
	...
	##make raster layer for Hazard
	ras1<-fasterize(ad1c,template,field="Hazard",fun="sum")
	#ras2<-fasterize(ad1c,template,field="RiskP",fun="count")
	#ras1<-ras1/ras2
	ras1[is.na(ras1)]<-0
  ...
  rm(res8,res9,res10,ad1c)
	gc()
  print(i)
}

##save results
write.csv(res11,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\final_risk2.csv")


#res12<-	res11[,lapply(.SD,function (x) sum(x,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=names(res10)[3:13]]
