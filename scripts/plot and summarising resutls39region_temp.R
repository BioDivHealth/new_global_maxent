...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6$Response=res6$richnessALLdiff
  res6$Response[res6$Response<0]<-rnorm(n=nrow(res6),mean=(-1),sd=0.001)
  res6$Response[res6$Response>0]<-rnorm(n=nrow(res6),mean=(1),sd=0.001)
  ...
  sum1=NULL
  ##loop to create map
  for (fff in 1:length(mods)){
  
      
    
    res6x<-res6[res6$model %in% mods[fff],]
    ...
    aq@data<-cbind(aq@data,over(aq,regionsB))
    
    eff<-aggregate(aq@data[,82:92],by=list(aq@data$IMAGE24),median)
    
    reg2<-merge(regionsB@data,eff,by.x="IMAGE24",by.y="Group.1",all.x=TRUE)
    
    reg2<-reg2[match(regionsB@data$IMAGE24,reg2$IMAGE24),]
    
    regionsB@data<-reg2
    
    regions2<-st_as_sf(regionsB)
    
    ggplot()+
      geom_sf(data=regions2,aes(fill=richnessALLdiff))+
      theme_map()
  ...
  ggplot()+
    geom_tile(data=sum3, aes(x=x,y=y,fill=layer))+
    theme_map()+
