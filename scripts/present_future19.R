...
for (i in (i+1):length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	breaks1<-seq(from=0,to=max(res8$pop_ssp3),by=(max(res8$pop_ssp3)/10))
	
	res8[,ExposureSSP1:=cut(pop_ssp1,breaks=breaks1,labels=FALSE)/10]
	res8[,ExposureSSP2:=cut(pop_ssp2,breaks=breaks1,labels=FALSE)/10]
	res8[,ExposureSSP3:=cut(pop_ssp3,breaks=breaks1,labels=FALSE)/10]
	res8[,Exposurepresent:=cut(humans2010,breaks=breaks1,labels=FALSE)/10]
	
	breaks2<-seq(from=0,to=max(res8$gdp_ssp1),by=(max(res8$gdp_ssp3)/10))
	
	res8[,VulnerabilitySSP1:=(1-(cut(gdp_ssp1,breaks=breaks2,labels=FALSE)/10))*value]
	res8[,VulnerabilitySSP2:=(1-(cut(gdp_ssp2,breaks=breaks2,labels=FALSE)/10))*value]
	res8[,VulnerabilitySSP3:=(1-(cut(gdp_ssp3,breaks=breaks2,labels=FALSE)/10))*value]
	res8[,Vulnerabilitypresent:=(1-(cut(humans2010,breaks=breaks2,labels=FALSE)/10))*valuep]
	...
	res8[,RiskP:=Hazard * Exposurepresent *1]# pov_frac_present]
