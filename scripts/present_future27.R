...
for (i in (i+1):length(pres1)){
  ...
  rm(res6);gc()
	...
	res9$SAMP<-1000#sample(c(500,1000,2500),1)
	  
	res9$PAn=10000#SAMP*sample(c(0.5,1,10,25),1)
	...
	##try each one
	thresh<-thr[tt]
	res9$TSSv<-TSS(true=as.numeric(res6d$ea2),predicted=res6d$value,threshold=thresh[[1]])$TSS
	
	###END OF THRESHOLDING
	
	##make thresholded version of risk
	ad1c$HazardT<-ad1c$Hazard
	ad1c$HazardT[ad1c$HazardT>thresh[[1]]]<-1
	ad1c$HazardT[ad1c$HazardT!=1]<-0
	
	##make thresholded version of risk
	res9$HazardT<-res9$Hazard
	res9$HazardT[res9$HazardT>=thresh[[1]]]<-1
