...
glF3$spillover_rate2<-cut(glF3$cases_per_year,breaks=c(0,0.99,999,9999,999999999999),labels=FALSE)/4
table(glF3$spillover_rate2)
glF3$dummy=1

xtabs(data=glF3,dummy~spillover_rate2+Type2)
##popdgp
...
for (j in j:length(ad1)){
  ...
  ##no non diseases
     ...
     for (k in 1:4){
       ...
       typex=NULL
       skip=0
       if(k==1) {typex="Emerging";if(disdet$spillover_rate2 %in% c(0.5,0.75,1)){skip=1}}
       if(k==2) {typex="Established";if(disdet$spillover_rate2 %in% c(0.25,0.75,1)){skip=1}}
       if(k==3) {typex="Endemic";if(disdet$spillover_rate2 %in% c(0.25,0.5,1)){skip=1}}
       if(k==4) {typex="Hyperendemic";if(disdet$spillover_rate2 %in% c(0.25,0.5,0.75)){skip=1}}
