...
for (x in 1:length(dis1)){
    ...
    res6h[,.("clim_upper95","clim_lower95","realised_upper95","realised_lower95","clim_present_mean","lc_suit_present_mean","abundance_present_mean","realised_present_mean","speed_mean_vector","d_mean_vector","density_mean_vector"),]<-1
                }
    ...
    if(ncol(lt3)>1){
      ...
      res6<-res6[ltp,nomatch=0]
              } else{ res6[,secondary_future:=0];res6[,secondary_present:=0]} ## should be zero because adding to hosts where host + vectors>1
    rm(lt3)#;gc()
    ...
    ##endemic_countries <- too validate <-----------------problem ##################sort out which are in endemic countries
    ...
    res6<-res6[,names(res6)[!names(res6) %in% c("cell.id.2","i.id","i.ID","i.cell.id.2","i.time","i.time","i.type","i.cell.id","i.cell.id.1") ], with = FALSE ]
