...
for (x in 9:length(dis1)){
  ...
  ##choose one disease
	...
	if(paste(" ",dis1[x]," summary.csv",sep="") %in% list.files("X:\\DRtemp\\per_disease\\", pattern=".csv",full.names=FALSE)){next}
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			if(nrow(spec_desp2)==0|nrow(spec_dist2)==0){
			  ...
			  ff1<-ff[ff$data_source_title=="NCBI",];db="ncbi"
			  if(nrow(ff1)==0){ff1<-ff[ff$data_source_title=="EOL",];db="eol"}
			  if(nrow(ff1)==0){ff1<-ff[ff$data_source_title=="ITIS",];db="itis"}
			  if(nrow(ff1)==0){break}
			  ff1<-ff1[!duplicated(ff1$user_supplied_name),"matched_name"]
			  ff2<-get_uid_(ff1)
			  ...
			  ffx1<-ffx[ffx$data_source_title=="NCBI",];db="ncbi"
			  if(nrow(ffx1)==0){ffx1<-ffx[ffx$data_source_title=="EOL",];db="eol"}
			  if(nrow(ffx1)==0){ffx1<-ffx[ffx$data_source_title=="ITIS",];db="itis"}
			  if(nrow(ffx1)!=0){
			     ffx<-ffx1[!duplicated(ffx1$user_supplied_name),"matched_name"]
			     ffx2<-get_uid_(ffx)
			     if(!is.null(ffx2[[1]])){
			      ffx3<-do.call("rbind",ffx2)
			
			       c2<-classification(ffx3$uid,db="ncbi", accepted = TRUE)
			       species2f<-base::suppressWarnings(tolower(unique(unlist(lapply(names(c2), function(x) NewVar(c1[[x]], "family" ,"name"))))))
			       species2o<-base::suppressWarnings(tolower(unique(unlist(lapply(names(c2), function(x) NewVar(c1[[x]], "order" ,"name"))))))
			       species2c<-base::suppressWarnings(tolower(unique(unlist(lapply(names(c2), function(x) NewVar(c1[[x]], "class" ,"name"))))))
			     }
			   } 
  		...
  		##put in mean if still none <- ADD MORE TO DATAFRAME
			...
			link4<-link4[link4$year!=2010,]
					...
					if((class(clims2)[1]=="simpleError")==TRUE){print("no present day");next}
	 ...
	 ##no present day to compare and not run any so next x
	 if(is.null(res4)|is.null(res5)){next}
    ...
    #save present day
    save(res6,file=paste("X:/DRtemp/per_disease/",dis3$disease,"present.r"))
    
    
    ### THEN VALIDATION
    ### AUC using the present reliased and the ec variable for all cells
    ### need to make present realised out of 1? what about extreme values?
    ### if in top 5% then 1 otherwise
    #library(dismo)
    #res1$AUC1<-evaluate(p=res6$present_realised[res6$ea==1],a=res6$present_realised[res6$ea==0])@auc
    
    for(qq in 1:5){
      
      humans=(res6[,paste("ssp",qq,sep=""),with=FALSE])/5.6
    ...
    write.csv(res10,file=paste("X:/DRtemp/per_disease/",dis3$disease,"summary.csv"))
