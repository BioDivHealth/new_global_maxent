...
for (x in 1:length(dis1)){
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			if( paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,".tif",sep="") %in% list.files("X:/DRtemp/resultsZ/",pattern=".tif",full.names=TRUE)){
					...
					dt2<-dt1[!is.na(extract(ws_ras,dt1)),]
					
					##only use countries in subset
					countr2<-crop(wrld_simpl,template2)
    ...
    for(qq in 1:5){
      ...
      #res6[,future_realised_change_upper_div:= present_realised_upper/future_realised_upper,]
      ...
      #res6[,future_realised_change_lower_div:= present_realised_lower/future_realised_lower,]
    ...
    #res8<-res7[,.(sum(present_realised_upper,na.rm=TRUE),sum(future_realised_upper,na.rm=TRUE),sum(present_realised_lower,na.rm=TRUE),sum(future_realised_lower,na.rm=TRUE),mean(future_realised_clim_upper,na.rm=TRUE),mean(future_realised_clim_upper_div,na.rm=TRUE),mean(future_realised_clim_lower,na.rm=TRUE),mean(future_realised_clim_lower_div,na.rm=TRUE),sum(future_realised_change_upper,na.rm=TRUE),sum(future_realised_change_lower,na.rm=TRUE),sum(future_realised_change_lower_div,na.rm=TRUE)),.(year,RCP,ssp)]
    #names(res8)[4:ncol(res8)]<-c("present_realised_upper","future_realised_upper","present_realised_lower","future_realised_lower","future_realised_clim_upper","future_realised_clim_upper_div","future_realised_clim_lower","future_realised_clim_lower_div","future_realised_change_upper","future_realised_change_lower","future_realised_change_lower_div")
    
    res8<-res7[,.(sum(present_realised_upper,na.rm=TRUE),sum(future_realised_upper,na.rm=TRUE),sum(present_realised_lower,na.rm=TRUE),sum(future_realised_lower,na.rm=TRUE),mean(future_realised_clim_upper,na.rm=TRUE),mean(future_realised_clim_lower,na.rm=TRUE),sum(future_realised_change_upper,na.rm=TRUE),sum(future_realised_change_lower,na.rm=TRUE)),.(year,RCP,ssp)]
    names(res8)[4:ncol(res8)]<-c("present_realised_upper","future_realised_upper","present_realised_lower","future_realised_lower","future_realised_clim_upper","future_realised_clim_lower","future_realised_change_upper","future_realised_change_lower")
