...
for (i in (i+1):length(pres1)){
  ...
  ###ad in samples countries if not in database
  ...
  #rm(res6);gc()
	...
	res8[,Vulnerability:=log((gdp2010/(humans2010+0.01))+1)]
	#res8$Vulnerability<-max(res8$Vulnerability,na.rm=TRUE)-res8$Vulnerability
	res8[,VulnerabilitySSP1:=log((gdp_ssp1/(pop_ssp1+0.01))+1)]
	res8[,VulnerabilitySSP2:=log((gdp_ssp2/(pop_ssp2+0.01))+1)]
	res8[,VulnerabilitySSP3:=log((gdp_ssp3/(pop_ssp3+0.01))+1)]
	
		
	#high gdp -10 low vulnerability
	#1-gdp - poor is high
	
	###gini 1 is unequal so high is bad
	...
	###but where hazard is great/less than threshold!
	
	#res8[,RiskP:= Hazard]# pov_frac_present]
	...
	adc_data<-res8[res8$year_RCP==unique(res8$year_RCP)[3],]
	ad1b<-ad1[ad1$diss_me %in% unique(res8$diss_me),]
	
	## plot result
	#adc_data<-adc_data[!is.na(adc_data$diss_me),]
	adc_data<-adc_data[(match(ad1b$diss_me,adc_data$diss_me)),]
	ad1b@data<-cbind(ad1b@data,as.data.frame(adc_data))
	t2<-crop(ws2,ad1b)
	ad1b$dummy<-1
	ad1b<-st_as_sf(ad1b)
	mask1<-t2#fasterize(ad1b,t2,"dummy")
	expo<-fasterize(ad1b,t2,"humans2010")
	vuln<-fasterize(ad1b,t2,"gdp2010")
	vuln<-vuln/expo
	expo=log(expo+1)
	vuln=log(vuln+1)
	vuln<-vuln/max(values(vuln),na.rm=TRUE)
	vuln[is.na(vuln)]<-median(values(vuln),na.rm=TRUE)
	vuln<-mask(vuln,mask1)
	expo<-expo/max(values(expo),na.rm=TRUE)
	expo[is.na(expo)]<-median(values(expo),na.rm=TRUE)
	expo<-mask(expo,mask1)
	...
	#vw<-exp(sample(1:5,1,replace=TRUE))/exp(3)
	#ew<-exp(sample(1:5,1,replace=TRUE))/exp(3)
	#if(dd %in% c(4,5,10,12,13)){next}
	
	tt<-expand.grid(seq(0,1,1/3),seq(0,1,1/3))
	tt<-tt[c(1,4,7,8,10,12:16),]
	tt<-rbind(tt,tt[3:nrow(tt),])

	
	###just convert to sf and then can used natively
	ts1<-Sys.time()
  res95<-NULL;res95b<-NULL
	for (dd in 1:nrow(tt)){
	  ...
	  ##weights
	  if(dd>9){vuln_sign=0} else {vuln_sign=1}
    if(vuln_sign==1){vuln1=vuln} else {vuln1=1-vuln}
	  mask2=mask1*(((vuln1^2)*vw)/2)+(((1-(expo^2))*ew)/2)
	  if(dd==1){mask2[mask2==0]<-1}
	  
	  ###CREATE WEIGHTED RISK
	  ### create weighted background
	  
	  #adc_data[,RiskP:= scale(Hazard) + (vw*scale(Vulnerability)) + (ew*scale(Exposure))]
	...
	###if there are real data then also use that 
	#if(nrow(pd)>0){pd2<-st_as_sf(pd)}else{pd2<-st_as_sf(zz)}
	
	##create sf from endemic countries  
	countr2<-st_as_sf(ad2)
	
	##make raster layer
	ras1<-fasterize(ad1c,mask1,field="Hazard")  ###shouldn't need a function
	#ras2<-fasterize(ad1c,template,field="RiskP",fun="count")
	#ras1<-ras1/ras2
	ras1[is.na(ras1)]<-0
	ras1<-mask(ras1,mask1)
	...
	##generate psuedoabsence numbers
	SAMP<-sample(c(100,500,1000,1500),1)
	PAn<-SAMP*sample(c(0.5,1,10),1)
	 
	##measure AUC
	#system.time(rand1<-randomPoints(mask=mask2,n=res9$PAn[1]))###takes time
	
	#ts1<-Sys.time()
	xs<-sample(seq(from=extent(mask2)[1],to=extent(mask2)[2],by=0.01),replace=TRUE,PAn*100)
	ys<-sample(seq(from=extent(mask2)[3],to=extent(mask2)[4],by=0.01),replace=TRUE,PAn*100)
	rand1<-data.frame(x=xs,y=ys)
	probs<-extract(mask2,rand1)
	rand1<-rand1[!is.na(probs),]
	probs<-probs[!is.na(probs)]
	rand1<-rand1[sample(1:nrow(rand1),size=PAn,prob=probs,replace=TRUE),]
	#print(	Sys.time()-ts1)
	
	
	rand2<-extract(ras1,rand1)
	real2<-spsample(ad2,SAMP,type="random")
	real3<-extract(ras1,real2)
	
	opts<-c('ROC', 'TSS', 'KAPPA', 'ACCURACY', 'BIAS', 'POD', 'FAR', 'POFD', 'SR', 'CSI', 'ETS', 'HK', 'HSS', 'OR' , 'ORSS')
	
	#if real data the do AUC
	#if(nrow(pd)>0){
	  
	#  realPD<-extract(ras1,pd2)
	#  ##comparison by country/admin areas
	#  res6d<-rbind(data.frame(ea2=rep("0",length(rand1)),value=rand2),data.frame(ea2=rep("1",length(realPD)),value=realPD))
	 
	#  thr1pd<-Find.Optim.Stat(Stat=opts[1],Fit=res6d$value,Obs=res6d$ea2)
	#  thr2pd<-Find.Optim.Stat(Stat=opts[2],Fit=res6d$value,Obs=res6d$ea2)
	#  thr3pd<-Find.Optim.Stat(Stat=opts[3],Fit=res6d$value,Obs=res6d$ea2)
  #	}
	
	##comparison by country/admin areas
	res6d<-rbind(data.frame(ea2=rep("0",length(rand1)),value=rand2),data.frame(ea2=rep("1",length(real3)),value=real3))
	
	thr1<-Find.Optim.Stat(Stat=opts[1],Fit=res6d$value,Obs=res6d$ea2)
	colnames(thr1)<-paste("AUC_",colnames(thr1),sep="")
	thr2<-Find.Optim.Stat(Stat=opts[2],Fit=res6d$value,Obs=res6d$ea2)
	colnames(thr2)<-paste("TSS_",colnames(thr1),sep="")
	thr3<-Find.Optim.Stat(Stat=opts[3],Fit=res6d$value,Obs=res6d$ea2)
	colnames(thr3)<-paste("KAPPA_",colnames(thr1),sep="")
	
	#res6d$value<-res6d$value+abs(min(res6d$value,na.rm=TRUE))
	#res6d$value<-res6d$value/max(res6d$value,na.rm=TRUE)
	...
	##make thresholded version of risk
	...
	reszz<-cbind(data.frame(name=dis_trans$name,SAMP,PAn,ew,vw,vuln_sign,zz),thr1,thr2,thr3)
	
	
	#if(nrow(pd)>0){res9<-cbind(res9,thr1,thr2,thr3,thr1pd,thr2pd,thr3pd)} else {res9<-cbind(res9,thr1,thr2,thr3,thr1,thr2,thr3)}
	#names(res9)[(ncol(res9)-11):ncol(res9)]<-paste(names(res9)[(ncol(res9)-11):ncol(res9)],"_pd",sep="")
	
	#ggplot(data=aa)+
	#  geom_sf()	

	if(zz==1){res9<-reszz} else {res9<-rbind(res9,reszz)}
	}###auc loop zz
	
	
	#res
	res9b<-res9[res9$AUC_best.stat>=quantile(res9$AUC_best.stat,0.75),]
	res9b<-aggregate(res9b[,2:ncol(res9)],by=list(res9b$name),mean,na.rm=TRUE)
	
	if(is.null(res95)){res95<-res9;res95b<-res9b}else{res95<-rbind(res95,res9);res95b<-rbind(res95b,res9b)}
	rm(res9,res9b)
	...
	res99<-res95b
 #res99<-res95[!duplicated(res95$AUC_best.stat),]
	res99$vuln_sign[res99$vuln_sign==0]<-(-1)
 res99$vw<-res99$vw*(-1*res99$vuln_sign)
 res99$auc=round(res99$AUC_best.stat,5)
 plot(res99$vw,res99$ew,cex=res99$auc*3,pch=20,main=pres2[i])

 vw1<-min(res99[res99$auc==max(res99$auc),"vw"],na.rm=TRUE) ### closest to 1 that is the same
 ew1<-min(res99[res99$auc==max(res99$auc),"ew"],na.rm=TRUE)
 points(vw1,ew1,col="red",cex=2,pch=20)
 
#if(i==1){df1<-data.frame(name=pres2[i],vw1,ew1,SAMP=res9$SAMP[1],PAn=res9$PAn[1],auc=thr1[1])} else {df1<-rbind(df1,data.frame(name=pres2[i],vw1,ew1,SAMP=res9$SAMP[1],PAn=res9$PAn[1],auc=thr1[1]))}
 if(i==1){res100<-res99;resALL<-res95} else {res100<-rbind(res100,res99);resALL<-rbind(resALL,res95)}
  rm(res95,res99)
...
save(resALL,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\resALL.r")
save(res100,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\res100d.r")

df1<-aggregate(res100$auc,by=list(res100$name),max)

hist(df1$x,breaks=20)
abline(v=0.5)


df1$uni<-paste(df1$name,df1$auc,sep="_")
df1<-df1[!duplicated(df1$uni),]

hist(df1$auc,breaks=20)
...
nrow(df1[df1$x>0.5,])
	...
	##make thresholded version of risk
	res9$RiskPT<-res9$Hazard
	res9$RiskPT[res9$RiskPT>=thr1[[2]]]<-1
	res9$RiskPT[res9$RiskPT!=1]<-0
	
		###future risk
	res9[,RiskSSP1:= scale(Hazard) + (vw*scale(Vulnerability))* (ew*scale(Exposure))]
	     
	res9[,RiskSSP2:= RiskPT + (1+VulnerabilitySSP2) * (1+ExposureSSP2)]
	res9[,RiskSSP3:= RiskPT + (1+VulnerabilitySSP3) * (1+ExposureSSP3)]
	
	###do again for present day
	#res8[,RiskP:= scale(Hazard,center=FALSE,scale=TRUE) * scale(Exposure,center=FALSE,scale=TRUE) * scale(Vulnerability,center=FALSE,scale=TRUE)]# pov_frac_present]
	#res8[,RiskSSP1:= scale(Hazard_future,center=FALSE,scale=TRUE) * scale(VulnerabilitySSP1,center=FALSE,scale=TRUE) * scale(ExposureSSP1,center=FALSE,scale=TRUE)]
	#res8[,RiskSSP2:= scale(Hazard_future,center=FALSE,scale=TRUE) * scale(VulnerabilitySSP2,center=FALSE,scale=TRUE) * scale(ExposureSSP2,center=FALSE,scale=TRUE)]
	#res8[,RiskSSP3:= scale(Hazard_future,center=FALSE,scale=TRUE) * scale(VulnerabilitySSP3,center=FALSE,scale=TRUE) * scale(ExposureSSP3,center=FALSE,scale=TRUE)]
  ...
  rm(res8,res9,adc_data,ad1c)
	gc()
  print(i)
}

##save results
write.csv(res11,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\final_risk3.csv")


#res12<-	res11[,lapply(.SD,function (x) sum(x,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=names(adc_data)[3:13]]
