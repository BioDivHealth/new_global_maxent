...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  ##aggregate by page
  res7<-aggregate(res6[,c("disease_richness","humans","cases","deaths","x","y","max_x","max_y","min_x","min_y","RCP","Year","SSP","CFR.low","CFR.high","cases_per_year","spillover_rate2","Outbreak","Vectored")],by=list(res6$page,res6$model),FUN=mean)
  
  
  
  ggplot(res7,aes(x=Year,shape=as.factor(Vectored),y=humans,col=as.factor(RCP))) +
  ...
  ggplot(res7,aes(x=RCP,y=disease_richness,col=as.factor(Year))) +
    stat_summary(fun = function(x) mean(x,trim=0.1), geom = "point",size=2,position = position_dodge(3)) + 
   # stat_summary(fun.data = mean_se, geom = "errorbar",width=0.2,position = position_dodge(3))+
    #geom_violin()+
    #geom_point(aes(fill=as.factor(RCP)),size=3,alpha=0.5) +
    #scale_x_log10()+
    ...
    facet_wrap(.~Vectored)+
    xlab("RCP")+
    ylab("Change in size of endemic area")+
    geom_abline(intercept=0,slope=1,lty=2)+
    xlim(2,9)
  
  
  ggplot(res7,aes(x=RCP,shape=as.factor(Vectored),y=cases,col=as.factor(Year))) +
    stat_summary(fun = mean, geom = "point",size=2,position = position_dodge(3)) + 
    stat_summary(fun.data = mean_se, geom = "errorbar",width=0.2,position = position_dodge(3))+
    #geom_point(aes(fill=as.factor(RCP)),size=3,alpha=0.5) +
    #scale_x_log10()+
    ...
    #geom_line(aes(group =  disease ),color="grey")+
    xlab("RCP")+
    ylab("Change in cases")+
    geom_abline(intercept=0,slope=1,lty=2)+
    xlim(2,9)
  
  
  
  ggplot(res7,aes(x=RCP,shape=as.factor(Vectored),y=deaths,col=as.factor(Year))) +
    stat_summary(fun = mean, geom = "point",size=2,position = position_dodge(3)) + 
    stat_summary(fun.data = mean_se, geom = "errorbar",width=0.2,position = position_dodge(3))+
    #geom_point(aes(fill=as.factor(RCP)),size=3,alpha=0.5) +
    #scale_x_log10()+
