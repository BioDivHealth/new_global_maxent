...
for (i in (i+1):length(pres1)){
  ...
  rm(res6);gc()
	...
	for (dd in 1:100){
	  ...
	  vw<-exp(sample(1:5,1,replace=TRUE))/exp(4)
	  ew<-exp(sample(1:5,1,replace=TRUE))/exp(4)
	...
	res9$SAMP<-sample(c(500,1000,2500),1)
	...
	thr1<-Find.Optim.Stat(Stat=stat1,Fit=res6d$value,Obs=res6d$ea2)
	#thr1
