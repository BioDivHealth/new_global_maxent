...
for (x in 1:length(dis1)){
    ...
    res5b[is.na(res5b)]<-0
    names(res5b)[3:ncol(res5b)]<-c(paste(names(res5)[4:7],"mean",sep="-"),paste(names(res5)[4:7],"sd",sep="-"))
    ...
    ##across models - remove models effect
    res5b<-res5[ ,.(mean(clim_future,na.rm=TRUE), mean(lc_suit_future,na.rm=TRUE), sum(abundance_future,na.rm=TRUE), sum(realised_future,na.rm=TRUE)), by = .(type,species,RCP,cell_by_year)]
    ...
    names(res4b)[3:ncol(res4b)]<-c(paste(names(res4)[c(8:11,4:6)],"mean",sep="-"))
    
    ##future by type cellby RCP 
    res5c<-res5b[ ,.(mean(clim_future,na.rm=TRUE), mean(lc_suit_future,na.rm=TRUE), sum(abundance_future,na.rm=TRUE), sum(realised_future,na.rm=TRUE)), by = .(type,cell_by_year_by_RCP)]
    
    ###turn both data.frames into wide???
    ...
    ###make year by cell.id
    res6$code<-gsub("_2",";2",res6$code)
    res6$code<-gsub("0_","0;",res6$code)
    res6[,c("model","year","RCP"):=tstrsplit(code,";")]
