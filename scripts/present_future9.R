...
for (i in i:length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res7<-res6[!is.na(sub_region),lapply(.SD,sum,na.rm=TRUE),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_UW","Hazard_future","Hazard_future_UW")]
	...
	res6[,Exposure:=sum_human_population_future]
	...
	res6[,res6$RiskP:=HazardUW * CFR * spillover_rate * sum_human_population * pov_frac]
