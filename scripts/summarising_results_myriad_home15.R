...
#for (j in j:length(ad1)){
  ...
  ##no non diseases
     ...
     ## non disease dummy
     if(disdet$disease=="None reported"){ resF[,dummy1:=0]} else { resF[,dummy1:=0]}
     ...
     # raster of present cell disease only
    ...
    ### humans      
     ...
     df1<-data.frame(disease=resF$name[1],model="present",richness=sum(values(pres4),na.rm=TRUE),disease_richness=sum(values(pres4a),na.rm=TRUE),humans=sum(values(pres4b),na.rm=TRUE),cases=sum(values(pres_spill),na.rm=TRUE),deaths=sum(values(pres_spill),na.rm=TRUE))
     ...
     #for (qq in 1:nrow(combs)){
       ...
       df2<-data.frame(disease=resF$name[1],model=paste(combs[qq,2],"all",sep="_"),richness=sum(values(fut4),na.rm=TRUE),disease_richness=sum(values(fut4a),na.rm=TRUE),humans=sum(values(fut4b),na.rm=TRUE),cases=sum(values(fut_spill),na.rm=TRUE),deaths=sum(values(fut_spill),na.rm=TRUE))
     ...
     ##indivdual results
     df4<-rbind(df1,df3)
     write.csv(df4,format="GTiff",file=paste("C:\\Users\\david.redding\\Documents\\more_results\\per_disease_results","_",sample(1e7,1),".csv",sep=""))
     
     ##remove for next round
     rm(fut4,fut4a,fut4b,fut_spill,fut_spill2,qq,df1,df2,df3,df4)
...
##get names
names1<-ls_then2[!ls_then2 %in% c("qq","ls_now")]

## add together
names2<-c(pres_names,names1)

for (i in 6:length(names2)){

  rast1<-get(names2[i])
  
writeRaster(rast1,format="GTiff",file=paste("C:\\Users\\david.redding\\Documents\\more_results\\",names2[i],"_",sample(1e7,1),".tif",sep=""))

}
  

##get types
types<-read.table(text=names1,sep="_",stringsAsFactors = F)

##get ssp
sspxrcp<-read.table(text=types$V1,sep="X",stringsAsFactors = F)

##put into new table
types2=data.frame(name=names1,types,ssp=sspxrcp$V1,rcp=sspxrcp$V2,stringsAsFactors = F)

##rename variables
types2$V2<-gsub("XALL","",types2$V2)
names(types2)[3]<-c("Year")
names(types2)[7]<-c("Change")
types2$type<-paste(types2$V4,types2$V5)

##choose key columns
types2<-types2[, c("name","type","Year","Change","rcp","ssp")]

###make uni without ssp
types2$uni2<-paste(types2$type,types2$Year,types2$Change,types2$rcp,sep="_")

###
namesx<-unique(types2$uni2)
...
for (w in 1:length(names1)){
  ...
  dataf1<-data.frame(types,data=sum(values(rast1)))
  
  if(is.null(dataf2)){dataf2<-dataf1} else {dataf2<-rbind(dataf1,dataf2)}
...
test2<-aggregate(ssp3X8.5_2080XHOST_HUMAN_future_endemic_richness_increase,100,method='bilinear')
test2[test2==0]<-NA
...
plot(ssp3X2.6_2070XALL_future_endemic_richness,main="ALL",col=viridis(20,option="A"))
plot(ssp3X2.6_2070XALL_future_endemic_richness_increase/ssp3X2.6_2070XALL_future_endemic_richness,main="ALL",col=viridis(20,option="A"))
plot((-1*ssp3X2.6_2070XALL_future_endemic_richness_decrease)/ssp3X2.6_2070XALL_future_endemic_richness,main="ALL",col=viridis(20,option="A"))
