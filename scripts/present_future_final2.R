...
source("C:\\Users\\david.redding\\Dropbox\\R_scripts\\TSS.r")
...
load(file="C:\\Users\\david.redding\\Documents\\globalIUCN\\popgdp7.r")
...
gini<-stack("C:\\Users\\david.redding\\Documents\\globalIUCN\\GINI_STACK.tif")
names(gini)<-read.csv("C:\\Users\\david.redding\\Documents\\globalIUCN\\GINI_STACK_NAMES.csv")$x
gini2<-velox(gini)
gini3<-as.data.frame(gini2$extract(ad1,fun=function (x) mean(x,na.rm=TRUE)))
#gini3<-extract(gini2,ad1,mean,na.rm=TRUE)

names(gini3)<-read.csv("C:\\Users\\david.redding\\Documents\\globalIUCN\\GINI_STACK_NAMES.csv")$x
...
save(gini5,file="C:\\Users\\david.redding\\Documents\\globalIUCN\\GINI5.r")
load(file="C:\\Users\\david.redding\\Documents\\globalIUCN\\GINI5.r")

##read in two country points
lesstwo<-read.csv("C:\\Users\\david.redding\\Dropbox\\data\\disease_in_two_or_fewer_countries4.csv",stringsAsFactors=FALSE)
...
###read disease data
d1<-read.csv("C:\\Users\\david.redding\\Dropbox\\disease_table32b.csv",stringsAsFactors=FALSE)
...
###find all present day
pres1<-list.files("C:\\Users\\david.redding\\Documents\\per_disease3\\",pattern=" ALL4",full.names=TRUE)
pres2<-list.files("C:\\Users\\david.redding\\Documents\\per_disease3\\",pattern=" ALL4",full.names=FALSE)
...
for (i in 1:length(pres1)){
  ...
  ##optimums
  #res100c<-res100b[1,]#paste(" ",res100b$Group.1,sep="")==pres2[i],]
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	for (uu in 1:10){
	  ...
	  if(nrow(res6[is.na(res6$countr),])<100){noabs=1;break}
  
	  ##random absence points
	  rand2=res6$Hazard[sample((1:nrow(res6))[is.na(res6$countr)],10000,replace=TRUE)]
	  real3=res6$Hazard[sample((1:nrow(res6))[!is.na(res6$countr)],10000,replace=TRUE)]
	...
	res6<-res6[,c("cell.id","year","RCP","x","y","countr","sub_region","region","diss_me","year_RCP","clim_present_mean2","clim_future_mean2","Hazard","Hazard_future","HazardT","Hazard_futureT","year_diss_me")]
	...
	#pl1<-ws2
	#pl1[pl1==1]<-0
	pl1[res6$cell.id[res6$year==2030]]<-res6$HazardT[res6$year==2030]
	plot(pl1)
	#points(xyFromCell(template,unique(res6$cell.id)),col="red",pch=20)
  ...
  #population gdp
	...
	#gini
	res8<-res8[gini5[,c("year_dissme","SSP1","SSP2","SSP3")],nomatch=0]
	...
	res8[,Vulnerability:=1-((  (  (1-(PRESENT/100))/(1+(PRESENT/100))  )*  ( ((gdp2010*1000)/humans2010)/thresh))^ ( ((PRESENT/100)+1)/(2*(PRESENT/100))  ))]
	res8$Vulnerability[	res8$Vulnerability>1]<-1
	res8$Vulnerability[	res8$Vulnerability<0]<-0
	#hist(res8$Vulnerability[res8$year==2050])
	res8[,c("i.year","PRESENT","gdp2010","humans2010")]<-NULL
	
	res8[,VulnerabilitySSP1:=1-((  (  (1-(SSP1/100))/(1+(SSP1/100))  )*  ( ((gdp_ssp1*1000)/pop_ssp1)/thresh))^ ( ((SSP1/100)+1)/(2*(SSP1/100))  ))]
	res8$VulnerabilitySSP1[	res8$VulnerabilitySSP1>1]<-1
	res8$VulnerabilitySSP1[	res8$VulnerabilitySSP1<0]<-0
	#hist(res8$VulnerabilitySSP1[res8$year==2030],add=T,col="red")
	res8[,c("SSP1","gdp_ssp1","pop_ssp1")]<-NULL;gc()
	
	res8[,VulnerabilitySSP2:=1-((  (  (1-(SSP2/100))/(1+(SSP2/100))  )*  ( ((gdp_ssp2*1000)/pop_ssp2)/thresh))^ ( ((SSP2/100)+1)/(2*(SSP2/100))  ))]
	res8$VulnerabilitySSP2[	res8$VulnerabilitySSP2>1]<-1
	res8$VulnerabilitySSP2[	res8$VulnerabilitySSP2<0]<-0
	#hist(res8$VulnerabilitySSP2[res8$year==2030],add=T,col="green")
	res8[,c("SSP2","gdp_ssp2","pop_ssp2")]<-NULL
	
	res8[,VulnerabilitySSP3:=1-((  (  (1-(SSP3/100))/(1+(SSP3/100))  )*  ( ((gdp_ssp3*1000)/pop_ssp3)/thresh))^ ( ((SSP3/100)+1)/(2*(SSP3/100))  ))]
	...
	res8[,c("SSP3","gdp_ssp3","pop_ssp3")]<-NULL;gc()
	
	# remove uneffected people
	res8[HazardT==0,c("Exposure","Vulnerability")]<-0
	res8[Hazard_futureT==0,c("ExposureSSP1","ExposureSSP2","ExposureSSP3","VulnerabilitySSP1","VulnerabilitySSP2","VulnerabilitySSP3")]<-0
	
  # risk calculate	
	res8[,Risk:=Hazard*Exposure*Vulnerability]
	...
	res7<-res8[!is.na(diss_me),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_future","HazardT","Hazard_futureT","clim_present_mean2","clim_future_mean2","Exposure","Vulnerability","ExposureSSP1","ExposureSSP2","ExposureSSP3","VulnerabilitySSP1","VulnerabilitySSP2","VulnerabilitySSP3","Risk")]
  #rm(res8);gc()
