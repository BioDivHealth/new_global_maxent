...
data(wrld_simpl)
#setwd('/scratch/scratch/ucbtdw0/dis2')
...
#bv<-dis1[x]
#save(bv,file="X:\\DRtemp\\babesia_problem.r")
...
#save(dis1,file="C:\\Users\\xxxx\\Dropbox\\legion2\\dis1.r")

load(file="C:\\Users\\xxxx\\Dropbox\\legion2\\dis1.r")
...
#write.csv(diseases,file="C:\\Users\\xxxx\\Dropbox\\legion2\\diseases1.csv")

diseases<-read.csv(file="C:\\Users\\xxxx\\Dropbox\\legion2\\diseases1.csv",stringsAsFactors = FALSE)


##get all points data for each
points1a<-list.files("C:\\Users\\Public\\Documents\\points\\", pattern="all_points",full.names=TRUE)
points1<-list.files("C:\\Users\\Public\\Documents\\points\\", pattern="_points",full.names=TRUE)
points2b<-points1[!points1 %in% points1a]
points1a<-list.files("C:\\Users\\Public\\Documents\\points\\", pattern="all_points",full.names=FALSE)
points1<-list.files("C:\\Users\\Public\\Documents\\points\\", pattern="_points",full.names=FALSE)
...
lt2<-fread("C:\\Users\\Public\\Documents\\livestock_future_2030_2050_2070_2080b.csv")
...
link2b<-list.files("C:\\Users\\Public\\Documents\\resultsY2", pattern=".tif",full.names=TRUE)
link2<-list.files("C:\\Users\\Public\\Documents\\resultsY2", pattern=".tif",full.names=FALSE)
#link2b2<-list.files("V:/resultsY", pattern="_present_",full.names=TRUE)
#link22<-list.files("V:/resultsY", pattern="_present_",full.names=FALSE)
#link2b<-c(link2b,link2b2)
#link2<-c(link2,link22)
link2<-gsub("_6.tif","_6.0.tif",link2, fixed=TRUE)
link2<-gsub("_present_",";2010;present",link2, fixed=TRUE) ##check when in
...
###
futlg<-read.csv(file="C:\\Users\\Public\\Documents\\futlg.csv",stringsAsFactors=FALSE)
futlg$filen<-gsub("C:","C:\\Users\\Public\\Documents",futlg$filen,fixed=TRUE)
...
spec_hab<-read.csv("C:\\Users\\xxxx\\Dropbox\\legion2\\habitat_both_fin.csv",stringsAsFactors=FALSE)
...
spec_dist<-read.csv("C:\\Users\\xxxx\\Dropbox\\legion2\\host_density_d_distance.csv",stringsAsFactors=FALSE)
spec_desp<-read.csv("C:\\Users\\xxxx\\Dropbox\\legion2\\dispersal_distance_all.csv",stringsAsFactors=FALSE)
...
##46 PROBLEM
##67 problem
# 165 - no vector or host
##196 - vector or host prob

##216 fail<-check or run loop
...
#dis1<-dis1[!paste0(" ",dis1) %in% pres2]

#dis1<-c("borrelia burgdorferi","lassa")
...
for (x in (x+1):length(dis1)){
  ...
  #### NUMBER 42 not in database - why???
  ## also: number c(42,63,56,54,50,51)
  
  ##choose one disease
	dis2<-diseases[diseases$disease==dis1[x],]
	if(length(dis2$tag[!is.na(dis2$tag)])==0){print("PROBLEM1")}#;next}
	
	#if(dis1[x]==bv){dis1[x]<-"babesia venatorum"}
	
	##only redo those with animals in
	#if(!any(dis2$name1 %in% c("ducks","chickens","cattle","ducks","goats","pigs","sheep"))){next}			
	
	###remove duplicates
	dis2<-dis2[!duplicated(dis2$name1),]
	
	#if(nrow(dis2)>=20){print("many_hosts");next}
	
	
	#print(dis2)
	#x=x+1
	#paste("C:/Users/Public/Documents/per_disease3/",dis3$disease,"ALL4.r")
	
	if(paste("",dis1[x]," ALL4.r",sep="") %in% list.files("C:/Users/Public/Documents/per_disease3/", pattern=" ALL4.r",full.names=FALSE)){next}
	
	
	
	if(dis1[x]=="tensaw "){dis1[x]<-"tensaw"}
		##endemic region
	dis_trans<-d1[d1$name==dis1[x],]
	if(dis_trans$no.countries>150){next}
	ccc<-strsplit(dis_trans$countries,",")[[1]]
	#if(length(ccc)>=75){print("not widespread");next}
	
	if(is.na(ccc[1])){print("no recognised countries");next}
	countr<-wrld_simpl[wrld_simpl$ISO2 %in% ccc ,]
	#if(nrow(countr)!=length(ccc)){print("issue with countries");print(ccc)}#;next}
	if(nrow(countr)==0){print("no recognised countries");next}
	...
	regn2<-suppressWarnings(crop(regn2,remove_small_islands(regn2,min_val=10)))
	...
	if(length(bothmod)==0){print("ERROR NO SCENARIO OVERLAP");print("PROBLEM2");next}

	##randomly choose hosts if too many
	if(nrow(dis2[dis2$type=="hosts"])>10)	{
	  
	  row_drop<-sample((1:nrow(dis2))[dis2$type=="hosts"],nrow(dis2[dis2$type=="hosts"])-10,replace=FALSE)
	...
	if(nrow(dis2[dis2$type=="vectors"])>10)	{
	  ...
	  row_drop<-sample((1:nrow(dis2))[dis2$type=="vectors"],nrow(dis2[dis2$type=="vectors"])-10,replace=FALSE)
    ...
    #dis2
		...
		for(z in 1:nrow(dis2)){
		  ...
		  #z=z+1
			...
			if(dis3$name1 %in% c("ducks","chickens","cattle","ducks","goats","human","pigs","sheep")){next}			
				...
				### load a mask
				#template2<-raster(paste("X:\\DRtemp\\temp\\masks\\",paste(ttt2,collapse="_"),".tif",sep=""))
			  #template2<-crop(ws_ras,template2)
				
				#lc_mask<-stack(futlgx1[futlgx1$V2==(strsplit(ttt,";")[[1]][3]),"filen"])
				#fnf2<-raster(fnfx1[fnfx1$V2==(strsplit(ttt,";")[[1]][3]),"filen"])
				#template2<-crop(template,template2)
			...
			##spec_hab ok
			if(nrow(spec_desp2)==0){spec_desp2<-data.frame(binom=NA,min=mean(spec_desp$min,trim=0.1,na.rm=TRUE),max=mean(spec_desp$max,trim=0.1,na.rm=TRUE),mean=mean(spec_desp$mean,trim=0.1,na.rm=TRUE),min_mean=mean(spec_desp$min_mean,trim=0.1,na.rm=TRUE),max_mean=mean(spec_desp$max_mean,trim=0.1,na.rm=TRUE),stringsAsFactors = FALSE)}
			if(nrow(spec_dist2)==0){spec_dist2<-data.frame(Taxa=NA,lower=NA,Average.of.Mass=mean(spec_dist$Average.of.Mass,trim=0.1,na.rm=TRUE),Average.of.walking.speed=mean(spec_dist$Average.of.walking.speed,trim=0.1,na.rm=TRUE),Average.of.d=mean(spec_dist$Average.of.d,trim=0.1,na.rm=TRUE),Average.of.density=mean(spec_dist$Average.of.density,trim=0.1,na.rm=TRUE),stringsAsFactors = FALSE)}
			...
			#fw1<-focalWeight(template2, d=c(disp, dispsd), "Gauss")### could make the shape a function of maximum to mean
			#dim(fw1)
			#sum(fw1)
          ...
          #world_simpl@data[wrld_simpl]
					...
					#dt2<-dt1[!is.na(as.vector(extract(template2,dt1))),]
					...
					#lcsuit2<-lcsuit#mask(lcsuit,template2)
					#specX<-rep(link5$species,ncell(template2))
					#specX[is.na(values(template2))]<-NA
					
					res1<-data.table(disease=dis3$disease,type=dis3$type,species=link5$species,speed=link5$speed,d=link5$d,density=link5$density,cell.id=values(template2), clim=values(clims2),lc_suit=values(lcsuit),i=1)
				...
				#print(ii)
				  ...
				  res_foreach<-data.table(species=link5$species,type=dis3$type,RCP=link5$RCP, year=link5$year,type=dis3$type,cell.id=values(template2), clim_future=values(clims2),lc_suit_future=values(lcsuit),i=ii)
				  ...
				  #if(ii==1){res3<-res_foreach} else {res3<-rbindlist(list(res3,res_foreach))}
				  #gc()
				  rm(lcsuit,clims2)
				  #return(res_foreach)
	 ...
	 if(is.null(res4)|is.null(res5)){print("PROBLEM5");next}
	 
    #plot(countr)
	  #points(xyFromCell(template,unique(res4$cell.id)))
    ...
    res4b<-res4[ ,.(median(clim_present,na.rm=TRUE), median(lc_suit_present,na.rm=TRUE),median(speed,na.rm=TRUE), median(d,na.rm=TRUE), mean(density,na.rm=TRUE)), by = .(type,cell.id,merge1)]
    ...
    #plot(countr)
    #points(xyFromCell(template,unique(res4b$cell.id)))
    ...
    ### the column heads are wrong
    #res5c<-res5[ ,.(mean(clim_future_mean,na.rm=TRUE), mean(clim_future_sd,na.rm=TRUE),mean(lc_suit_future_mean,na.rm=TRUE), mean(lc_suit_future_sd,na.rm=TRUE) ), by = .(type,cell_by_year_by_RCP,merge1)]
    res5c<-res5[ ,.(median(clim_future,na.rm=TRUE), sd(clim_future,na.rm=TRUE),median(lc_suit_future,na.rm=TRUE), sd(lc_suit_future,na.rm=TRUE)), by = .(type,cell_by_year_by_RCP,merge1)]
    ...
    #res5c[,c("cell.id","year","RCP"):=tstrsplit(cell_by_year_by_RCP,"_")]
    ...
    #& "seconds" %in% dis2$type
    #& dis_trans$Type == "HUMAN->VECTOR->HUMAN"
    #### if human vec human but has secondary ##rare??
    if(nrow(res6h)==0  ){res6h<-res6v;
    ...
    #save present day
    save(res6,file=paste("C:/Users/Public/Documents/per_disease3/",dis3$disease,"ALL4.r"))
    rm(res6);gc()
