...
lesstwo<-read.csv("E:\\Dropbox\\data\\disease_in_two_or_fewer_countries4.csv",stringsAsFactors=FALSE)
lesstwo$name2<-paste(" ",lesstwo$name,sep="")
lesstwo$dummy<-1
coordinates(lesstwo)<-~Longitude+Latitude

##read in admins to move away from GRID
ad1<-readOGR("E:\\Dropbox\\New_Global_MAXENT\\admin1\\ne_10m_admin_1_states_provinces.shp","ne_10m_admin_1_states_provinces")

##read in empress-i point data
point_data<-read.csv("E:\\Dropbox\\New_Global_MAXENT\\all_empress_i_data.csv",stringsAsFactors = FALSE)
point_data$LU<-paste(" ",point_data$name_LU,sep="")

###read disease data
d1<-read.csv("W:\\DRtemp\\disease_table30.csv",stringsAsFactors=FALSE)
...
#AUCs<-read.csv("E:\\Dropbox\\New_Global_MAXENT\\AUC3.csv",stringsAsFactors = FALSE)
...
#  system.time(
  ...
  for (xx in 1:2){
    ...
    #pdf(file=paste("E:\\Dropbox\\New_Global_MAXENT\\Present_maps\\Present_maps",pres2[i],"_",sample(1:1000,1),".pdf",sep=""),width=15,height=10)
    #plot(t3)
    #plot(ad2,add=TRUE,border="blue")
    #plot(countr,add=TRUE,border="red")
    ...
    ##when best breaks are found 
    pdf(file=paste("E:\\Dropbox\\New_Global_MAXENT\\Present_maps\\Present_maps_thresh_",pres2[i],"_",sample(1:1000,1),"clim.pdf",sep=""),width=15,height=10)
    ...
    ##when best breaks are found 
    pdf(file=paste("E:\\Dropbox\\New_Global_MAXENT\\Present_maps\\Present_maps_thresh_",pres2[i],"_",sample(1:1000,1),".pdf",sep=""),width=15,height=10)
...
##save results
write.csv(res3,file="E:\\Dropbox\\New_Global_MAXENT\\AUC2.csv")

##all AUCS
hist(res1$AUC,xlim=c(0,1),breaks=50,main="AUC scores of all diseases")
...
##save
write.csv(res1,file="E:\\Dropbox\\New_Global_MAXENT\\AUC2.csv")
