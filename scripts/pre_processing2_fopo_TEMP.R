...
futlg$scrap2<-gsub(".txt",".tif",futlg$filen,fixed=TRUE)
futlg$scrap2<-gsub("C:","X:/DRtemp/LUH1",futlg$scrap2,fixed=TRUE)

futlg$scrap3<-gsub("C:","X:/DRtemp/LUH1",futlg$scrap3,fixed=TRUE)

if(!dir.exists(file.path(mainDir, subDir)){ dir.create(file.path(mainDir, subDir)), FALSE)


xxx$filen<-gsub("C:","X:\\DRtemp\\LUH1",xxx$filen)
xxx$filen<-gsub("C:","X:\\DRtemp\\LUH1",xxx$tif)


for (i in 1:160){

temp1<-raster(futlg$filen)
writeRaster(temp1,format="GTiff",file=futlg$scrap2[i])
print(i)
}
