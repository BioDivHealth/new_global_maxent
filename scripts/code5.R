...
for (x in 1:length(dis1)){
	...
	dis2<-res1[res1$disease==dis1[x],]
		...
		for(z in 1:length(dis2)){
			...
			if(dis3$name1 %in% c("cattle","ducks","goats","human","pigs","sheep")){next}
			...
			species2<-unique(read.table(text=species1,sep="_",stringsAsFactors=FALSE)$V1)		
				...
				for(y in 1:length(years)){
					...
					### 
						for (i in 1:1000){
							...
							### dispersal ability scales with home range size, body mass and geographic range size
							### strong phylogenetic signal
							### how many have single birds - be generic with them???							

							### can then use daily movement distance to describe ## read going throught the motions

							## make matrix = to dispersal ability


							IP<-focal(IP,w= focalWeight(IP, d=c(2, 10), "Gauss"),mean,na.rm=TRUE)
