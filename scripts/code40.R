			...
			if(fail1==TRUE){  
			  ...
			  #}else{   
			    ...
			    if(sum (values(IP),na.rm=TRUE)==0){next}
					...
					res1<-data.table(disease=dis3$disease,type=dis3$type,species=specX,speed=link5$speed,d=link5$d,density=link5$density,cell.id=values(template2), clim=values(clims2),lc_suit=values(lcsuit2),abundance=values(k),realised=values(IP),i=1)
              ...
              ##if so just start here with IP
				  ...
				  #if(ii==2){res3<-res_foreach} else {res3<-rbl(res3,res_foreach)}
				  gc()
