...
###read disease data
d1<-read.csv("C:\\Users\\xxxx\\Dropbox\\disease_table32.csv",stringsAsFactors=FALSE)
...
for (i in i:length(pres1)){
  ...
  pres_ad2<-fasterize(st_as_sf(wrld_simpl),template,field="REGION")
  res6$region<-extract(pres_ad2,res6$cell.id)
  
  conts<-unique(na.omit(res6$region))
  
  ###do it per continent
  for(zz in 1:length(conts)){
...
#  system.time(
  for (xx in 1:2){
    ...
    res6b<-res6[!is.na(realised_present_mean2) & res6$region==conts[zz],]
    ...
    for (z  in 1:5){
      ...
      if(nrow(pd)>0){
        ...
        if(is.null(res1)){res1<-data.frame(disease=pres2[i],AUC=e@auc,thr,z,lcsuit,AUC_PD=epd@auc,thrpd,p.data,region=conts[zz])}else{res1<-rbind(res1,data.frame(disease=pres2[i],AUC=e@auc,thr,z,lcsuit,AUC_PD=epd@auc,thrpd,p.data,region=conts[zz]))}
