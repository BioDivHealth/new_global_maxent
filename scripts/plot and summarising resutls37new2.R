...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6q2<-res6[,c(36:46)]-res6[,c(5:15)]
  ...
  ggplot(res6q[res6q$Kingdom!="fungi", ],aes(x=(richness/richnessALL),col=as.factor(Year)))+
    geom_density(adjust=2)+
  ...
  ggplot(res6q[res6q$Kingdom!="fungi", ],aes(x=(richness/richnessALL),fill=as.factor(Year)))+
  ...
  ###examining disease no disease
  
  res6q<-res6[ res6$AUC>0.5  & res6$threshold=="ALL",]
  
  
  xt<-xtabs(data=res6q[!duplicated(res6q$disease) ,],dummy~Disease+Vectored)
  chi1<-chisq.test(xt)
  chi1$observed
  #xt/chi1$expected
  chi1$stdres
  ...
  xt<-xtabs(data=res6q[!duplicated(res6q$disease) ,],dummy~Disease+Kingdom)
  chi1<-chisq.test(xt)
  chi1$observed
  #xt/chi1$expected
  chi1$stdres
  
  xt<-xtabs(data=res6q[!duplicated(res6q$disease) ,],dummy~Disease+TempTrop)
  chi1<-chisq.test(xt)
  chi1$observed
  #xt/chi1$expected
  chi1$stdres
  
  zz<-aggregate(log(res6q[ ,c(5:15,36:46)]),by=list(res6q$Year,res6q$Disease),median)
  ((exp(zz$richness[zz$Group.1==2080 & zz$Group.2=="Disease"]) -exp(zz$richness[zz$Group.1==2030 & zz$Group.2=="Disease"]))*ar1)/50
  ((exp(zz$richness[zz$Group.1==2080 & zz$Group.2=="No Disease"]) -exp(zz$richness[zz$Group.1==2030 & zz$Group.2=="No Disease"]))*ar1)/50
...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  typesX<-types#"Baseline","Vectored","Emerging","Kingdom","Tropical/Temperate",
  ...
  for(tt in 1:length(types)){
    ...
    for(ff in 1:100){
      ...
      dones<-c(dones,paste(paste(samp1,collapse=":"),paste(count1,collapse=":"),paste(samp3,collapse=":"),paste(samp4,collapse=":"),paste(samp5,collapse=":"),paste(samp6,collapse=":"),paste(samp7,collapse=":"),paste(samp8,collapse=":"),paste(samp9,collapse=":"),paste(samp10,collapse=":"),paste(samp11a,collapse=":"),paste(samp11b,collapse=":"),paste(samp12,collapse=":"),sep="_"),paste(samp13,collapse=":"))
  ...
  ##write to disck
  #write.csv(res7,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings06042022.csv")
  res7<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings06042022.csv",stringsAsFactors = F)
    ...
    res7b<-res7[!is.na(res7$correlation) & res7$panel2!="fungi" & res7$panel!="Emerging" ,] #& res7$panel=="RCP"
