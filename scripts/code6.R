...
for (x in 69:length(dis1)){
	...
	dis2<-res1[res1$disease==dis1[x],]
		...
		for(z in 1:nrow(dis2)){
			...
			if(length(species1)>0){species2<-unique(read.table(text=species1,sep="_",stringsAsFactors=FALSE)$V1)}else{species2<-c()} ##genus in gbif
			...
			names(mask1)<-"mask1"
			writeRaster(mask1,format="GTiff",file=paste("C:\\temp\\masks\\",paste(ttt2,collapse="_"),".tif",sep=""))
		...
		print(x)
			...
			### loop through years to create dataframe
				for(y in 1:length(years)){
					...
					k<-resample(k,clims2,method="ngb")
