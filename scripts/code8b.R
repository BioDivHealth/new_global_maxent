		...
		for(z in 1:nrow(dis2)){
			...
			### loop through years to create dataframe
				for(y in 1:length(years)){
					...
					futlh<-futlg[futlg$year==years[y] & futlg$model=="LUHa_u2t1.v1_aim.v1.1",]
					...
					disp=(spec_desp2$mean/10)[1]
					### set of lots of if but - or sort out beforehand
					fw1<-focalWeight(IP, d=c(disp, disp*2.8), "Gauss")
					#dim(fw1)
					#sum(fw1)
					fw1<-fw1*1.25/max(fw1)#sum(dim(fw1))#(1/min(fw1))#spec_dist2$Average.of.density
					#sum(fw1)
					plot(raster(fw1))	
				...
				##create data frame
				res1<-data.frame(values(clims2),values(lc),forest=values(f1),nonforest=values(nf1),water=values(water),lc_suit=values(lcsuit),
