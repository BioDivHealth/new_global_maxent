...
for (i in (i+1):length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res7<-res6[!is.na(diss_me),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_future","Hazard_delta","clim_present_mean2","clim_future_mean2")]
  ...
  rm(res6);gc()
	...
	adc_data<-res8[res8$year_RCP=="2.6_2030",]
	ad1b<-ad1[ad1$diss_me %in% adc_data$diss_me,]
	...
	expo<-rasterize(ad1b,template,"humans2010")
	...
	for (dd in 1:25){
	  ...
	  tt<-expand.grid(1:5,1:5)
	  vw<-exp(tt[dd,1])/exp(3)
	  ew<-exp(tt[dd,2])/exp(3)
	...
	##make raster layer
	ras1<-fasterize(ad1c,template,field="RiskP")  ###shouldn't need a function
	...
	res9$SAMP<-sample(c(100,500,1000,2500),1)
	res9$PAn<-res9$SAMP*sample(c(0.5,1,10,25),1)
	 
	##measure AUC
	rand1<-spsample(regnPA,res9$PAn[1],type="random")
	rand2<-extract(ras1,rand1)
	real2<-spsample(ad2,res9$SAMP[1],type="random")
  ...
  #	}
	...
	res9<-cbind(res9,thr1,thr2,thr3)
...
save(df1,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\df1b.r")
