...
for (j in sample(1:length(ad1))){
  ...
  if(paste(res9b$name[1],"_",1,"_cellids.csv",sep="") %in% list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/cellids/",full.names=FALSE)){next}
   ...
   #titls<-paste(c("HazardT","HazardL","HazardFLSSP3","Hazard Change"),round(mean(thr2[,1]),2),sep=" ")
    ...
    #for(xx in 1:length(cols1)){
     ...
     #res9b<-res9[ !is.na(res9$countr2),]
      res9b<-res9[ !is.na(res9$host_range) & !is.na(res9$surrounding)  & !is.na(res9$present2010max) & res9$HazardTmax==1 ,]
      res9b$time1="present"
      res9c<-res9[ !is.na(res9$host_range) & !is.na(res9$surrounding)  & !is.na(res9$presentfuturemax) & res9$Hazard_futureTmax==1,]
     ...
     pdf(file=paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/future_maps1/",res9b$name[1],"_",sample(1:1000,1),"all_plots.pdf",sep=""),width=8,height=8)
     # pdf(file=paste("C:\\Users\\david.redding\\Documents\\future_maps1\\",res9b$name[1],"_",sample(1:1000,1),"-all_plots.pdf",sep=""),width=16,height=8)
      ...
      plot(pl2-pl1,col=brewer.pal(3, "RdBu"),main=paste("In future - ",res9b$name[1]," ",unis[yy],sep=""),colNA="seagreen")
  ...
  write.csv(res11,file=paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/cellids/",res9b$name[1],"_",1,"_cellids.csv",sep=""))
  
}

#write.csv(res11,file="/home/ucbtdw0/Scratch/New_Global_MAXENT/res11.csv")
