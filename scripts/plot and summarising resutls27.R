...
res1<-list.files("C:\\Users\\david.redding\\Documents\\more_results5\\",pattern=".csv",full.names=TRUE)
res2<-list.files("C:\\Users\\david.redding\\Documents\\more_results5\\",pattern=".csv",full.names=FALSE)
#res2<-gsub("_ALL","",res2)
res2<-gsub(".csv","",res2)
...
res3a$model<-gsub("present","presentX0_2010XALL",res3a$model)
res3a$mer1<-paste0(res3a$diseaseALL,res3a$thresholdALL)
...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6$Tropical[res6$y>(-33) & res6$y<(33)]<-"Tropical"
  
  ##make main geography
  res6$Hemisphere="North"
  res6$Hemisphere[res6$y>(-33) & res6$y<(33)]<-"Tropical"
  
  
  ##
  ##difference from 2010
  res6$change_x<-res6$x-res6$xALL
  res6$change_y<-res6$y-res6$yALL
  
  ##make plot
  gg <- ggplot()
  gg <- gg + geom_point(data=res6[res6$AUC>0.5 & res6$RCP %in% c(6.0,0.0) & res6$Year %in% c(2070)  ,], 
                        aes(x=log(change_x), y=log(change_y), group=disease,col=y), 
                        size=1, alpha=1)
  gg <- gg+theme_cowplot()
  gg <- gg + geom_hline(yintercept = 0)
  gg <- gg + geom_vline(xintercept = 0)
  gg <-facet_wrap(~Tropical)
  #gg <- gg + xlim(-20,20)
  gg
  ...
  datPF$change_y<-datPF$y.x-datPF$y.y
  datPF$change_max_y<-datPF$max_y.x-datPF$max_y.y
  datPF$change_min_y<-datPF$min_y.x-datPF$min_y.y
  ...
  dataPF1<-datPF
  dataPF1$EDGE<-"Leading"
  dataPF2<-datPF
  dataPF2$EDGE<-"Trailing"
  
  dataPF3<-rbind(dataPF1,dataPF2)
  
  dataPF3$Location<-NA
  dataPF3$Location[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="North"]<-dataPF3$change_max_y[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="North"]
  dataPF3$Location[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="South"]<-dataPF3$change_min_y[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="South"]
  dataPF3$Location[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="North"]<-dataPF3$change_min_y[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="North"]
  dataPF3$Location[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="South"]<-dataPF3$change_max_y[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="South"]
  
    
  dataPF3$Location2<-NA
  dataPF3$Location2[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="North"]<-dataPF3$change_max_x[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="North"]
  dataPF3$Location2[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="South"]<-dataPF3$change_min_x[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="South"]
  dataPF3$Location2[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="North"]<-dataPF3$change_min_x[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="North"]
  dataPF3$Location2[dataPF3$EDGE=="Leading" & dataPF3$Pole.x=="South"]<-dataPF3$change_max_x[dataPF3$EDGE=="Trailing" & dataPF3$Pole.x=="South"]
  

  ##make plot
  gg <- ggplot(data=dataPF3[dataPF3$Disease=="Disease",])
  gg <- gg + geom_point( aes(x=(Location2), y=Location, group=disease,fill=EDGE), #
                        size=3,pch=21)
  gg <- gg + geom_line( aes(x=(Location2), y=Location, group=disease),lty=2)
  gg <- gg+theme_cowplot()
  gg <- gg + geom_hline(yintercept = 0)
  gg <- gg + geom_vline(xintercept = 0)
  gg <- gg + xlim(-5,5)
  gg <- gg + ylim(-5,5)
  gg <- gg + xlab("Change in longitude")
  gg <- gg + ylab("Change in latitude")
  #gg <- gg + scale_fill_distiller(palette = "YlOrRd")
  gg <- gg + facet_wrap(~Region.x)
...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  p<-ggplot(r6x[ !r6x$ff %in% c("2.6_3","2.6_5","2.6_2") & r6x$threshold=="ALL",],aes(y=lm2_fitted2*abs(cor2),x=Year,col=SSP)) +
  #p<-ggplot(r6x[ !r6x$ff %in% c("2.6_3","2.6_5","2.6_2") & r6x$threshold=="ALL",],aes(y=lm3_fitted2*abs(cor3),x=Year,col=SSP)) +
    #geom_histogram()+
    ...
    #stat_summary(fun.data=HDPI_ggplot, geom = "errorbar",width=0.2,position = position_dodge(3))+
  ...
  #x=1
  ###foreach
  NDIS<-c(40,80,120,160) ###check sim p value
  ...
  ##make dummy
    ...
    if(jj==4){res6x$sort<-paste0(res6x$range,res6x$group);xxx<-"Both"}
      ...
      sim_slp<-rep(0,length(unique(resN$disease)))
      sim_sd<-rep(0,length(unique(resN$disease)))
      ...
      for(i in 1:length(unique(resN$disease))){
        ...
        for (hh in 1:reps){
          ...
          #lm1<-lm(data=r3,formula="richness~Year")$coefficients
          lm2<-cor(r4$richness,r4$Year,method="spearman")[c(1,1)]
          cor1[hh]<-lm2[2]
        
           }## and of hh simulate
          
        sim_slp[i]<-length(cor1[cor1>slp[i]])/reps
      ...
      cordata1<-data.frame(disease=unique(resN$disease),correlation=slp,bootstrap=x,NDIS=NDIS[qq],ndis=length(unique(resN$disease)),panel=xxx,pval=sim_slp,sd=sim_sd)
      ...
      if(is.null(resN2)) {resN2<-resN} else {resN2<-rbind(resN2,resN)}
    ...
    res7$correlation2<-rnorm(n=rep(1,nrow(res7)),mean=rep(0,nrow(res7)),sd=res7$sd)
    res7$correlation2[res7$correlation2<(-1)]<-(-1)
    res7$correlation2[res7$correlation2>(1)]<-(1)
    ...
    res7$uni<-paste(res7$bootstrap,res7$NDIS,res7$panel)
    
    res7b<-res7[!duplicated(paste0(res7$correlation,res7$bootstrap,res7$NDIS,res7$panel)),]
    
    res7b$uni2<-1
    
    #res7b$Trend<-cut(res7b$correlation,breaks=c(-1,-0.75,-0.25,0.25,0.75,1))
    res7b$Trend<-cut(res7b$correlation,breaks=c(-1,-0.25,0.25,1))
    ...
    #res7b$Trend<-cut(res7b$correlation,breaks=c(-1,-0.75,-0.25,0.25,0.75,1))
    res7b$Trend2<-cut(res7b$correlation2,breaks=c(-1,-0.25,0.25,1))
    ...
    ##swap for mean n
    ttx<-aggregate(res7b$ndis,by=list(res7b$NDIS),mean)
    
    res7b$NDIS[res7b$NDIS==ttx[1,1]]<-round(ttx[1,2],0)
    res7b$NDIS[res7b$NDIS==ttx[2,1]]<-round(ttx[2,2],0)
    res7b$NDIS[res7b$NDIS==ttx[3,1]]<-round(ttx[3,2],0)
    res7b$NDIS[res7b$NDIS==ttx[4,1]]<-round(ttx[4,2],0)
    
    ##findout how often in categories
    res7c<-aggregate(res7b$uni2,by=list(res7b$bootstrap,res7b$NDIS,res7b$panel,res7b$Trend),sum)
    
    names(res7c)<-c("bootstrap","NDIS","Panel","Trend","Count")

    res7c$Trend<-factor(res7c$Trend, levels=c("-1 to -0.25","-0.25 to 0.25","0.25 to 1"))
    
    
    ##findout how often in categories
    res7c2<-aggregate(res7b$uni2,by=list(res7b$bootstrap,res7b$NDIS,res7b$panel,res7b$Trend2),sum)
    
    names(res7c2)<-c("bootstrap","NDIS","Panel","Trend","Count")
    
    res7c2$Trend<-factor(res7c2$Trend, levels=c("-1 to -0.25","-0.25 to 0.25","0.25 to 1"))
    
    
      
    gg<-ggplot(data=res7c,aes(x=Trend,y=Count,col=as.factor(Panel))) +
    #res7$uni<-paste(res7$SSP,res7$rep)]
    #geom_density(alpha=0.1,col=NA)+
    #ggplot(res7,aes(x=jitter(Year),y=richness,col=as.factor(SSP),group=uni)) +
    #stat_summary(fun =  mean, geom = "point",size=2) + 
    stat_summary(data=res7c2,aes(x=Trend,y=Count,group=as.factor(Panel)),col="light grey",alpha=0.75,fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    stat_summary(data=res7c2,aes(x=Trend,y=Count,group=as.factor(Panel)),col="light grey",alpha=0.75,fun.data=median_hilow, geom = "errorbar",width=0.2,position = position_dodge(3)) + 
    stat_summary(fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
    #stat_summary(fun =  function(x) quantile (x,0.45), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    #stat_summary(fun =  function(x) quantile (x,0.55), geom = "line",size=2,position = position_dodge(3),col="grey") + 
    #stat_summary(fun =  median, geom = "point",size=2,position = position_dodge(3)) + 
