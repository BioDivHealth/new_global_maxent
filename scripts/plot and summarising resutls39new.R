...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  for (ff in 1:50){
    ...
    res6x<-res6[res6$model %in% sample(mods,1),]
    ...
    resNtemp<-res6x[1:sample(c(25,50,75,100),1),]
   ...
   ##set template
    g <- as(template1, 'SpatialGrid')
    
    projection(aq)<-projection(template1)

    if(nrow(zerodist(aq))!=0){aq <- aq [-zerodist(aq)[,1],]}
    
    #ras1<-rasterize(aq,r,"richnessALLdiff",mean)
    #plot(ras1)
    
    #library(gstat)
    gs <- gstat(formula=(scale(richnessALLdiff))~1, locations=aq)
    v <- variogram(gs, width=sample(c(25,50,75,100,125),1))
    #head(v)
    #plot(v)
    warning(immediate. = FALSE)  
    
    
    fve <- fit.variogram(v, vgm(5, "Exp", 75, 5))
    
    ss<-print(warnings()[1])
    
    st1=0
    while(names(ss)==c("No convergence after 200 iterations: try different initial values?")){
      
      #print(st1)
      
      warning(immediate. = FALSE)  
      
      fve <- fit.variogram(v, vgm(sample(seq(0.1,25,0.5),1), sample(c("Exp","Sph","Gau"),1), sample(seq(1,125,5),1),sample(seq(0.1,25,0.5),1)))
      
      ss<-print(warnings()[1])
      
      #if(!names(ss) %in% c("No convergence after 200 iterations: try different initial values?")){break}
      st1=st1+1
      if(st1>99){break}
    }
    ...
    if(names(ss) %in% c("No convergence after 200 iterations: try different initial values?")){next}
    
    #plot(variogramLine(fve, 400), type='l', ylim=c(0,120))
    #points(v[,2:3], pch=20, col='red')
  
    k <- gstat(formula=(scale(richnessALLdiff))~1, locations=aq, model=fve)
    #k
    # predicted values
    kp <- predict(k, g)
    ...
    if(is.null(sum1)) {sum1=ok$prediction} else{sum1=sum1+ok$prediction}
  ...
  ggplot2()+
    geom_raster(data=sum3, aes(x=x,y=y,fill=))
