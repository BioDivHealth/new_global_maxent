...
ad1$AREA<-as.numeric(st_area(st_as_sf(ad1)))
ad1sub<-ad1@data[,c("diss_me","AREA")]
setDT(ad1sub)
setkey(ad1sub,diss_me)
...
for (i in i:length(pres1)){
  ...
  ### Vulnerability - proportion in poverty from gini * GDP -how does this then intersect with spillover rate?
    ...
    res6[,clim_present_mean2:= clim_present_mean,]### change to fit in below
     ...
     res6[,lc_suit_future_mean2:= lc_suit_future_mean,]  
  ...
  rm(res6);gc()
	...
	res8[,Exposure:=cut(scale(log((humans2010+1)),center=FALSE,scale=TRUE),breaks=10,labels=FALSE)/10]
