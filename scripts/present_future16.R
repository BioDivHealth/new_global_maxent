...
for (i in i:length(pres1)){
  ...
  res6[,year_RCP:=paste(RCP,year,sep="_")]
	...
	## final Vulnerability
	res8[,VulnerabilitySSP1:=pov_frac_SSP1]
	res8[,VulnerabilitySSP2:=pov_frac_SSP2]
	res8[,VulnerabilitySSP3:=pov_frac_SSP3]
	...
	res9<-res8[,lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","HazardUW","Hazard_future","Hazard_future_UW","RiskP","RiskSSP1","RiskSSP2","RiskSSP3","delta_riskSSP1","delta_riskSSP2","delta_riskSSP3")]
...
#}## end of i loop
  ...
  pdf(file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\risk_maps\\",pres2[i],"_",sample(1:1000,1),".pdf",sep=""),width=15,height=10)
    print(
	  ggplot(data = ad1c) +
    geom_sf(aes(fill = RiskP),col="#D3D3D320",lwd=0.01) +
