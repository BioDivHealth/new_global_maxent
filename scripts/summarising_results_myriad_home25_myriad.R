...
#ad1<-list.files("D:/cellids/",pattern="_2_cellids",full.names=TRUE)
#ad2<-list.files("D:/cellids/",pattern="_2_cellids",full.names=FALSE)
ad3<-gsub("_2_cellids.csv","",ad2)


gl1<-list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/gainlossresults/",pattern="_2.csv",full.names=TRUE)
gl2<-list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/gainlossresults/",pattern="_2.csv",full.names=FALSE)
...
for (i in 1:length(gl1)){
  ...
  glF[,orig_name:=gl3[i]]
  names(glF)[6]<-"issue_notes"
  if(class(glF)[1]=="simpleError"){ break }
  
  if(i==1) {glF2<-glF} else {glF2<-rbindlist(list(glF2,glF))}
...
#gini5<-fread(file="D:\\Users\\Public\\Documents\\GINI_by_cell1.csv")
...
#load(file="C:/Users/david.redding/Dropbox/New_Global_MAXENT/fitted_gini_50th.r")
load(file="/home/ucbtdw0/Scratch/New_Global_MAXENT/data/fitted_gini_50th.r")
...
##all future combs
#fut[,i.cell.id:=NULL]
combs2<-expand.grid(c("ssp1","ssp2","ssp3","ssp4","ssp5"),c(c("2.6_2080","4.5_2080","6_2080","8.5_2080"),c("2.6_2070","4.5_2070","6_2070","8.5_2070"),c("2.6_2050","4.5_2050","6_2050","8.5_2050"),c("2.6_2030","4.5_2030","6_2030","8.5_2030")))
...
combs3<-paste(combs2[,2],combs2[,1],sep="_")
        ...
        df3<-NULL
        j=1
        samps2<-sample(j:length(ad1),length(ad1),replace=F)
        #ls_then2<-NULL
        ...
        for (j in samps2){
          ...
          done1<-list.files("C:\\Users\\david.redding\\Documents\\more_results5\\",pattern=".csv",full.names=F)
          #done1<-list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/",pattern=".csv",full.names=F)
          
          
          done2<-read.table(text=done1,sep="_")
          #done2$uni<-paste(done2$V4,done2$V5,done2$V6,sep="_")
          #done2$uni<-done2$V4
          
          #done1<-list.files("C:\\Users\\david.redding\\Documents\\more_results2\\",pattern=".csv",full.names=F)
          #done1<-list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/",pattern=".csv",full.names=F)
         ...
         #done2$uni<-paste(done2$V4,done2$V5,done2$V6,sep="_")
          ...
          if(ad3[j] %in% done2$V4){next}
         
          #if(combs3[qq] %in% done2$uni){next}
          
          #fi1<-file.info(ad1[j])
          #if(fi1$size>1000000000 ){next}
          #z=z+1
          #print(z)}
          ...
          ###disease details
          disdetX<-strsplit(x=glF3[glF3$name==resF2$name[1],]$orig_name,split="_")
          
          ####get all rows
          disdet<-glF3[glF3$name %in% disdetX[[1]],c(1:21,47:60)]
          
          ##get all countries
          ccc<-unique(strsplit(paste(disdet$countries,collapse = ","),",")[[1]]) ##deal with mulitple ROWS ##unique
          ...
          ###remove no gini cells
          #dim(resF)
          #resF<-resF[!is.na(i.cell.id),]
          #dim(resF)
          ...
          resF[,people_in_povertySSP1:=(prop_in_pov_futureSSP1*ssp1)]
          resF[,people_in_povertySSP2:=(prop_in_pov_futureSSP2*ssp2)]
          resF[,people_in_povertySSP3:=(prop_in_pov_futureSSP3*ssp3)]
          resF[,people_in_povertySSP4:=(prop_in_pov_futureSSP4*ssp4)]
          resF[,people_in_povertySSP5:=(prop_in_pov_futureSSP5*ssp5)]
          ...
          ###
          #resF[,i.cell.id:=i.cell.id.1]
          resF[,c("SSP1","SSP2","SSP3","SSP4","SSP5","PRESENT","i.cell.id.1","i.year","prop_in_pov","prop_in_pov_futureSSP1","prop_in_pov_futureSSP2","prop_in_pov_futureSSP3","prop_in_pov_futureSSP4","prop_in_pov_futureSSP5"):=NULL]
          ##create dummy
          resF[,dummy:=1]
          
          ## non disease dummy
          if(disdet$name[1] %in% no_disease){ resF[,dummy1:=0]} else { resF[,dummy1:=1]}
          
          ##make coordinates
          resF[ ,c("x","y"):=as.data.frame(xyFromCell(template,resF$cell.id))]
          ...
          k=1;typex="ALL"
          combs2$uni<-paste(combs2$Var1,combs2$Var2,typex,sep="X")
          ...
          #dim(pres)
          #identical(pres$cell.id,pres$i.cell.id)
          #pres<-pres[!duplicated(pres$i.cell.id),]
          #dim(pres)
          
          #table(pres[is.na(pres$i.cell.id),"name"])
          
          ##split
          pres<-resF[resF$time1=="present",]
          pres[,c("cell_by_year","time1","time"):=NULL]
          pres<-pres[!is.na(i.cell.id),]
          pres<-pres[!duplicated(cell.id),]
          pres[,c("ssp1","ssp2","ssp3","ssp4","ssp5","year_RCP","year","RCP","people_in_povertySSP1","people_in_povertySSP2","people_in_povertySSP3","people_in_povertySSP4","people_in_povertySSP5"):=NULL]
          
          ##cases per year divide by population at risk
          ##these is tempered into four categories
          spill_over_rate<-sum(disdet$cases_per_year2,na.rm=TRUE)/sum(pres$humans_present,na.rm=TRUE)
          spill_over_rate2<-sum(disdet$cases_per_year,na.rm=TRUE)/sum(pres$humans_present,na.rm=TRUE)
          
          df1<-data.frame(disease=resF$name[1],model="present",richness=sum(pres$dummy),disease_richness=sum(pres$dummy1),humans=sum(pres$humans_present),cases=sum(pres$humans_present*spill_over_rate),poor_cases=sum(pres$people_in_poverty*spill_over_rate),x=mean(pres$x),y=mean(pres$y),max_x=max(pres$x),max_y=max(pres$y),min_x=min(pres$x),min_y=min(pres$y))
          
          
          ###future 
          fut<-resF[resF$time1!="present",]
          fut<-fut[!is.na(i.cell.id),]
          fut[,c("cell_by_year","time1","time"):=NULL]
          
          ##if future comb not in disease then next j
          ...
          #assign(combs2$Var2[qq],pres_spill)
         ...
         ## if(class(tryCatch(get(paste(typex,"endemic_disease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(typex,"endemic_disease",sep="_"),pres4a)} else {assign(paste(typex,"endemic_disease",sep="_"),get(paste(typex,"endemic_disease",sep="_"))+pres4a)}
          ...
          #pres4bX<-df_to_rast(ws2,pres,"people_in_poverty")
         ...
         ## if(class(tryCatch(get(paste(typex,"human_cases",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(typex,"human_cases",sep="_"),pres_spill)} else {assign(paste(typex,"human_cases",sep="_"),get(paste(typex,"human_cases",sep="_"))+pres_spill)}
          ...
          #pres_spill2<-log((pres4bX*spill_over_rate)+1)
         ...
         ## if(class(tryCatch(get(paste(typex,"human_deaths",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(typex,"human_deaths",sep="_"),pres_spill2)} else {assign(paste(typex,"human_deaths",sep="_"),get(paste(typex,"human_deaths",sep="_"))+pres_spill2)}
          ...
          #df1<-data.frame(disease=resF$name[1],model="present",richness=sum(values(pres4),na.rm=TRUE),disease_richness=sum(values(pres4a),na.rm=TRUE),humans=sum(values(pres4b),na.rm=TRUE),cases=sum(values(pres_spill),na.rm=TRUE),poor_cases=sum(values(pres_spill2),na.rm=TRUE),x=centroid1[1],y=centroid1[2],max_x=max1[1],max_y=max1[2],min_x=min1[1],min_y=min1[2])
          ...
          #for (qq in 1:nrow(combs2)){
          
          ##choose combs 
          #combs<-combs[sample(1:nrow(combs2)),]
          
          #ord1=sample(1:nrow(combs2))
          
          
          df3<-NULL
          #options(warn=2)
          ##each disease do 1 scenario
          for(qq in 1:nrow(combs2)){
            ...
            if(!combs2[qq,"Var2"] %in% fut$year_RCP){next}
          ...
          #fut4<-df_to_rast(ws2,fut[fut$year_RCP==combs2$Var2[qq],],"dummy")
          ...
          ##add in max and min
          #max2<-apply(xys2,2,max,na.rm=TRUE)
          #min2<-apply(xys2,2,min,na.rm=TRUE)
          
          #assign(combs2$Var2[qq],fut_spill)
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_endemic_richness_all",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_endemic_richness_all",sep="_"),fut4)} else {assign(paste(combs2$Var2[qq],"future_endemic_richness_all",sep="_"),get(paste(combs2$Var2[qq],"future_endemic_richness_all",sep="_"))+fut4)}
         ...
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_endemic_richness_increase",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_endemic_richness_increase",sep="_"),change1)} else {assign(paste(combs2$Var2[qq],"future_endemic_richness_increase",sep="_"),get(paste(combs2$Var2[qq],"future_endemic_richness_increase",sep="_"))+change1)}
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_endemic_richness_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_endemic_richness_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_endemic_richness_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_endemic_richness_decrease",sep="_"))+change2)}
          ##rm(change1,change2) 
          
          ###disease causing only
          #fut4a<-df_to_rast(ws2,fut[fut$year_RCP==combs2[qq,2],],"dummy1")
          
          #assign(combs2$Var2[qq],fut_spill)
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_richness_disease_all",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_richness_disease_all",sep="_"),fut4a)} else {assign(paste(combs2$Var2[qq],"future_richness_disease_all",sep="_"),get(paste(combs2$Var2[qq],"future_richness_disease_all",sep="_"))+fut4a)}
         ...
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_richness_disease_increase",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_richness_disease_increase",sep="_"),change1)} else {assign(paste(combs2$Var2[qq],"future_richness_disease_increase",sep="_"),get(paste(combs2$Var2[qq],"future_richness_disease_increase",sep="_"))+change1)}
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_richness_disease_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_richness_disease_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_richness_disease_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_richness_disease_decrease",sep="_"))+change2)}
          #rm(change1,change2) 
          
          
          ###humans  
          #fut4b<-df_to_rast(ws2,fut[fut$year_RCP==combs2[qq,2],],combs2[qq,1])
          
          
          ###humans in pov
          #fut4bX<-df_to_rast(ws2,fut[fut$year_RCP==combs2[qq,2],],paste0("people_in_poverty",toupper(combs2[qq,1])))
          
          
          #assign(combs2$Var2[qq],fut_spill)
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_humans_interactions_all",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_humans_interactions_all",sep="_"),fut4b)} else {assign(paste(combs2$Var2[qq],"future_humans_interactions_all",sep="_"),get(paste(combs2$Var2[qq],"future_humans_interactions_all",sep="_"))+fut4b)}
          ...
          #change1[is.na(change1)]<-0
          
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_humans_interactions_increase",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_humans_interactions_increase",sep="_"),change1)} else {assign(paste(combs2$Var2[qq],"future_humans_interactions_increase",sep="_"),get(paste(combs2$Var2[qq],"future_humans_interactions_increase",sep="_"))+change1)}
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_humans_interactions_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_humans_interactions_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_humans_interactions_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_humans_interactions_decrease",sep="_"))+change2)}
          #rm(change1,change2) 
          ...
          #assign(combs2$Var2[qq],fut_spill)
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_cases_all",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_cases_all",sep="_"),fut_spill)} else {assign(paste(combs2$Var2[qq],"future_human_cases_all",sep="_"),get(paste(combs2$Var2[qq],"future_human_cases_all",sep="_"))+fut_spill)}
         ...
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_cases_increase",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_cases_increase",sep="_"),change1)} else {assign(paste(combs2$Var2[qq],"future_human_cases_increase",sep="_"),get(paste(combs2$Var2[qq],"future_human_cases_increase",sep="_"))+change1)}
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_cases_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_cases_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_human_cases_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_human_cases_decrease",sep="_"))+change2)}
          #rm(change1,change2) 
          
          #fut_spill2<-log((fut4bX*spill_over_rate)+1)
          
          #assign(combs2$Var2[qq],fut_spill)
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_deaths_all",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_deaths_all",sep="_"),fut_spill2)} else {assign(paste(combs2$Var2[qq],"future_human_deaths_all",sep="_"),get(paste(combs2$Var2[qq],"future_human_deaths_all",sep="_"))+fut_spill2)}
         ...
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_deaths_increase",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_deaths_increase",sep="_"),change1)} else {assign(paste(combs2$Var2[qq],"future_human_deaths_increase",sep="_"),get(paste(combs2$Var2[qq],"future_human_deaths_increase",sep="_"))+change1)}
          
         # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"))+change2)}
          #rm(change1,change2) 
          
            fut2<-fut[fut$year_RCP==combs2$Var2[qq],]
            
            hums<-as.character(combs2[qq,1])
            
            poor<-paste0("people_in_poverty",toupper(combs2[qq,1]))
            
            ##colsums by in data.table
            
            df2<-data.frame(disease=resF$name[1],model=combs2[qq,"uni"],richness=sum(fut2$dummy),disease_richness=sum(fut2$dummy1),humans=sum(fut2[[ (1:length(fut2))[names(fut2)==hums] ]]),cases=sum(fut2[[ (1:length(fut2))[names(fut2)==hums] ]]*spill_over_rate),poor_cases=sum(fut2[[ (1:length(fut2))[names(fut2)==poor] ]]*spill_over_rate),x=mean(fut2$x),y=mean(fut2$y),max_x=max(fut2$x),max_y=max(fut2$y),min_x=min(fut2$x),min_y=min(fut2$y))
            
          
          ##make data.frame
          #df2<-data.frame(disease=resF$name[1],model=combs2[qq,"uni"],richness=sum(values(fut4),na.rm=TRUE),disease_richness=sum(values(fut4a),na.rm=TRUE),humans=sum(values(fut4b),na.rm=TRUE),cases=sum(values(fut_spill),na.rm=TRUE),poor_cases=sum(values(fut_spill2),na.rm=TRUE),x=centroid2[1],y=centroid2[2],max_x=max2[1],max_y=max2[2],min_x=min2[1],min_y=min2[2])
          
          
          ##indivdual results
          #df4<-cbind(rbind(df1,df2),disdet2)
          
          if(is.null(df3)){df3<-df2} else {df3<-rbind(df3,df2)}
          
          #write.csv(df4,file=paste("C:\\Users\\david.redding\\Documents\\more_results2\\per_disease_results_",combs2[qq,2],"_",combs2[qq,1],"_",resF$name[1],"_",typex,".csv",sep=""))
          #write.csv(df4,file=paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/per_disease_results_",combs2[qq,2],"_",combs2[qq,1],"_",resF$name[1],"_",typex,".csv",sep=""))
          
          #if(is.null(df3)){df3<-df2} else {df3<-rbind(df3,df2)}
          
          ##remove for next round
          #rm(fut4,fut4a,fut4b,fut_spill,fut_spill2,df2,df4,centroid2)
          rm(fut2,df2,hums,poor)
          gc()
          #rm(pres,fut)
          ...
          #rm(ls_then[!ls_then %in% c("df1","df3")])
          
          #gc()
          #if(qq %in% seq(from=10,to=80,by=10)) {gc()}
          
          print(qq)
        } ##end of qq loop ###################### all diseases looped
        
          ##sum scross disdet
          disdet2<-data.frame(disease_full=nnn,spill_over_rate=spill_over_rate,real_rate=spill_over_rate2,countries=paste(ccc,collapse =","),disdet[1,c("final_weighting","AUC_cutoff", "AUC","AUC_cutoffmax","AUCmax","AUC_cutoffmin","AUCmin","orig_name","Type2","dummy","cases_per_year2")],stringsAsFactors = FALSE)
          
          df1b<-cbind(df1)
          
          df4<-rbind(df1,df3)
          
          df4<-cbind(df4,disdet)
          
          write.csv(df4,file=paste("C:\\Users\\david.redding\\Documents\\more_results5\\per_disease_results_",resF$name[1],"_",typex,".csv",sep=""))
          #write.csv(df4,file=paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/per_disease_results_",combs2[qq,2],"_",combs2[qq,1],"_",resF$name[1],"_",typex,".csv",sep=""))
        ...
        ##get names
        #names1<-ls_then2[!ls_then2 %in% c("qq","ls_now","max2","min2","max1","min1","xys","xys2","ls_then","names1","pres_names","centroid2","fut_spill","fut4a","fut4b","df2","df4","fut_spill2","fut4")]
        
        ## add together
        #names2<-c(pres_names,ls()[1:15])
        
        #dfx<-NULL
        #i=1
        #for (i in i:length(names2)){
          
        #  if(paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/per_disease_results_",names2[i],"_",typex,".tif",sep="") %in% list.files("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/",full.names = TRUE)){next}
        ...
        # writeRaster(rast1,format="GTiff",file=paste("/home/ucbtdw0/Scratch/New_Global_MAXENT/more_results2/per_disease_results_",names2[i],"_",typex,".csv",sep=""))
          
         # plot(rast1,main=names2[i])
        ...
        #} ###end of i loop # saving tifs
        
        #rm(resF);gc()
        
        #dim((fut[fut$year_RCP==combs2[qq,2],"cell.id"]))
        #print("###################")
       # print("END OF LOOP")
       # print(qq)
       # print("###################")
