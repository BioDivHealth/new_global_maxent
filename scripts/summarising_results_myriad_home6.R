...
for (j in 1:length(ad1)){
  ...
  for (k in 1){
     ...
     #assign(combs$uni[qq],pres_spill)
     if(j==1){assign(paste(typex,"humans_interactions",sep="_"),pres4b)} else {assign(paste(typex,"humans_interactions",sep="_"),get(paste(typex,"humans_interactions",sep="_"))+pres4b)}
     
     pres_spill<-pres4b*log(spill_over_rate+1)
     
     #assign(combs$uni[qq],pres_spill)
     if(j==1){assign(paste(typex,"human_cases",sep="_"),pres_spill)} else {assign(paste(typex,"human_cases",sep="_"),get(paste(typex,"human_cases",sep="_"))+pres_spill)}
     
     pres_spill2<-pres4b*spill_over_rate*((disdet$CFR.high+disdet$CFR.low)/2)
     
     #assign(combs$uni[qq],pres_spill)
     if(j==1){assign(paste(typex,"deaths",sep="_"),pres_spill2)} else {assign(paste(typex,"deaths",sep="_"),get(paste(typex,"deaths",sep="_"))+pres_spill2)}
     ...
     for (qq in 1:nrow(combs)){
       ...
       if(j==1){assign(paste(combs$uni[qq],"future_endemic",sep="_"),fut4)} else {assign(paste(combs$uni[qq],"future_endemic",sep="_"),get(paste(combs$uni[qq],"future_endemic",sep="_"))+fut4)}
       
       fut4b<-df_to_rast(ws2,fut[fut$year_RCP==combs[qq,2],],combs[qq,1])
       
       #assign(combs$uni[qq],fut_spill)
       if(j==1){assign(paste(combs$uni[qq],"future_humans_interactions",sep="_"),fut4b)} else {assign(paste(combs$uni[qq],"future_humans_interactions",sep="_"),get(paste(combs$uni[qq],"future_humans_interactions",sep="_"))+fut4b)}
       
       fut_spill<-fut4b*log(spill_over_rate+1)
       
       #assign(combs$uni[qq],fut_spill)
       if(j==1){assign(paste(combs$uni[qq],"future_change_human_cases",sep="_"),fut_spill)} else {assign(paste(combs$uni[qq],"future_human_cases",sep="_"),get(paste(combs$uni[qq],"future_human_cases",sep="_"))+fut_spill)}
       
       fut_spill2<-fut4b*spill_over_rate*((disdet$CFR.high+disdet$CFR.low)/2)
       
       #assign(combs$uni[qq],fut_spill)
       if(j==1){assign(paste(combs$uni[qq],"future_deaths",sep="_"),fut_spill2)} else {assign(paste(combs$uni[qq],"future_deaths",sep="_"),get(paste(combs$uni[qq],"future_deaths",sep="_"))+fut_spill2)}
