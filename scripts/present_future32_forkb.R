...
for (i in (i+1):length(pres1)){
  ...
  #rm(res6);gc()
	...
	tt<-tt[c(4,7,8,10,12:16),]
	tt<-rbind(tt,tt[2:nrow(tt),])
	...
	for (dd in 1:nrow(tt)){
	  ...
	  ##weights
	  if(dd>9){vuln_sign=0} else {vuln_sign=0}
    if(vuln_sign==1){vuln1=vuln} else {vuln1=1-vuln}
	  mask2=mask1*((vuln1*vw)/2)+(((1-expo)*ew)/2)
	...
	SAMP<-sample(c(100,500,1000,2500),1)
  ...
  #	}
	...
	res9<-aggregate(res9[,2:ncol(res9)],by=list(res9$name),mean,na.rm=TRUE)
	
	if(is.null(res95)){res95<-res9}else{res95<-rbind(res95,res9)}
 ...
 res99$auc=round(res99$AUC_best.stat,2)
 ...
 if(i==1){res100<-res99} else {res100<-rbind(res100,res99)}
  rm(res95,res99)
...
save(df1,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\df1c.r")
save(res100,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\res100c.r")
