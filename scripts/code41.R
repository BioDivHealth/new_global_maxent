...
for (x in 5:length(dis1)){
	...
	for (j in 1:3){
	  ...
	  bothmod2<-bothmod[!is.duplicated(bothmod)]
	  if(j==1){bothmod3<-bothmod2} else {bothmod3<-c(bothmod2,bothmod3)}
	  
	}
	bothmod<-bothmod3;rm(bothmod2)
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			if( paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,".tif",sep="") %in% list.files("X:/DRtemp/resultsZ/",pattern=".tif",full.names=TRUE)){
					  ...
					  #tryCatch(IP+clims2+lcsuit+k)
					  test2<-tryCatch(IP+clims2+lcsuit+k,error=function(e) e)
					  if(class(test2)[1]=="simpleError"){fail1=TRUE} else {fail1=FALSE}
					  rm(test2)
			} else {fail1=TRUE}
			...
			if(fail1==TRUE){  
			  ...
			  #}else{   
					...
					resx<-c()
						for (i in 1:150){
								...
								print(resx[i])
								if(i>1 & resx[i]==0) {IP[!is.na(values(IP))]<-0;break}
						  ...
						  writeRaster(IP,format="GTiff",file=paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,".tif",sep=""),overwrite=TRUE)				
						  writeRaster(k,format="GTiff",file=paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"k.tif",sep=""),overwrite=TRUE)				
						  writeRaster(lcsuit,format="GTiff",file=paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"lcsuit.tif",sep=""),overwrite=TRUE)				
						  writeRaster(clims2,format="GTiff",file=paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"clims2.tif",sep=""),overwrite=TRUE)				
			    ...
			    ##if 0 realised
			    if(sum (values(IP),na.rm=TRUE)==0){next}
					...
					res1<-data.table(disease=dis3$disease,type=dis3$type,species=specX,speed=link5$speed,d=link5$d,density=link5$density,cell.id=values(template2), clim=values(clims2),lc_suit=values(lcsuit2),abundance=values(k),realised=values(IPP),i=1)
					res1<-res1[!is.na(species)]	
					names(res1)[8:11]<-paste(names(res1)[8:11],"present",sep="_")###lazy sort out!
					res1[, merge1:= paste(cell.id,type,sep="-")]
					rm(IP,k,lcsuit,lcsuit2,specX,clims2)
				...
				res3 = foreach(ii=2:nrow(link4), .combine=rbl,.packages=c("raster","data.table")) %dopar%  {
				  ...
				  if( paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,".tif",sep="") %in% list.files("X:/DRtemp/resultsZ/",pattern=".tif",full.names=TRUE)){
				    ...
				    IP<-tryCatch(raster(paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,".tif",sep="")),error=function(e) e)
				    clims2<-tryCatch(raster(paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"clims2.tif",sep="")),error=function(e) e)
				    lcsuit<-tryCatch(raster(paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"lcsuit.tif",sep="")),error=function(e) e)
				    k<-tryCatch(raster(paste("X:/DRtemp/resultsZ/",link5$species,"_",link5$code,"k.tif",sep="")),error=function(e) e)
				    
				    
				      if(class(IP)[1]=="simpleError"|class(clims2)[1]=="simpleError"|class(lcsuit)[1]=="simpleError"|class(k)[1]=="simpleError"){fail1=TRUE} else {
				    
				        test2<-tryCatch(IP+clims2+lcsuit+k,error=function(e) e) ## time expensive way to trigger
				        if(class(test2)[1]=="simpleError"){fail1=TRUE} else {fail1=FALSE}

				        rm(test2)
		  		      } 
				    
				  } else {fail1=TRUE} ##hasn't failed but using this to run and create new ones
              ...
              ##if so just start here with IP
				  ...
				  res_foreach<-data.table(species=specX,RCP=link5$RCP, year=link5$year,type=dis3$type,cell.id=values(template2), clim_future=values(clims2),lc_suit_future=values(lcsuit2),abundance_future=values(k),realised_future=values(IP),i=ii)
				  ...
				  gc()
				  rm(IP,k,lcsuit2,lcsuit,specX,clims2)
				  return(res_foreach[!is.na(species)])
    ...
    res5b<-res5[ ,.(mean(clim_future,na.rm=TRUE), mean(lc_suit_future,na.rm=TRUE), mean(abundance_future,na.rm=TRUE), mean(realised_future,na.rm=TRUE),sd(clim_future,na.rm=TRUE), sd(lc_suit_future,na.rm=TRUE), sd(abundance_future,na.rm=TRUE), sd(realised_future,na.rm=TRUE)), by = .(species,cell_by_year_by_RCP,merge1,type)]
    
    ### id standard deviation is NA replace with 0
    res5b[is.na(res5b)]<-0
    names(res5b)[5:ncol(res5b)]<-c(paste(names(res5)[6:9],"mean",sep="_"),paste(names(res5)[6:9],"sd",sep="_"))
    rm(res5)
    ...
    res4b<-res4[ ,.(mean(clim_present,na.rm=TRUE), mean(lc_suit_present,na.rm=TRUE), sum(abundance_present,na.rm=TRUE), sum(realised_present,na.rm=TRUE),mean(speed,na.rm=TRUE), mean(d,na.rm=TRUE), mean(density,na.rm=TRUE)), by = .(type,cell.id,merge1)]
    names(res4b)[4:ncol(res4b)]<-c(paste(names(res4)[c(8:11,4:6)],"mean",sep="_"))
    ...
    res5c<-res5b[ ,.(mean(clim_future_mean + (clim_future_sd*2),na.rm=TRUE), mean(clim_future_mean - (clim_future_sd*2),na.rm=TRUE), sum(realised_future_mean + (realised_future_sd*2),na.rm=TRUE), sum(realised_future_mean - (realised_future_sd*2),na.rm=TRUE)), by = .(type,cell_by_year_by_RCP,merge1)]
    names(res5c)[4:ncol(res5c)]<-c("clim_upper95","clim_lower95","realised_upper95","realised_lower95")
    rm(res5b);gc()
    ...
    res6h[,c("clim_upper95","clim_lower95","realised_upper95","realised_lower95","clim_present_mean","lc_suit_present_mean","abundance_present_mean","realised_present_mean","speed_mean_vector","d_mean_vector","density_mean_vector"),]<-1
                }
    
    #### if human vec human
    if(nrow(res6h)==0 & dis_trans$Type == "HUMAN->VECTOR->HUMAN"){res6h<-res6v;
    ...
    ### stop if not
    if(nrow(res6v)==0 | nrow(res6h)==0){break}
    ...
    res6v<-res6v[,c("cell_by_year_by_RCP","clim_present_mean","lc_suit_present_mean","abundance_present_mean","realised_present_mean","clim_upper95","clim_lower95","realised_upper95","realised_lower95","speed_mean","d_mean","density_mean"),with=FALSE]
    ...
    for(qq in 1:5){
      ...
      humans=(res6[,qq+31,with=FALSE])/5.6
    
      ##homegenous mixing with gas model parameters
      ##2 standard deviations each way
      
      res6[,present_realised:= c((realised_present_mean*speed_mean*d_mean) * (realised_present_mean_vector*speed_mean_vector*d_mean_vector) *(secondary_present) * (humans * 0.0001 * 5)),]
      res6[,future_realised_upper:= c((realised_upper95*speed_mean*d_mean) * (realised_upper95_vector*speed_mean_vector*d_mean_vector) *(secondary_future) * (humans * 0.0001 * 5)),]
      res6[,future_realised_change_upper:= future_realised_upper-present_realised,]
      #res6[,future_realised_change_upper_div:= present_realised/future_realised_upper,]
                
      res6[,future_realised_lower:= c((realised_lower95*speed_mean*d_mean) * (realised_lower95_vector*speed_mean_vector*d_mean_vector) *(secondary_future) * (humans * 0.0001 * 5)),]
      res6[,future_realised_change_lower:= future_realised_lower-present_realised,]
      #res6[,future_realised_change_lower_div:= present_realised/future_realised_lower,]
      ...
      ##remove infinite - no comparison possible.
      #res6[is.infinite(res6)]<-NA
    
      res6<-res6[,names(res6)[!names(res6) %in% c("cell_by_year_by_RCP","cell_by_year","merge1","time","id","cell.id.2","i.id","i.ID","i.cell.id.2","i.time","i.time","i.type","i.cell.id","i.cell.id.1") ], with = FALSE ]
    
      #use gini to work out % in poverty assuming a parabolic shape
      #gini<-fread("X:/DRtemp/ssp_gini_countries.csv",header=TRUE)
      #setkey(gini,UN)
      #setkey(res6,ec)
      #res6<-res6[gini,nomatch=0]

      res6[,ssp:=paste("ssp",qq,sep="_")]
        
      #if(qq==1){res7<-res6[,c("RCP","year","cell.id","ssp","present_realised","future_realised_upper","future_realised_lower","future_realised_clim_upper","future_realised_clim_upper_div","future_realised_clim_lower","future_realised_clim_lower_div","future_realised_change_upper","future_realised_change_lower","future_realised_change_lower_div")]} else {res7<-rbl(res7,res6[,c("RCP","year","cell.id","ssp","present_realised","future_realised_upper","future_realised_lower","future_realised_clim_upper","future_realised_clim_upper_div","future_realised_clim_lower","future_realised_clim_lower_div","future_realised_change_upper","future_realised_change_lower","future_realised_change_lower_div")])}
      if(qq==1){res7<-res6[,c("RCP","year","cell.id","ssp","present_realised","future_realised_upper","future_realised_lower","future_realised_clim_upper","future_realised_clim_lower","future_realised_change_upper","future_realised_change_lower")]} else {res7<-rbl(res7,res6[,c("RCP","year","cell.id","ssp","present_realised","future_realised_upper","future_realised_lower","future_realised_clim_upper","future_realised_clim_lower","future_realised_change_upper","future_realised_change_lower")])}
          
      }#end of qq
    
    #save(res7,file=paste("X:/DRtemp/per_disease/",dis3$disease,"full.r"))
    
    #summarise by year and RCP and ssp
    #currently means need median and sd ===== CHECK
    ...
    res8<-res7[,.(sum(present_realised,na.rm=TRUE),sum(future_realised_upper,na.rm=TRUE),sum(future_realised_lower,na.rm=TRUE),mean(future_realised_clim_upper,na.rm=TRUE),mean(future_realised_clim_lower,na.rm=TRUE),sum(future_realised_change_upper,na.rm=TRUE),sum(future_realised_change_lower,na.rm=TRUE)),.(year,RCP,ssp)]
    names(res8)[4:ncol(res8)]<-c("present_realised","future_realised_upper","future_realised_lower","future_realised_clim_upper","future_realised_clim_lower","future_realised_change_upper","future_realised_change_lower")
    rm(res7)
    
    ##add in disease data
    res9<-cbind(res8,dis_trans)
    rm(res8)
    save(res9,file=paste("X:/DRtemp/per_disease/",dis3$disease,"summary.r"))
    
    if(x==1){res10<-res9} else {res10<-rbind(res10,res9)}
    rm(res9)
