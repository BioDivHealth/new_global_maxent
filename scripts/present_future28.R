...
for (i in (i+1):length(pres1)){
  ...
  rm(res6);gc()
	...
	##make raster layer
	ras1<-fasterize(ad1c,template,field="Hazard")  ###shouldn't need a function
	...
	res9$PAn=SAMP*sample(c(0.5,1,10,25),1)
	 
	##measure AUC
	rand1<-spsample(regnPA,PAn[1],type="random")
	rand2<-extract(ras1,rand1)
	real2<-spsample(ad2,SAMP[1],type="random")
	...
	ad1c$HazardT<-ad1c$Hazard
	ad1c$HazardT[ad1c$HazardT>thr1[[2]]]<-1
	ad1c$HazardT[ad1c$HazardT!=1]<-0
	
	##make thresholded version of risk
	res9$HazardT<-res9$Hazard
	res9$HazardT[res9$HazardT>=thr1[[2]]]<-1
	res9$HazardT[res9$HazardT!=1]<-0
	...
	###future risk
	res9[,RiskSSP1:= HazardT * (1+VulnerabilitySSP1) * (1+ExposureSSP1)]
	res9[,RiskSSP2:= HazardT * (1+VulnerabilitySSP2) * (1+ExposureSSP2)]
	res9[,RiskSSP3:= HazardT * (1+VulnerabilitySSP3) * (1+ExposureSSP3)]
  ...
  jpeg(file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\risk_maps\\",pres2[i],"_",sample(1:1000,1),"Hazard.jpg",sep=""),width=2500,height=1000)
	...
	print(
	  ggplot(data = ad1c) +
	    geom_sf(aes(fill = HazardT),colour=NA) +
	    scale_fill_viridis_c(option = "plasma")+
