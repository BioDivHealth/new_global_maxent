...
pres1<-list.files("V:\\per_disease2\\",pattern=" present",full.names=TRUE)
pres2<-list.files("V:\\per_disease2\\",pattern=" present",full.names=FALSE)
  ...
  for (xx in 1:2){
    ...
    for (z  in 1:5){
      ...
      if(nrow(pd)>0){
        ...
        if(is.null(res1)){res1<-data.frame(disease=pres2[i],AUC=e@auc,thr,z,lcsuit,AUC_PD=epd@auc,thrpd,p.data)}else{res1<-rbind(res1,data.frame(disease=pres2[i],AUC=e@auc,thr,z,lcsuit,AUC_PD=epd@auc,thrpd,p.data))}
...
res1b<-aggregate(res1[,c(2:8,11:17)],by=list(res1$disease,res1$lcsuit,res1$p.data),mean)
names(res1b)[1:3]<-c("disease","lcsuit","p.data")
