...
load("C:\\temp\\livestock_future2.r")
	...
	dis2<-res1[res1$disease==dis1[x],]
		...
		for(z in 1:nrow(dis2)){
			...
			link4$density[link4$density>3000000]<-3000000
				...
				if(nrow(link4)>25){yyy<-c(1,sample(2:nrow(link4),24,replace=FALSE))}else{yyy<-1:nrow(link4)}
				...
				if(y==1){res1<-data.table(disease=dis3$disease,type=dis3$type,species=link5$species,speed=link5$speed,d=link5$d,density=link5$density,speed=link5$speed, clim_present=values(clims2),lc_suit_present=values(lcsuit2),abundance_present=values(k),realised_present=values(IP),values(futpop2),values(gdppop2));res1<-res1[!is.na(clim_present)];next}
				gc()
				#res2<-data.table(year=link5$year,RCP=link5$RCP,model=link5$model,code=link5$code, clim_future=values(clims2),lc_suit_future=values(lcsuit),abundance_future=values(k),realised_future=values(IP),values(futpop2),values(gdppop2))
				#gc()
				if(y==2){res3<-data.table(year=link5$year,RCP=link5$RCP,model=link5$model,code=link5$code, clim_future=values(clims2),lc_suit_future=values(lcsuit),abundance_future=values(k),realised_future=values(IP),values(futpop2),values(gdppop2))}else{res3<-rbindlist(list(res3,data.table(year=link5$year,RCP=link5$RCP,model=link5$model,code=link5$code, clim_future=values(clims2),lc_suit_future=values(lcsuit),abundance_future=values(k),realised_future=values(IP),values(futpop2),values(gdppop2))))}
				res3<-res3[!is.na(clim_future)]
				gc()
