...
for (i in i:length(pres1)){
  ...
  ###get popGDP
  sp1<-spatialPoints(res6[,c("x","y")])
  popGDP2<-extract(popGDP,sp1)
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res7<-res6[!is.na(sub_region),lapply(.SD,sum,na.rm=TRUE),by=.(year_RCP,diss_me),.SDcols=c("Hazard","HazardUW","Hazard_future","Hazard_future_UW")]
	setkey(res7,diss_me)
  ...
  ##facet plot by RCP_year
	ggplot(ad1bDF,aes(x=long,y=lat,group=group.x,fill=delta_risk))+
			geom_polygon()+
	  facet_wrap(~year_RCP)
