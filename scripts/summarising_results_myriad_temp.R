...
gini<-stack("C:\\Users\\Public\\Documents\\GINI_STACK1.tif")
names(gini)<-read.csv("C:\\Users\\Public\\Documents\\GINI_STACK_NAMES1.csv")$x

gini3<-extract(gini,xyFromCell(template,1:ncell(template)))
gc()
fwrite(gini3,file="C:\\Users\\Public\\Documents\\GINI_STACK1.csv")
names(gini3)<-read.csv("C:\\Users\\Public\\Documents\\GINI_STACK_NAMES1.csv")$x
rm(gini3);gc()
gini4<-fread(file="C:\\Users\\Public\\Documents\\GINI_STACK1.csv")
...
##dcast across SSPs
gini6<-dcast(gini5,cell.id+year+cell_by_year~SSP,value.var="value")

##split across present future
gini6a<-gini6[year==2010,]
gini6a<-gini6a[!duplicated(cell.id),]
gini6a[,c("year","SSP1","SSP2","SSP3","SSP4","SSP5","cell_by_year"):=NULL]

#g6<-gini6a
setkey(gini6a,cell.id)
gc()
gini6b<-gini6[year!=2010,]
gini6b[,c("PRESENT"):=NULL]
setkey(gini6b,cell.id)
gc()

gini7<-gini6a[gini6b]
gc()
#gini7[,future_value2:=NULL]
fwrite(gini7,file="C:\\Users\\Public\\Documents\\GINI_by_cell1.csv")

gini5<-fread(file="C:\\Users\\Public\\Documents\\GINI_by_cell.csv")
