...
###find all present day
pres1c<-list.files("V:\\per_disease2\\",pattern=" ALL3",full.names=TRUE)
pres2c<-list.files("V:\\per_disease2\\",pattern=" ALL3",full.names=FALSE)
pres2c<-gsub("  ALL3.r","",pres2c)
pres2c<-gsub(" ALL3.r","",pres2c)
...
for (i in (i+1):length(pres1)){
  ...
  #population gdp
	...
	thresh<-quantile((res8$gdp2010*1000)/res8$humans2010,0.10,na.rm=TRUE)
	...
	res7<-res8[!is.na(diss_me),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_future","HazardT","Hazard_futureT","clim_present_mean2","clim_future_mean2","Exposure","Vulnerability","ExposureSSP1","ExposureSSP2","ExposureSSP3","VulnerabilitySSP1","VulnerabilitySSP2","VulnerabilitySSP3")]
  rm(res8);gc()
  ...
  jpeg(file=paste("C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\risk_maps\\",pres2[i],"_",sample(1:1000,1),"HazardT2.jpg",sep=""),width=2500,height=1000)
  print(
    ggplot(data = ad1c) +
...
##save results
fwrite(res11,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\final_risk5c.csv")
