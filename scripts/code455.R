  ...
  if(x==3){next}
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			##spec_hab ok
			if(nrow(spec_desp2)==0|is.na(spec_desp2$mean)){print("no dispersal data");spec_desp2<-data.frame(binom=NA,min=mean(spec_desp$min,trim=0.1,na.rm=TRUE),max=mean(spec_desp$max,trim=0.1,na.rm=TRUE),mean=mean(spec_desp$mean,trim=0.1,na.rm=TRUE),min_mean=mean(spec_desp$min_mean,trim=0.1,na.rm=TRUE),max_mean=mean(spec_desp$max_mean,trim=0.1,na.rm=TRUE),stringsAsFactors = FALSE)}
