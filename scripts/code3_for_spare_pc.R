...
setwd('E:\\Dropbox\\New_Global_MAXENT\\')
...
##models
sdm1<-list.files("E:/disease_analyses3/", pattern="maxent",full.names=TRUE)
sdm2<-list.files("E:/disease_analyses3/", pattern="maxent",full.names=FALSE)
...
spdata<-stack("E://predictorsX.tif")
names(spdata)<-read.csv("E://names1.csv",stringsAsFactors=FALSE)$x
spdata<-subset(spdata,c(8,10:length(names(spdata))))
Altitude<-subset(spdata,1)
#names(Altitude)<-"Altitude"

#clims<-clims[sample(1:nrow(clims),nrow(clims),replace=FALSE),]

reps2<-unique(clims$rep)
reps2<-reps2[sample(1:length(reps2),length(reps2),replace=FALSE)]
reps2<-c("present",reps2)
...
for (x in 1:length(reps2)){
	...
	kkk=1:length(sdm1)#sample(1:length(sdm1),length(sdm1),replace=FALSE)
	
	for(k in kkk){

		load(sdm1[k])

		if(paste(sdm2[k],"_",reps2[x],"_XXX.r",sep="") %in% list.files("C://resultsX/",pattern="XXX.r",full.names=FALSE)){next}
		print("no")
	...
	writeRaster(fin_pred,method="GTiff",file=paste("C://resultsY/",sdm2[k],"_",reps2[x],"_XXX.tif",sep=""),overwrite=TRUE)
	tt<-NULL; save(tt,file=paste("C://resultsX/",sdm2[k],"_",reps2[x],"_XXX.r",sep=""))
	#gc();removeTmpFiles(1)
	}
}
