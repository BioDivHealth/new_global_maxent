	...
	dis2<-res1[res1$disease==dis1[x],]
		...
		for(z in 1:nrow(dis2)){
			...
			link4$density[link4$density>3000000]<-3000000
				...
				if(y==1){res1<-data.table(disease=dis3$disease,type=dis3$type,species=link5$species,speed=link5$speed,d=link5$d,density=link5$density,speed=link5$speed, clim_present=values(clims2),lc_suit_present=values(lcsuit2),abundance_present=values(k),realised_present=values(IP));res1<-res1[!is.na(clim_present)];next}
				gc()
				...
				#gc()
				if(y==2){res3<-data.table(clim_future=values(clims2),lc_suit_future=values(lcsuit),abundance_future=values(k),realised_future=values(IP))}
				res3<-res3[!is.na(clim_future)]
				res1<-cbind2(res1,res3)
				names(res1)[ncol(res1)
				gc()
