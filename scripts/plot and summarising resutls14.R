...
for (j in 1:length(zz)){
  ...
  res5z2<-res5z[res5z$richness>quantile(res5z$richness,0.05) & res5z$richness<quantile(res5z$richness,0.95),]
  
  #dcast(res3,MinMax+RCP+Year+disease+pres_size~Type,value.var=c("Gain","change"))#rbind(res4,res3)
...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6x<-res6[ res6$AUCmax>0.6 & res6$disease.y!="None reported"  ,]# res6$Vectored==0 & 
