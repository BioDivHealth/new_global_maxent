...
for (i in (nums)){
  ...
  ###determine if in regn and regn2
	...
	res6b<-res6[!is.na(res6$sub_region),]
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	if(nrow(res6a)==0){res6a<-res6[res6$year==2050 & res6$RCP==4.5 & !is.na(res6$regionPA),]}
	if(nrow(res6a)==0){res6a<-res6[res6$year==2050 & res6$RCP==6 & !is.na(res6$regionPA),]}
	if(nrow(res6a)==0){res6a<-res6[res6$year==2050 & res6$RCP==8.5 & !is.na(res6$regionPA),]}
	noabs=0
	##threshold again
	for (uu in 1:20){
	  ...
	  ##random absence points
	  rand2=res6a$Hazard[sample((1:nrow(res6a))[is.na(res6a$host_range2)],10000,replace=TRUE)]
	  real3=res6a$Hazard[sample((1:nrow(res6a))[!is.na(res6a$host_range2)],10000,replace=TRUE)]
	  
	  ##comparison by country/admin areas
	  res6d<-rbind(data.frame(ea2=rep("0",length(rand2)),value=rand2),data.frame(ea2=rep("1",length(real3)),value=real3))
	  res6d<-res6d[!is.na(res6d$value),]
	  
	  thr1x<-Find.Optim.Stat(Stat="ROC",Fit=res6d$value,Obs=res6d$ea2)
	  colnames(thr1x)<-paste("AUC_",colnames(thr1x),sep="")
	  
	  if(uu==1){thr2x=thr1x} else {thr2x<-rbind(thr2x,thr1x)}
	  ...
	  ##random absence points
	  rand2=res6a$HazardD[sample((1:nrow(res6a))[is.na(res6a$countr)],10000,replace=TRUE)]
	  real3=res6a$HazardD[sample((1:nrow(res6a))[!is.na(res6a$countr)],10000,replace=TRUE)]
	  
	  ##comparison by country/admin areas
	  res6d<-rbind(data.frame(ea2=rep("0",length(rand2)),value=rand2),data.frame(ea2=rep("1",length(real3)),value=real3))
	  res6d<-res6d[!is.na(res6d$value),]
	  
	  thr1y<-Find.Optim.Stat(Stat="ROC",Fit=res6d$value,Obs=res6d$ea2)
	  colnames(thr1y)<-paste("AUC_",colnames(thr1),sep="")
	  
	  if(uu==1){thr2y=thr1y} else {thr2y<-rbind(thr2y,thr1y)}
	  ...
	  ##random absence points
	  rand2=res6a$HazardD[sample((1:nrow(res6a))[is.na(res6a$host_range2)],10000,replace=TRUE)]
	  real3=res6a$HazardD[sample((1:nrow(res6a))[!is.na(res6a$host_range2)],10000,replace=TRUE)]
	  
	  ##comparison by country/admin areas
	  res6d<-rbind(data.frame(ea2=rep("0",length(rand2)),value=rand2),data.frame(ea2=rep("1",length(real3)),value=real3))
	  res6d<-res6d[!is.na(res6d$value),]
	  
	  thr1xy<-Find.Optim.Stat(Stat="ROC",Fit=res6d$value,Obs=res6d$ea2)
	  colnames(thr1xy)<-paste("AUC_",colnames(thr1xy),sep="")
	  
	  if(uu==1){thr2xy=thr1xy} else {thr2xy<-rbind(thr2xy,thr1xy)}
	  ...
	  if(nrow(pd)>0){
	    pl1<-ws2
	    pl1[pl1==1]<-0
	    pl1[res6a$cell.id]<-res6a$HazardD
	    pl1<-crop(pl1,extent( min(res6a$x), max(res6a$x),min(res6a$y), max(res6a$y)))
	    real1<-extract(pl1,pd)
	    not_real<-extract(pl1,randomPoints(pl1,10000))
	    ...
	    thr1wy<-Find.Optim.Stat(Stat="ROC",Fit=res6d$value,Obs=res6d$ea2)
	    colnames(thr1wy)<-paste("AUC_",colnames(thr1wy),sep="")
	    
	    if(uu==1){thr2wy=thr1wy} else {thr2wy<-rbind(thr2wy,thr1wy)}
	  }
	...
	##set cut off
	AUC_cutoff<-0.1;AUC_cutoff2<-0.1;AUC_cutoff_real<-0.1;AUC_cutoff_real<-NA
	if(noabs!=1){AUC_cutoff<-mean(thr2[,2],na.rm=TRUE);AUC_cutoff2<-mean(thr2x[,2],na.rm=TRUE)}
	
	if(noabs!=1 & nrow(pd)>0){AUC_cutoff_real<-mean(thr2w[,2],na.rm=TRUE)}
	
	##put in main data frame
	res6$AUC_cutoff<-AUC_cutoff
	res6$AUC_cutoff2<-AUC_cutoff2
	res6$AUC_cutoff_real<-AUC_cutoff_real
	
	##hazardD
	
	AUC_cutoffD<-0.1;AUC_cutoff2D<-0.1;AUC_cutoff_realD<-0.1;AUC_cutoff_realD<-NA 
	if(noabs!=1){AUC_cutoffD<-mean(thr2y[,2],na.rm=TRUE);AUC_cutoff2D<-mean(thr2xy[,2],na.rm=TRUE)}
	
	if(noabs!=1 & nrow(pd)>0){AUC_cutoff_real<-mean(thr2wy[,2],na.rm=TRUE)}
	
	##put in main data frame
	res6$AUC_cutoffD<-AUC_cutoffD
	res6$AUC_cutoff2D<-AUC_cutoff2D
	res6$AUC_cutoff_realD<-AUC_cutoff_realD
	...
	res6[,HazardTD:=ifelse(HazardD>=AUC_cutoffD,1,0)]
	res6[,Hazard_futureTD:=ifelse(Hazard_futureD>=AUC_cutoffD,1,0)]
  ...
  # risk calculate	
	...
	res8[,RiskL:=HazardD*log(Exposure+1)]
	...
	fwrite(res8,file=paste("C:\\Users\\xxxx\\Documents\\datasets1\\",nam_dis,"_",sample(1:1000,1),"_all_data.csv",sep=""))
	...
	titls<-paste(cols1,round(mean(thr2[,1]),2),sep=" ")
	

	jpeg(file=paste("C:\\Users\\xxxx\\Documents\\risk_maps2\\",nam_dis,"_",sample(1:1000,1),"all_plots.jpg",sep=""),width=2500,height=2500)
	par(mfrow=c(3,3))
	for (zz in 1:9){
	...
	pl1[res9$cell.id]<-res9[,names(res9)[names(res9)==cols1[zz]],drop=TRUE]
	...
	plot(pl1,main=titls[zz],font=32,cex.main=3)
	#points(xyFromCell(template,unique(res6$cell.id)),col="red",pch=20)
	plot(countr,border="red",add=TRUE)
