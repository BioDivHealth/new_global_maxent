...
for (i in (i+1):length(pres1)){
  ...
  ##what does this mean?
	...
	res8[,RiskP:= scale(Hazard) + (vw*scale(Vulnerability)) + (ew*scale(Exposure))]
	res8[,RiskSSP1:= scale(Hazard) + (vw*scale(VulnerabilitySSP1))* (ew*scale(ExposureSSP1))]
	res8[,RiskSSP2:= scale(Hazard) + (vw*scale(VulnerabilitySSP2))* (ew*scale(ExposureSSP2))]
	res8[,RiskSSP3:= scale(Hazard) + (vw*scale(VulnerabilitySSP3))* (ew*scale(ExposureSSP3))]
...
ggplot(data=resll,aes(x=year,y=RiskSSP2/RiskP,col=group))+
  geom_ribbon()
