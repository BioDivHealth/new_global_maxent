...
for (x in 1:length(dis1)){
    ...
    #system.time(
        ...
        #system.time(
				...
				if(y==1){res1<-data.table(disease=dis3$disease,type=dis3$type,species=specX,speed=link5$speed,d=link5$d,density=link5$density,cell.id=values(template2), clim_present=values(clims2),lc_suit_present=values(lcsuit2),abundance_present=values(k),realised_present=values(IP));res1<-res1[!is.na(species)];next}
				gc()
    ...
    ### need to summarise vector values
    ###should be sum for number
    ### only need to do the summary across models for species
    ### two levels?? species summary by year and model and cell
    ### then summaries ofhosts and vector
    ...
    ##not models remove the effect of species
    ## need to include all movement parameters in present
    ## present should be cells x type
    ## don't want to aggregate over type
    ## need to include standard deviation
    ## what about mapping underlying species level patterns rather than disease level
  
    ## REMOVE EFFECT OF SPECIES
    ##present day and gas models params summarised by vectors
    ...
    names(res4b)[3:ncol(res4b)]<-c(paste(names(res4)[c(8:11,4:6)],"mean",sep="_"))
    res4b[,merge1:=paste(cell.id,type,sep="_")]
    
    ##future by type cellby RCP 
    ## sum has to sum * variation to pass on variance in abundance <-----------------DO THIS
    ## what about variation across species + mutliplying probabitiles
    res5c<-res5b[ ,.(mean(clim_future_mean + clim_future_sd,na.rm=TRUE), mean(clim_future_mean - clim_future_sd,na.rm=TRUE), sum(abundance_future_mean + abundance_future_sd,na.rm=TRUE), sum(abundance_future_mean - abundance_future_sd,na.rm=TRUE)), by = .(type,cell_by_year_by_RCP)]
    names(res5c)[3:ncol(res5c)]<-c("clim_upper95","clim_lower95","abund_upper95","abund_lower95")
    ...
    ###turn both data.frames into wide??? <- no just do as before
    ###but would be years * RCP wide * length(min,max,mean)
    ####go to wide via type - type and
    ####
    
    ##merge present and future
    ### merge1 is now wrong
    ### <------------------------ remove redundant columns from 4b only two though
    setkey(res4b,merge1)
    setkey(res5c,merge1)
    ...
    res6v<-res6x[res6x$type=="vectors",c("cell_by_year_by_RCP","clim_upper95","clim_lower95","abund_upper95","abund_lower95","clim_present_mean","lc_suit_present_mean","abundance_present_mean","realised_present_mean"),with=FALSE]        
    
    if(nrow(res6v==0)){}#### if true absence - generate dataframe with same RCP year combinations but all zeros
    ...
    ###if human hosts turn vectors into hosts
    ### but what about human+secondary->vector->human
    if(dis_trans$Type == "HUMAN->VECTOR->HUMAN"){res6$type[res6$type!="second"]<-"hosts"}
    ###if host vect and no hosts then no hosts make vect hosts
    if(dis_trans$Type %in% Vect & !("hosts" %in% unique(res6$type))){res6$type[res6$type!="second"]<-"hosts"}
    ...
    res6<-res6[,which( !duplicated( t( res6 ) ) ), with = FALSE ]
    ##how to bring hosts and vectors together? switch to wide format? <---- SORT OUT!!!
    gc()
    
    #testz<-dis2[,1:11]
    #testx<-diseases[diseases$disease=="borna",][,]
    #uniq<-unique(diseases[diseases$type=="second","disease"])
    #testy<-diseases[diseases$disease=="wuchereria bancrofti",][1:2,]
    #dis2<-rbind(testz[1,],testx[1,],testy[1,])
    ...
    #### find top and bottom possible values??
    #### aggregate by model to mean and sd of abundance...
    if("vectors" %in% unique(res6$type)){
      
      
      res6v<-res6[res6$type=="vectors"]
      res6h<-res6[res6$type=="hosts"]      
      
      ####summarise all the dataframe by cell.id?? including movement parameters? inlcuding RCP(no?)? lose of lots detail does it matter?
      
      ##one by cell.id by year dataframe of vectors and hosts 
      ##cbind them/join? plus variation
      ##input variation plus parameter variation - just once in the gas model (cheat?)
      ##
      
      res6v2<-res6[ ,.(mean(clim_future,na.rm=TRUE), mean(lc_suit_future,na.rm=TRUE), mean(abundance_future,na.rm=TRUE), mean(realised_future,na.rm=TRUE),mean(clim_present,na.rm=TRUE), mean(lc_suit_present,na.rm=TRUE), mean(abundance_present,na.rm=TRUE), mean(realised_present,na.rm=TRUE)), by = .(type,RCP,cell_by_year)]
      
    
    ### CAN YOU CREATE STANDARD DATAFRAMES WITH 1 FOR SECONDARY, VECTOR (+parameters) WHEN NOT PRESENT?? MEMORY COST??
    ...
    ##if vectors doesn't exist then make vectors hosts or just treat differently with several ifs and summaries??
    ##same with secondary   ``
    ##if hosts are humans add in
    
    
    ##gas interactions with host -> vector at species level ### ok to use 1s?? or specify vectors as hosts in the if statement
    ##gas interactions with  -> vector at species level
      
    ## save all of the these and then write new models to simulate and
      
      }####### END X loop???????
      
    
    ## choose best parameters with country AIC and the predict future with them???? <---------------  VALIDATION REMOVE
    
    ##wrap around with samples with the same labels to visualise variations - sampling?
