 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  present2<-NULL
    for (j in 1:length(kk) ){
      ...
      if(is.null(present2)) {present2<-present1;future2<-future1;dis_trans2<-dis_trans} else {present2<-present2+present1;future2<-future2+future1;dis_trans2<-rbind(dis_trans2,dis_trans)} 
