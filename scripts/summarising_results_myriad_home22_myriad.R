...
for(qq in ord1){
 ...
 if(combs3[qq] %in% done2$uni){next}
  ...
  for (j in j:length(ad1)){
    ...
    ###
    resF[,c("SSP1","SSP2","SSP3","SSP4","SSP5","PRESENT","i.cell.id.1","i.year","prop_in_pov","prop_in_pov_futureSSP1","prop_in_pov_futureSSP2","prop_in_pov_futureSSP3","prop_in_pov_futureSSP4","prop_in_pov_futureSSP5","i.cell.id"):=NULL]
    ##create dummy
    resF[,dummy:=1]
   ...
   # if(class(tryCatch(get(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_")), error=function(err2) err2))[1]=="simpleError"){assign(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"),change2)} else {assign(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"),get(paste(combs2$Var2[qq],"future_human_deaths_decrease",sep="_"))+change2)}
    ...
    ##make data.frame
    df2<-data.frame(disease=resF$name[1],model=combs2[qq,"uni"],richness=sum(values(fut4),na.rm=TRUE),disease_richness=sum(values(fut4a),na.rm=TRUE),humans=sum(values(fut4b),na.rm=TRUE),cases=sum(values(fut_spill),na.rm=TRUE),deaths=sum(values(fut_spill2),na.rm=TRUE),x=centroid2[1],y=centroid2[2],max_x=max2[1],max_y=max2[2],min_x=min2[1],min_y=min2[2])
