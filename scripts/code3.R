...
##models
sdm1<-list.files("./disease_analyses3/", pattern="maxent",full.names=TRUE)
sdm2<-list.files("./disease_analyses3/", pattern="maxent",full.names=FALSE)
sdm2<-gsub("_maxent.r","",sdm2,fixed=TRUE)


##ciimates ##CMIP5 projections
fut1<-list.files("./future/",recursive=TRUE,pattern="w001001.adf",full.names=FALSE)
...
for (x in 1:length(reps2)){
	...
	clims2<-stack(paste("./future/",clims[clims$rep==reps2[x],"filen"],sep=""))
	names(clims2)<-clims[clims$rep==reps2[x],"name"]
	predictors2<-stack(clims2,Altitude)
	}

	kkk=sample(1:length(sdm1),length(sdm1),replace=FALSE)
	
	for(k in kkk){

		load(sdm1[k])

		if(paste(sdm2[k],"_",reps2[x],"_XXX.r",sep="") %in% list.files("./resultsX/",pattern="XXX.r",full.names=FALSE)){next}
		print("no")
	...
	writeRaster(fin_pred,method="GTiff",file=paste("./resultsY/",sdm2[k],"_",reps2[x],"_XXX.tif",sep=""),overwrite=TRUE)
	tt<-NULL; save(tt,file=paste("./resultsX/",sdm2[k],"_",reps2[x],"_XXX.r",sep=""))
	#gc();removeTmpFiles(1)
	}
}
