...
res3c<-cbind(res3b,res3b2[c(1,3)],res3b3)

##
#res3z<-cbind(res3b[,2:3],(res3c[,4:14])-res3c[,31:41],res3b3,res3b2[c(1,3)],res3c[,15:30])

res3z<-res3c[res3c$AUC>0.5 ,]#
#"dengue"

#res3z$SSP[ res3z$Year==2010]<-"ssp3"

slp<-rep(0,length(unique(res3z$disease)))

for(i in 1:length(unique(res3z$disease))){

  dis1<-unique(res3z$disease)[i]

  r3<-res3z[res3z$disease==dis1  & res3z$SSP=="ssp3",]
...
ggplot(r3,aes(x=jitter(Year),y=richness,col=as.factor(RCP))) +
...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  ##remove
  res6x<-res6[ res6$AUCmax.x>0.55 & res6$disease.y!="None reported"  ,]# res6$Vectored==0 & 

  
  hist(res6x$richness,breaks=50)  
  ##
  ...
  for (x in 1:100){
    ...
    if(zzz==1){
      ...
      #countries
        countr1<-wrld_simpl[wrld_simpl$NAME!="Antartica" & wrld_simpl$ISO2 %in% strsplit(cur1$countries,",")[[1]],]
        
      #regions
        reg1<-wrld_simpl[wrld_simpl$REGION %in% countr1$REGION,]
