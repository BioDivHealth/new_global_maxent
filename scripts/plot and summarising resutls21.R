...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  types<-c("Baseline","Accuracy","Regional","Vectored","RCP","SSP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","RCP/SSP")
  ...
  res6x<-res6[res6$AUC>samp1 & res6$count1!=0 & res6$Vectored %in% samp3 & res6$RCP %in% samp4 & res6$SSP %in% samp5 & res6$spillover_rate2 %in% samp6 & res6$disease.y!=samp7 & res6$group2!=samp8 & res6$Kingdom %in% samp9 & res6$Tropical %in% samp10 & res6$RCP %in% samp11a & res6$SSP %in% samp11b,]#  & 
  ...
  #  res6x$var1=rep("ALL",nrow(res6x))
   ...
   #     } else {
    ...
    res6x$var1=paste(paste(samp1,collapse=":"),paste(count1,collapse=":"),paste(samp3,collapse=":"),paste(samp4,collapse=":"),paste(samp5,collapse=":"),paste(samp6,collapse=":"),paste(samp7,collapse=":"),paste(samp8,collapse=":"),paste(samp9,collapse=":"),paste(samp10,collapse=":"),paste(samp11a,collapse=":"),paste(samp11b,collapse=":"),sep="_")
...
,by=disease]
  ...
  r6x<-res6z[res6z$results_type=="Baseline",]
  setkey(r6x,disease2)
  
  r6z<-res6z[res6z$results_type=="Baseline" & res6z$Year==2030,]
  
  r6z[,names(r6z)[!names(r6z) %in% c("disease2","lm1_fitted","lm2_fitted")]:=NULL]
  names(r6z)[1]<-"X2030lm1"
  names(r6z)[2]<-"X2030lm2"
  setkey(r6z,disease2)
...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  ggplot(r6x[r6x$Kingdom!="fungi",],aes(y=lm1_fitted2*abs(cor1),x=Year,col=Kingdom)) +
    #geom_histogram()+
    ...
    #stat_summary(fun =  weighted.mean,w=abs(r6x$cor1), geom = "point",size=2,position = position_dodge(3)) +
  ...
  res6a$Group<-factor(res6a$Group,levels=c("Baseline  ","Emerging  ","No Hantaviridae ",sort(unique(res6a$Group)[!unique(res6a$Group) %in% c("Baseline  ","Emerging  ","No Hantaviridae ")])))
  
  
  aggregate(res6[,c("richness")],by=list(res6$model),mean)
  aggregate(res6[,c("richness")],by=list(res6$model),sd)
  
  ##results
  res6a[,mean(lm1*abs(cor1)),by=Group]
  
  ggplot(res6a[res6a$results_type!="SSP",],aes(y=lm1*abs(cor1),x=Group,group=Group,col=results_type)) +
