    ...
    #system.time(
		...
		for(z in 1:nrow(dis2)){
			...
			link4$density[link4$density>3000000]<-3000000
					...
					##PRESENT DAY FIRST
					link5<-link4[1, ]		
					if(link5$year!=2010){print("no present day");break}
				...
				st1<-Sys.time()
				res3 = foreach(i=2:nrow(link4), .combine=rbl,.packages=c("raster","data.table")) %dopar%  {
				  #for(y in yyy){
              ...
              ##if so just start here with IP
				  ...
				  res3<-data.table(species=specX,code=link5$code,cell.id=values(template2), clim_future=values(clims2),lc_suit_future=values(lcsuit2),abundance_future=values(k),realised_future=values(IP),i=i)
