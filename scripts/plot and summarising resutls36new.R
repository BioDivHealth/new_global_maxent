...
d1$Vectored[d1$Type %in% c("HUMAN->VECTOR->HUMAN","HUMAN->VECTOR->HUMAN->HUMAN")]<-2
...
#d2<-read.csv("C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\diseases4a.csv",stringsAsFactors=FALSE)
 ...
 # res7<-merge(res6,d2,by.x="disease",by.y="disease",all.x=TRUE,all.y=FALSE)
  ...
  res6q<-res6[ res6$AUC>0.5 & res6$Disease=="Disease",]
  
  ##
  aggregate(res6q[,c(5:15,36:46,82:94,)],by=list(res6q$Year,res6q$RCP,res6q$SSP),median,na.rm=TRUE)
  
  aggregate(res6q[,c(5:15,36:46,82:94)],by=list(res6q$Year,res6q$TempTrop),function (x) mean(x,trim=0.1,na.rm=TRUE))
  ((176249-168335)*(11.11111^2))/60
  ((274800-269577)*(11.11111^2))/60
  
  
  aggregate(res6q[,c(5:15,36:46,82:94)],by=list(res6q$Year),function (x) mean(x,trim=0.1,na.rm=TRUE))
  ((213840.0-212867.5)*(11.11111^2))/60
...
#  hist(temp1$lm1)
 ...
 # r6x[,mean(lm1*abs(cor1)),by=.(Year,Kingdom)]
  ...
  typesX<-types#c("Baseline","Accuracy","Vectored","RCP","Burden","Emerging","Hantaviridae","Kingdom","Tropical/Temperate","RCP/SSP","Threshold")
  ...
  ##write to disck
  #write.csv(res7,file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings29032022.csv")
  res7<-read.csv(file="C:\\Users\\david.redding\\Dropbox\\New_Global_MAXENT\\workings29032022.csv",stringsAsFactors = F)
    ...
    res7b<-res7[!is.na(res7$correlation) & res7$panel=="Baseline" ,]
    ...
    res7c$Panel<-gsub("Kingdom ","",ress7c$Panel,fixed=TRUE)
    ...
    gg<-
      ggplot(data=res7c[,],aes(x=Trend,y=Count,col=as.factor(Panel))) +
    #res7$uni<-paste(res7$SSP,res7$rep)]
