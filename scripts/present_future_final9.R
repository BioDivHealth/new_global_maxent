...
#for (i in (nums)){
 # gc()
  ...
  res6[,countr2:=ifelse(!is.na(countr) & !is.na(host_range),1,NA)]
  ...
  ##threshold again
    ...
    thr1<-do_auc(data1=res6a,test_column="Hazard",group_column="countr2",weights="dist_cent")
    thr2<-do_auc(data1=res6a,test_column="clim_present_mean2",group_column="countr2",weights="dist_cent")
  ...
  #cols1<-c("clim_present_mean2","Hazard","HazardT","HazardTD","Hazard_future","Hazard_futureTD","host_range","host_range2","RiskL")
    ...
    ##based on thomas et al (science 2011) its mean 16.9 or four cells so 7 decades times four is 28 but that is for 2080 #should prevent from going too quick - what about each host at time?
    #pl2<-suppressWarnings(buffr(pl1,distance=28,units="cell",mask=FALSE))
    pl2<-suppressWarnings(buffr(pl1,distance=4*((as.numeric(res9$year[1])-2010)/10),units="cell",mask=FALSE))
    
    res9[,diss_me:=extract(pl2,res9$cell.id)]
    ...
    res9b<-res9[ !is.na(res6$surrounding),]
    
    pl1x[res9b$cell.id]<-res9b[,names(res9b)[names(res9b)==cols1[zz]],drop=TRUE]
    
    #if(zz==9){xxx<-(log(res9$Hazard+1))-(log(res9$Hazard_future+1));xxx[xxx==0]=NA;pl1[res9$cell.id]<-xxx}
    
    pl1x<-crop(pl1x,extent( min(res9b$x)-5, max(res9b$x)+5,min(res9b$y)-5, max(res9b$y)+5))
    pl1x[pl1x==0]<-NA
