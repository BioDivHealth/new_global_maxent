...
#writeRaster(ws_ras,format="GTiff",file="W:\\DRtemp\\temp\\wrld_simpl_raster.tif")
##load land area mask
ws_ras<-raster("W:\\DRtemp\\temp\\wrld_simpl_raster.tif")

##mask template by area
template<-mask(template,ws_ras)

source("W:\\DRtemp\\functions6.r")
...
###read disease data
d1<-read.csv("W:\\DRtemp\\disease_table28.csv",stringsAsFactors=FALSE)

###look up all diseases
link1<-list.files("W:\\DRtemp\\temp\\disease_analyses/", pattern=".csv",full.names=TRUE)
...
points1a<-list.files("W:\\DRtemp\\temp\\points\\", pattern="all_points",full.names=TRUE)
points1<-list.files("W:\\DRtemp\\temp\\points\\", pattern="_points",full.names=TRUE)
points2b<-points1[!points1 %in% points1a]
points1a<-list.files("W:\\DRtemp\\temp\\points\\", pattern="all_points",full.names=FALSE)
points1<-list.files("W:\\DRtemp\\temp\\points\\", pattern="_points",full.names=FALSE)
...
#lt2<-fread("W:/DRtemp/temp\\livestock_future_2030_2050_2070_2080.csv")
...
#fwrite(lt3,file="W:/DRtemp/temp\\livestock_future_2030_2050_2070_2080b.csv")
#lt2<-fread("W:/DRtemp/temp\\livestock_future_2030_2050_2070_2080b.csv")
...
##extract people   
#futpopx<-stack(list.files("W:\\DRtemp\\Spatial_population_scenarios_GeoTIFF\\GeoTIFF\\",pattern="_2",recursive=TRUE,full.names=TRUE)[seq(2,250,by=5)])
...
#fwrite(ft3,file="W:/DRtemp/temp\\future_pop_maskedb.csv")
##ft2b<-fread("W:/DRtemp/temp\\future_pop_maskedb.csv")
...
#fwrite(ft2,file="W:/DRtemp/temp\\future_pop_maskedb.csv")
...
for (i in 1:length(years)){
  ...
  for(j in 3:7){
    t3<-template
    t3[ft2[[1]][ft2$time==years[i]]]<-ft2[[j]][ft2$time==years[i]]
  }
...
#fwrite(GP3,file="W:/DRtemp/temp\\future_gdppop_maskedb.csv")
#GP2<-fread("W:/DRtemp/temp\\future_gdppop_maskedb.csv")
...
#fwrite(GP2,file="W:/DRtemp/temp\\future_gdppop_maskedb.csv")
#GP2<-fread("W:/DRtemp/temp\\future_gdppop_maskedb.csv")
#setkey(GP2,cell_by_year) ##create cell.id by code??

####Get all future climate niches
link2b<-list.files("W:/DRtemp/resultsY", pattern=".tif",full.names=TRUE)
link2<-list.files("W:/DRtemp/resultsY", pattern=".tif",full.names=FALSE)
...
###lc
#futlgx2<-list.files("W:\\DRtemp\\temp\\input_masked\\",recursive=TRUE,pattern="lcXXX",full.names=TRUE)
#futlgx1<-futlgx2
...
#rm(futlgx2)
#futlg<-read.csv(file="W:\\DRtemp\\temp\\futlg.csv",stringsAsFactors=FALSE)
...
###fnf
#fnfx2<-list.files("W:\\DRtemp\\temp\\input_masked\\",recursive=TRUE,pattern="fnfXXX",full.names=TRUE)
#fnfx1<-fnfx2
...
fnf<-raster("W:\\DRtemp\\temp\\fnf_map.txt")
...
spec_hab<-read.csv("W:\\DRtemp\\habitat_both_fin.csv",stringsAsFactors=FALSE)
...
spec_dist<-read.csv("W:\\DRtemp\\host_density_d_distance.csv",stringsAsFactors=FALSE)
spec_desp<-read.csv("W:\\DRtemp\\dispersal_distance_all.csv",stringsAsFactors=FALSE)
spec_desp$binom<-gsub("_"," ",spec_desp$binom)

###get current bioclim data
#spdata<-stack("W:\\DRtemp\\predictorsX.tif")
#names(spdata)<-read.csv("W:\\DRtemp\\names1.csv",stringsAsFactors=FALSE)$x
...
#res10<-NULL
#gc()
for (x in sample(1:length(dis1),length(dis1),replace=FALSE)){
  ...
  ##choose one disease
	...
	#if(paste(" ",dis1[x]," present.r",sep="") %in% list.files("W:\\DRtemp\\per_disease\\", pattern="present.r",full.names=FALSE)){next}
    ...
    st2<-Sys.time()
		...
		for(z in 1:nrow(dis2)){
			...
			if(dis3$name1 %in% c("ducks","chickens","cattle","ducks","goats","human","pigs","sheep")){next}			
				...
				#if(paste(paste(ttt2,collapse="_"),".tif",sep="") %in% list.files("W:\\DRtemp\\temp\\masks\\", pattern=".tif",full.names=FALSE)){next}
				...
				### load a mask
				mask1<-raster(paste("W:\\DRtemp\\temp\\masks\\",paste(ttt2,collapse="_"),".tif",sep=""))
...
#			link4<-link4c;rm(link4b,link4c)
			...
			if(paste("V:\\present_day\\",dis3$disease,"_",link5$species,"_Present.r",sep="") %in% list.files("V:\\present_day\\",full.name=TRUE)){next}
			
			
					###future climate data
					clims<-raster(link5$filen)
					#clims2<-velox(clims)
					#clims2$crop(extent(template2))
			    ...
			    if(sum (values(IP),na.rm=TRUE)==0){next}
					...
					writeRaster(IPP,format="GTiff",file=paste("V:\\present_day\\",dis3$disease,"_",link5$species,"_IPP.tif",sep=""),overwrite=TRUE)
					
					save(res1,file=paste("V:\\present_day\\",dis3$disease,"_",link5$species,"_Present.r",sep=""))
					
					rm(IP,k,lcsuit,lcsuit2,specX,clims2)
					
	    }##end of z loop
					
	  print("END OF DISEASE")  
    		print(x)
    print("######")
}##end of x loop
