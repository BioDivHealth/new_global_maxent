...
pres1<-list.files("V:\\per_disease2\\",pattern=" ALL2",full.names=TRUE)
pres2<-list.files("V:\\per_disease2\\",pattern=" ALL2",full.names=FALSE)
pres2<-gsub("  ALL2.r","",pres2)
pres2<-gsub(" ALL2.r","",pres2)
...
for (i in i:length(pres1)){
  ...
  if(pres2[i]==" tensaw"){pres2[i]<-" tensaw "}
...
##save results
write.csv(res3,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\AUC_final3.csv")
...
nrow(res3[res3$AUC>0.55,])/nrow(res3[,])
