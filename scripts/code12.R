		...
		for(z in 1:nrow(dis2)){
			...
			link4$RCP[link4$RCP==999]<-2.6
				...
				for(y in 1:nrow(link4)){
					...
					### all mosquitos species ## several studies 40-60 species
					### 20-75000 per species including the rare species
					### 150-300,000 of single speciesin 4km2 [Constanini et al. Medical and Veterinary Entomology (1996) 10,203-219] 

					k<-lcsuit*mean(spec_dist2$Average.of.density*num_spec,na.rm=TRUE) ## make these number relavent to habitat type
					...
					IP[cellFromXY(IP,dt1)]<-mean(spec_dist2$Average.of.density*num_spec,na.rm=TRUE)
				...
				## run gas model??
				## issues of genera level will be lower than lots of species??
