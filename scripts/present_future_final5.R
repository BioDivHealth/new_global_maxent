...
for (i in (nums)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	if(noabs==1){AUC_cutoff<-0.1;AUC_cutoff_real<-0.1} else {AUC_cutoff<-mean(thr2[,2],na.rm=TRUE)}
	...
	res6<-res6[,c("cell.id","year","RCP","x","y","countr","sub_region","regionPA","surrounding","host_range","diss_me","year_RCP","clim_present_mean2","clim_future_mean2","Hazard","Hazard_future","HazardT","Hazard_futureT","year_diss_me")]
