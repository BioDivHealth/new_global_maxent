...
#names(popgdp6)[2:7]<-gsub("-","_",names(popgdp4)[2:7],fixed=TRUE)
...
for (i in (i+1):length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res6[,Hazard := clim_present_mean2 * lc_suit_present_mean ]
	
	###create realised present mean - abudance from habitat????
	res6[,Hazard_future:= clim_future_mean2 * lcsuit_future_mean ]
	...
	res7<-res6[!is.na(diss_me),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_future")]
  ...
  rm(res6);gc()
	...
	breaks1<-seq(from=0,to=max(log(res8$pop_ssp3+1),na.rm=TRUE),by=(max(log(res8$pop_ssp3+1),na.rm=TRUE)/10))
	
	res8[,ExposureSSP1:=cut(log(pop_ssp1+1),breaks=10,labels=FALSE)/10]
	...
	res8[,Exposure:=cut(log(humans2010+1),breaks=10,labels=FALSE)/10]
	
	breaks2<-seq(from=0,to=max(log(res8$gdp_ssp1+1),na.rm=TRUE),by=(max(log(res8$gdp_ssp1+1),na.rm=TRUE)/10))
	
	res8[,VulnerabilitySSP1:=(1-(cut(log(gdp_ssp1+1),breaks=breaks2,labels=FALSE)/10))*(gini_SSP1/100)]
	res8[,VulnerabilitySSP2:=(1-(cut(log(gdp_ssp2+1),breaks=breaks2,labels=FALSE)/10))*(gini_SSP2/100)]
	res8[,VulnerabilitySSP3:=(1-(cut(log(gdp_ssp3+1),breaks=breaks2,labels=FALSE)/10))*(gini_SSP3/100)]
	res8[,Vulnerability:=(1-(cut(log(gdp2010+1),breaks=breaks2,labels=FALSE)/10))*(X2010_GINI_PRESENT_C/100)]
...
##save results
write.csv(res11,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\final_risk1.csv")
