...
for (i in (i+1):length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	thresh<-quantile((res8$gdp2010*1000)/res8$humans2010,0.10)
	...
	hist(res8$Vulnerability[res8$year==2030])
	...
	res8[HazardT==0,c("Exposure","Vulnerability")]<-NA
	res8[Hazard_futureT==0,c("ExposureSSP1","ExposureSSP2","ExposureSSP3","VulnerabilitySSP1","VulnerabilitySSP2","VulnerabilitySSP3")]<-NA
	...
	res7<-res6[!is.na(diss_me),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","Hazard_future","HazardT","Hazard_futureT","clim_present_mean2","clim_future_mean2","Exposure","Vulnerability","ExposureSSP1","ExposureSSP2","ExposureSSP3","VulnerabilitySSP1","VulnerabilitySSP2","VulnerabilitySSP3")]
