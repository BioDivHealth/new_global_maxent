...
Auto_maxent<-function(
                      ...
                      min_obs = 10, # [NUMERICAL] Minimun number of observations to run the analysis
  ...
  list.of.packages<-c("dplyr","sf","terra","tidyr","ENMeval","parallel","dismo")
  ...
  if(mod.select==TRUE){
      if(n.m==1 | n.m<=n.mods){
        warning("Number of models to select equal or greather than models to run, check n.m and n.mods paramters! mod.select set to FALSE")
    #    mod.select=FALSE
  ...
  # 1.d Add a buffer around the study area ----
    sty.a <- sty.a %>% st_buffer(dist=buff_lim)
  ...
  if(select_var=="NUMERICAL"){
    vars <- var_select(x=mod.dat,VIF.threshold=5,cor.threshold=0.7)
    }
  ...
  if(select_var=="DIST"){
    ...
    if(length(vars)<3){
      print(paste("WARNING! Only",legnth(vars),"selected for the analysis!"))
    }else{
      print(paste(length(vars), "variables selected for the analysis"))
    } 
    
    mod.dat <- mod.dat[,colnames(mod.dat) %in% vars]
    
    if(type_bk %in% "EnvBK"){
      
      bk_points <-  bk_env(p.points=y_points,
                           env_var=pred.dat[[names(pred.dat) %in% vars]],
                           n_bk=n_bk,
                           density_pc=TRUE)[["points"]] %>% st_cast("POINT")
      ...
      obs_index <- obs_sp$presence %>% st_drop_geometry()
      mod.dat <- pred.dat[[names(pred.dat) %in% vars]] %>% terra::extract(obs_sp %>% vect(),ID=F)
    }
  }
  ...
  if(mod.select==TRUE){  
      ...
      # 6.3 Select the models based on its AICc and boice index
        if(n.mods>nrow(AICc_mods)){
          stop(paste0("Number of models (",n.mods,") to select, greater than number of tested models(",nrow(AICc_mods),")!"))
        }
        
        if(n.mods==nrow(AICc_mods)){
          print(paste0("WARNING: Number of models (",n.mods,") to select, equal than number of tested models(",nrow(AICc_mods),")!. All models will be selected"))
        }
