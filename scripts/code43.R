...
for (x in x:length(dis1)){
    ...
    ##create columns of current human population by cutting up ft2 and GP2 by 2010 and then matching by cell.id
    res6[,cell_present:=paste(cell.id,"2010",sep="_")]
    setkey(res6,cell_present)
    
    ##merge again
    res6b<-res6[ft2,nomatch=0]
    ...
    ### present day humans
    humans=res6$xxxx/5.6
    res6[,present_realised:= c((realised_present_mean*speed_mean*d_mean) * (realised_present_mean_vector*speed_mean_vector*d_mean_vector) *(secondary_present) * (humans2010 * 0.0001 * 5)),]
    ...
    library(dismo)
    AUC1<-evaluate(p=res6$present_realised[res6$ec==1],a=res6$present_realised[res6$ec==0])
