    ...
    #system.time(
		...
		for(z in 1:nrow(dis2)){
			...
			link4$density[link4$density>3000000]<-3000000
				...
				res3<-NULL				
				# <-----------------------------------   NEED TO MATCH VECTOR AND HOSTS (or summarise by RCP)
				##### <- choose the same things here at least one from each bothmod otherwise different non matching
				##### then no variation
				##### PARALLELISE
				
				#if(nrow(link4)>NUM1){yyy<-c(1,sample(2:nrow(link4),NUM1-1,replace=FALSE))}else{
				  yyy<-1:nrow(link4)#}
        ...
        #system.time(
				...
				foreach(y=yyy, .combine=rbl,.packages=c("raster","data.table")) %dopar%  {
				#for(y in yyy){
					...
					#res3<-data.table(species=specX,code=link5$code,cell.id=values(template2), clim_future=values(clims2),lc_suit_future=values(lcsuit2),abundance_future=values(k),realised_future=values(IP))
			  ...
			  #print(y)
			  #print("#########")
        #  			}##end of y foreach loopn ### some point sumarise by disease?? need to account for vectors? set a updating field for host presence? or vector presence or just create a bifg dataframe?
                #)##end of system.time
      stopCluster(cl)
	    
      res1<-res13[res13$code==x,]
      names(res1)[1:2]<-paste(names(res1)[1:2],"present",sep="_")

      res3<-res13[!res13$code==x,]
      names(res3)[1:2]<-paste(names(res3)[1:2],"future",sep="_")
