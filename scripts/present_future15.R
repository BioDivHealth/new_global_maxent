...
vvv=mean(GP2$value,na.rm=TRUE)
GP2[is.na(value),value:=vvv]
rm(gini3);gc()
names(GP2)[3:8]<-gsub("-","_",names(GP2)[3:8],fixed=TRUE)

##lauren's equation - check
## work out vulnerable per admin
GP2[,pov_frac_present:= ((1-( ((1+value)/(1-value)) * (1/(693.50/(gdp2010*100000000/(humans2010*10000)))) ^ ((1+value)/(2*value))))-1)/100]
GP2[,pov_frac_SSP1:= ((1-( ((1+value)/(1-value)) * (1/(693.50/(gdp_ssp1/(pop_ssp1*100000000)))) ^ ((1+value)/(2*value))))-1)/100]
GP2[,pov_frac_SSP2:= ((1-( ((1+value)/(1-value)) * (1/(693.50/(gdp_ssp2/(pop_ssp2*100000000)))) ^ ((1+value)/(2*value))))-1)/100]
GP2[,pov_frac_SSP3:= ((1-( ((1+value)/(1-value)) * (1/(693.50/(gdp_ssp3/(pop_ssp3*100000000)))) ^ ((1+value)/(2*value))))-1)/100]


#1-( ((1+value)/(1-value)) * (1/(693.50/(gdp2010/(humans2010*10000)))) )^ ((1+value)/(2*value))
#save(GP2,file="C:\\Users\\xxxx\\Dropbox\\New_Global_MAXENT\\GP2_gini.r")

###write file
fwrite(GP2,file="V:\\GP2_gini.csv")
...
for (i in i:length(pres1)){
  ...
  ### can measure intensity for only 10 or so diseases  
	...
	res8<-res6[GP2,nomatch=0]
	
	## final exposure
	## could include cattle??
	## could cut by at risk groups
	res8[,ExposureSSP1:=pop-ssp1]
	res8[,ExposureSSP2:=pop-ssp2]
	res8[,ExposureSSP3:=pop-ssp3]
	
	## final Vulnerability
	res8[,VulnerabilitySSP1:=pov_frac_futureSSP1]
	res8[,VulnerabilitySSP2:=pov_frac_futureSSP2]
	res8[,VulnerabilitySSP3:=pov_frac_futureSSP3]
	...
	res8[,RiskSSP2:=Hazard_future * VulnerabilitySSP3 * ExposureSSP3]
	
	###do again for present day
	res8[,RiskP:=Hazard * humans2010 * pov_frac_present]
	...
	#res7<-res6[!is.na(countr),lapply(.SD,function (x) mean(x,trim=0.1,na.rm=TRUE)),by=.(year_RCP,diss_me),.SDcols=c("Hazard","HazardUW","Hazard_future","Hazard_future_UW")]
