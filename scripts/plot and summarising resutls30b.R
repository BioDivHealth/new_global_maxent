...
###    Extend Data figure 2
  ...
  ##results
  res6a[,mean(na.omit(lm1*abs(cor1))),by=Group]
  
  hist(res6a[res6a$Group=="Vectored 1",lm1*abs(cor1)])
  hist(res6a[res6a$Group=="Vectored 0",lm1*abs(cor1)],col="red",add=TRUE)
  
  
  ggplot(res6a[!res6a$results_type %in% c("SSP","Regional","RCP/SSP") & res6a$threshold!="MIN" & res6a$Group!="Kingdom fungi",],aes(y=(cor1),x=Group,group=Group,col=results_type)) +
